...
ssh -i ssh-key-zion-edge root@77.42.71.94 "systemctl stop zion-edge zion-edge-pool" 2>$null
...
ssh -i ssh-key-zion-edge root@77.42.71.94 "rm -f /root/zion-2.9.6-main/data/*" 2>$null
...
$edgeStatus = ssh -i ssh-key-zion-edge root@77.42.71.94 "systemctl is-active zion-edge zion-edge-pool" 2>$null
if ($edgeStatus) {
...
Write-Host "  Edge: ssh -i ssh-key-zion-edge root@77.42.71.94 'journalctl -u zion-edge -f'" -ForegroundColor Gray