// ZION Pool v2.9.6 — L1 MainNet with CH v3 Revenue Orchestration
//
...
struct ApiState {
    ...
    merged_mining: Arc<MergedMiningManager>,
    start_time: i64,
    min_payout: f64,
...
async fn api_stats(State(state): State<ApiState>) -> Json<serde_json::Value> {
    ...
    Json(json!({
        ...
        "pool": {
            ...
            "humanitarian_tithe": state.humanitarian_tithe_percent,
            "issobella_fund": state.issobella_fund_percent,
            "miner_share": 100.0 - state.pool_fee_percent - state.humanitarian_tithe_percent - state.issobella_fund_percent,
            "min_payout": state.min_payout
        },
        "blockchain": {
            "height": height,
...
async fn api_external_mining(State(state): State<ApiState>) -> Json<serde_json::Value> {
    Json(json!({
        ...
        "merged_mining": state.merged_mining.stats_json(),
    }))
}
...
async fn main() {
    ...
    //
    // CPU-only mode (Helsinki ARM64): xmrig runs with 1 thread on MoneroOcean.
    ...
    let merged_mining = MergedMiningManager::from_env();
    ...
    let share_processor = Arc::new(ShareProcessor::new_with_merged_mining(
        validator,
        storage.clone(),
        Some(rpc_client.clone()),
        Some(merged_mining.clone()),
    ...
    server.set_revenue_proxy(revenue_proxy.clone());
    ...
    // Listen for mode_notify from StreamScheduler — push new Revenue jobs
    // to Revenue-group miners as soon as external pool jobs arrive.
    // This is critical for the first VRSC/XMR job after the proxy connects.
    {
        let scheduler = stream_scheduler.clone();
        let server_weak = Arc::downgrade(&server);
        tokio::spawn(async move {
            loop {
                scheduler.wait_mode_notify().await;
                if let Some(revenue_job) = scheduler.get_revenue_job().await {
                    let revenue_miners = scheduler.get_revenue_miners().await;
                    if !revenue_miners.is_empty() {
                        if let Some(server) = server_weak.upgrade() {
                            tracing::info!(
    ...
    let api_state = ApiState {
        ...
        merged_mining: merged_mining.clone(),
        ...
        issobella_fund_percent: 5.0, // L5/L6 ZION Issobella fund
