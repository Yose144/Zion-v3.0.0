...
// ── CHv3 byproduct / merged mining scaffolding ────────────────
static CHV3_BYPRODUCT_SAMPLES: OnceLock<IntCounter> = OnceLock::new();
static CHV3_BYPRODUCT_KECCAK_HITS: OnceLock<IntCounter> = OnceLock::new();
static CHV3_BYPRODUCT_SHA3_HITS: OnceLock<IntCounter> = OnceLock::new();
...
fn chv3_byproduct_samples() -> &'static IntCounter {
    CHV3_BYPRODUCT_SAMPLES.get_or_init(|| {
        IntCounter::new(
            "chv3_byproduct_samples_total",
            "Total CHv3 byproduct intermediate samples computed",
        )
        .unwrap()
    })
}

fn chv3_byproduct_keccak_hits() -> &'static IntCounter {
    CHV3_BYPRODUCT_KECCAK_HITS.get_or_init(|| {
        IntCounter::new(
            "chv3_byproduct_keccak_hits_total",
            "Total CHv3 keccak intermediate hit signals (leading zeros threshold)",
        )
        .unwrap()
    })
}

fn chv3_byproduct_sha3_hits() -> &'static IntCounter {
    CHV3_BYPRODUCT_SHA3_HITS.get_or_init(|| {
        IntCounter::new(
            "chv3_byproduct_sha3_hits_total",
            "Total CHv3 sha3 intermediate hit signals (leading zeros threshold)",
        )
        .unwrap()
    })
}
...
pub fn inc_chv3_byproduct_samples() {
    chv3_byproduct_samples().inc();
}

pub fn inc_chv3_byproduct_keccak_hits() {
    chv3_byproduct_keccak_hits().inc();
}

pub fn inc_chv3_byproduct_sha3_hits() {
    chv3_byproduct_sha3_hits().inc();
}
