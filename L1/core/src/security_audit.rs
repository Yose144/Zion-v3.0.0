...
async fn test_mempool_flooding(state: &State, results: &mut SecurityAuditResults) {
    ...
    if mempool_size > 5000 {
        results.add_finding(
            ...
            "dos".to_string(),
            format!(
                "Mempool accepts {} transactions (exceeds recommended 5000 limit)",
                mempool_size
            ),
        ...
        results.add_finding(
            ...
            "dos".to_string(),
            format!(
                "Mempool flooding took {:?} (should be < 10s for {} txs)",
                elapsed, flood_count
            ),
...
async fn test_large_transaction_attack(state: &State, results: &mut SecurityAuditResults) {
    ...
    if result.is_ok() {
        results.add_finding(
            "high".to_string(),
            "dos".to_string(),
            "Large transaction with 1000 inputs/outputs was accepted (should be rejected)"
                .to_string(),
        ...
        results.add_finding(
            ...
            "dos".to_string(),
            format!(
                "Large transaction validation took {:?} (should be < 100ms)",
                elapsed
            ),
...
async fn test_duplicate_transaction_attack(state: &State, results: &mut SecurityAuditResults) {
    ...
    if accepted > 1 {
        results.add_finding(
            ...
            "dos".to_string(),
            format!(
                "Duplicate transaction accepted {} times (should be rejected after first)",
                accepted
            ),
...
async fn test_pow_timing_leak(state: &State, results: &mut SecurityAuditResults) {
    ...
    if ratio > 1.5 || ratio < 0.67 {
        results.add_finding(
            ...
            "timing".to_string(),
            format!(
                "PoW validation timing differs by {:.1}x (valid: {:?}, invalid: {:?})",
...
fn generate_fuzzed_transaction(rng: &mut ChaCha8Rng) -> Transaction {
    ...
    let inputs: Vec<TxInput> = (0..num_inputs)
        .map(|_| TxInput {
            prev_tx_hash: (0..rng.gen::<usize>() % 128)
                .map(|_| format!("{:x}", rng.gen::<u8>()))
                .collect(),
            output_index: rng.gen::<u32>(),
            signature: (0..rng.gen::<usize>() % 256)
                .map(|_| format!("{:x}", rng.gen::<u8>()))
                .collect(),
            public_key: (0..rng.gen::<usize>() % 128)
                .map(|_| format!("{:x}", rng.gen::<u8>()))
                .collect(),
        })
        .collect();
