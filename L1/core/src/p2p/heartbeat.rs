    ...
    loop {
        ...
        if sync_status.is_ibd() && sync_status.is_stalled() {
            ...
            if exhausted {
                ...
                println!(
                    "⚠️ IBD stalled {} times, aborting. Will retry on next peer.",
                    sync::IBD_MAX_STALL_RETRIES
                );
            } else {
                // Re-request from where we left off
                let from = sync_status
                    .download_height
                    .load(std::sync::atomic::Ordering::Relaxed)
                    + 1;
                println!("⚠️ IBD stalled, re-requesting from height {}", from);
                peers
                    .broadcast(Message::GetBlocksIBD {
                        from_height: from,
        ...
        for peer_addr in &initial_peers {
            if let Ok(socket_addr) = peer_addr.parse::<SocketAddr>() {
                if !peers.is_connected(&socket_addr) {
                    ...
                    println!(
                        "Reconnecting to {} (attempt {}, backoff {}s)",
                        peer_addr,
                        failures + 1,
                        backoff_secs
                    );
