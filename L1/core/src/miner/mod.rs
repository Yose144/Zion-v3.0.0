    ...
    for nonce in 0..max_iterations {
        ...
        let hash = match algo {
            Algorithm::CosmicHarmony => {
                // CHv3 unified: delegates to zion_cosmic_harmony_v3 crate
                cosmic_harmony::hash(prefix_bytes, nonce, template.height)
            }
            Algorithm::Blake3 => {
                blake3_algo::blake3_hash_with_nonce(prefix_bytes, nonce as u32).to_vec()
            }
            Algorithm::RandomX => {
