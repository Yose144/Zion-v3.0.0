...
impl WhatToMineClient {
    ...
    pub async fn fetch_profitability(
        &self,
    ) -> anyhow::Result<HashMap<AlgorithmType, ProfitabilityData>> {
        // Check cache first
    ...
    fn process_whattomine_coins(
        ...
        for coin in coins.values() {
            if let Some((algo_type, ticker)) = algo_mapping.get(coin.algorithm.as_str()) {
                let price = prices.get(*ticker).copied().unwrap_or(coin.exchange_rate);
    ...
    pub async fn get_best_gpu_algorithm(
        &self,
    ) -> anyhow::Result<Option<(AlgorithmType, ProfitabilityData)>> {
        let data = self.fetch_profitability().await?;
    ...
    pub async fn get_best_cpu_algorithm(
        &self,
    ) -> anyhow::Result<Option<(AlgorithmType, ProfitabilityData)>> {
        let data = self.fetch_profitability().await?;
...
mod tests {
    ...
    async fn test_fallback_data() {
        ...
        if let Some(etc) = data.get(&AlgorithmType::Ethash) {
            ...
            println!(
                "ETC: ${:.2}/day revenue, ${:.2}/day profit",
