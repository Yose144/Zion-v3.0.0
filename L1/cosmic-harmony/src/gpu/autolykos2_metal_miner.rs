...
impl AutolykosTableInfo {
    ...
    fn calc_n(height: u64) -> u32 {
        ...
        let iters_number =
            ((height - AUTOLYKOS_INCREASE_START) / AUTOLYKOS_INCREASE_PERIOD + 1) as usize;
...
pub fn autolykos2_hash_cpu(header_hash: &[u8; 32], nonce: u64, height: u32, n: u32) -> [u8; 32] {
...
impl AutolykosMetalMiner {
    ...
    pub fn new(batch_size: usize) -> Result<Self, AutolykosMetalError> {
        let device = Device::system_default().ok_or(AutolykosMetalError::NoDevice)?;
    ...
    pub fn new_with_device(device: Device, batch_size: usize) -> Result<Self, AutolykosMetalError> {
        log::debug!(
            "🍎 Autolykos2 Metal miner (TABLELESS) initializing on: {}",
            device.name()
        );
        log::debug!(
            "   Available memory: {} MB",
        ...
        let mine_fn = library.get_function("autolykos2_mine", None).map_err(|e| {
            AutolykosMetalError::FunctionNotFound(format!("autolykos2_mine: {:?}", e))
        })?;
        let benchmark_fn = library
            .get_function("autolykos2_benchmark", None)
            .map_err(|e| {
                AutolykosMetalError::FunctionNotFound(format!("autolykos2_benchmark: {:?}", e))
            })?;
        ...
        let m_padding_buf =
            device.new_buffer_with_data(m_data.as_ptr() as *const _, m_data.len() as u64, options);

        log::debug!("   Params struct: {} bytes", params_size);
        log::debug!("   Result struct: {} bytes", result_size);
        log::debug!(
            "   M padding buffer: {} bytes (8 KB — TABLELESS mode!)",
            m_data.len()
        );
        log::debug!("   Threads per threadgroup: {}", threads_per_threadgroup);
        log::debug!("   Batch size: {}", batch_size);
        log::debug!(
            "✅ Autolykos2 Metal miner ready (TABLELESS — 0 bytes table, R computed on-the-fly)"
        );
    ...
    pub fn prepare_for_height(&mut self, height: u64) -> Result<(), AutolykosMetalError> {
        ...
        log::debug!(
            "🔧 Autolykos2 height {} → N={} — TABLELESS mode, no table needed!",
            height,
            info.n
        );
...
impl std::fmt::Display for AutolykosMetalError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::HeightNotSet => write!(
                f,
                "Autolykos2 height not set — call prepare_for_height() first"
            ),
            Self::TableNotLoaded => write!(
                f,
                "Autolykos2 height not set — call prepare_for_height() first"
            ),
            Self::TableTooLarge(need, have) => write!(
                f,
                "Table too large: need {:.2} GB, have {:.2} GB (use tableless mode)",
                *need as f64 / 1_073_741_824.0,
                *have as f64 / 1_073_741_824.0
            ),
...
mod tests {
    ...
    fn test_autolykos_calcn_ergo_test_vectors() {
        ...
        assert_eq!(AutolykosTableInfo::from_height(41_984_000).n, 2_143_944_600);
        // capped
    }
