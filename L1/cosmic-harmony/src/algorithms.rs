...
pub fn memory_hard_scratchpad(input: &[u8]) -> anyhow::Result<HashOutput> {
    if input.len() < 64 {
        return Err(anyhow::anyhow!(
            "memory_hard_scratchpad expects 64-byte input, got {}",
            input.len()
        ));
    }
...
mod tests {
    ...
    fn test_full_pipeline_matches_opt() {
        ...
        for &header in headers {
            ...
            let s4 = memory_hard_scratchpad(&s3.hash).unwrap();
            let legacy_final = cosmic_fusion(&s4.hash).unwrap();
