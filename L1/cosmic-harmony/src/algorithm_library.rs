...
impl AlgorithmModuleLibrary {
    ...
    pub fn list_native_algorithms(&self) -> Vec<&AlgorithmInfo> {
        self.algorithms.values().filter(|a| a.is_native).collect()
    }
...
impl PipelineExecutionResult {
    ...
    pub fn get_export_hashes(&self) -> Vec<(&AlgorithmResult, &str)> {
        self.algorithm_results
            .iter()
            .filter_map(|r| r.export_target.as_ref().map(|t| (r, t.as_str())))
            .collect()
    }
}
...
mod tests {
    ...
    async fn test_dynamic_switch() {
        ...
        lib.switch_dynamic_slot(AlgorithmType::KawPow)
            .await
            .unwrap();
    ...
    async fn test_invalid_pipeline() {
        ...
        let invalid = vec![AlgorithmType::Keccak256, AlgorithmType::Sha3_512];
