...
mod tests {
    ...
    fn test_ekam_differs_from_original() {
        // Ekam must produce DIFFERENT output than original scratchpad
        // (different init primitive = different scratchpad content = different hash)
        let input = [7u8; 64];
        let original = crate::scratchpad::memory_hard_transform(&input);
        let ekam = memory_hard_transform_ekam(&input);
        assert_ne!(
            original.data, ekam.data,
            "Ekam must differ from original (different primitives)"
        );
    }
}
