...
impl DaoClient {
    ...
    pub async fn submit_grant_proposal(&self, req: &DaoProposalRequest) -> FreeWorldResult<DaoProposalResponse> {
        ...
        let resp = self.http
            ...
            .send()
            .await
            .map_err(|e| crate::error::FreeWorldError::Other(format!("DAO API error: {}", e)))?;

        if !resp.status().is_success() {
            return Err(crate::error::FreeWorldError::Other(
                format!("DAO API returned {}", resp.status())
            ));
        }

        let body = resp.json::<DaoProposalResponse>().await
            .map_err(|e| crate::error::FreeWorldError::Other(format!("DAO parse error: {}", e)))?;
