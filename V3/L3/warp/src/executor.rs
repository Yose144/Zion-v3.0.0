...
use crate::adapter::{create_adapter, ChainAdapter};
use crate::error::{WarpError, WarpResult};
use crate::protocol::{MintInstruction, ValidatorSignature, WarpMessage};
use crate::router::WarpRouter;
use crate::types::WarpStatus;
...
impl OutboundExecutor {
    ...
    async fn execute_transfer(&self, transfer_id: uuid::Uuid) -> WarpResult<()> {
        ...
        let amount_dest_atomic = crate::types::convert_decimals(
        ...
        self.wait_for_confirmations(transfer_id, &*adapter, &tx_hash)
            .await?;
    ...
    fn test_executor_creation() {
        ...
        let _executor = OutboundExecutor::new(
            Arc::new(Mutex::new(WarpRouter::new(
                crate::registry::ChainRegistry::with_defaults(),
                crate::fees::FeeEngine::with_defaults(),
                validator_set.clone(),
            ))),
            validator_set,
        );
