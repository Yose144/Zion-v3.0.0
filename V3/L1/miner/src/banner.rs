...
pub fn print_banner(threads: usize) {
    ...
    rows.push(("consensus".to_string(), zion_core::consensus_profile().to_string()));
    rows.push(("protocol".to_string(), zion_pool::protocol_version().to_string()));
    ...
    {
        if is_x86_feature_detected!("avx512f") { simd_caps.push("AVX-512"); }
        if is_x86_feature_detected!("avx2") { simd_caps.push("AVX2"); }
        if is_x86_feature_detected!("sse4.1") { simd_caps.push("SSE4.1"); }
        if is_x86_feature_detected!("aes") { simd_caps.push("AES-NI"); }
    }
    ...
    rows.push((
        "simd".to_string(),
        if simd_caps.is_empty() { "none".to_string() } else { simd_caps.join(", ") },
    ));
    ...
    {
        rows.push(("gpu".to_string(), "disabled (compile with --features gpu-opencl)".to_string()));
    }
