...
mod tests {
    ...
    fn oasis_target_rejects_remote_without_override() {
        let err = parse_oasis_http_target("http://203.0.113.2:8094", false)
    ...
    fn oasis_target_allows_remote_with_override() {
        let (authority, path) = parse_oasis_http_target("http://203.0.113.2:8094/base", true)
            .expect("remote URL should be allowed when override is enabled");
        assert_eq!(authority, "203.0.113.2:8094");
        assert_eq!(path, "/base");
    }
