...
pub async fn run(cfg: &Config) -> Result<()> {
    ...
    let pool_alive = tcp_probe(&cfg.pool.host, cfg.pool.port, std::time::Duration::from_secs(3));
    if pool_alive {
        ui::print_ok(&format!("L1 stratum   {}:{} accepting connections", cfg.pool.host, cfg.pool.port));
    } else {
        ui::print_warn(&format!("L1 stratum   {}:{} not reachable", cfg.pool.host, cfg.pool.port));
    }
