...
async fn fetch_pool_stats(state: &AppState) -> anyhow::Result<PoolStats> {
    let resp: serde_json::Value = state.http
        .get(format!("{}/stats", state.pool_url))
    ...
    Ok(PoolStats {
        hashrate: resp["hashrate"]["pool"].as_f64().unwrap_or(0.0),
        hashrate_1h: resp["hashrate"]["pool_1h"].as_f64().unwrap_or(0.0),
        hashrate_24h: resp["hashrate"]["pool_24h"].as_f64().unwrap_or(0.0),
        active_miners: resp["miners"]["active"].as_u64().unwrap_or(0),
        total_miners: resp["miners"]["total"].as_u64().unwrap_or(0),
        valid_shares: resp["shares"]["valid"].as_u64().unwrap_or(0),
        invalid_shares: resp["shares"]["invalid"].as_u64().unwrap_or(0),
        total_shares: resp["shares"]["total"].as_u64().unwrap_or(0),
        blocks_found: resp["blocks"]["found"].as_u64().unwrap_or(0),
        accept_rate: {
            let v = resp["shares"]["valid"].as_f64().unwrap_or(0.0);
            let t = resp["shares"]["total"].as_f64().unwrap_or(1.0);
            if t > 0.0 { (v / t) * 100.0 } else { 0.0 }
        },
    })
}
