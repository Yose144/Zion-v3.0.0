...
fn make_treasury(balance: u64) -> Treasury {
...
fn test_db_proposal_status_update() {
    ...
    db.update_proposal_status(&row).unwrap();
...
fn test_full_e2e_treasury_proposal() {
    ...
    db.update_proposal_status(&row2).unwrap();
...
fn make_proposal_treasury(id: u64) -> Proposal {
    Proposal::new(
        ...
        "zion1proposer000000000000000000000000001".into(),
        2_000_000_000_000, // 2M ZION (above threshold)
        100_000,
    )
}
...
fn make_proposal_parameter(id: u64, param: &str, value: &str) -> Proposal {
    Proposal::new(
        id,
        format!("Change {}", param),
        format!("Update governance parameter {} to {}", param, value),
        ProposalType::Parameter {
            parameter_name: param.into(),
            current_value: "old".into(),
            proposed_value: value.into(),
...
fn make_proposal_emergency() -> Proposal {
    Proposal::new(
        99,
        "Emergency: Pause Bridge".into(),
        "Critical relay bug discovered — pausing to prevent loss of funds".into(),
        ProposalType::Emergency {
            action: "pause_bridge".into(),
            justification: "Critical bug discovered in relay".into(),
        },
...
fn test_db_proposal_status_update() {
    ...
    let mut p = db.get_proposal(4).unwrap().unwrap();
    assert_eq!(p.status, "Active");
    p.status = "Passed".into();
    db.update_proposal_status(&p).unwrap();

    let p2 = db.get_proposal(4).unwrap().unwrap();
    assert_eq!(p2.status, "Passed");
}
...
fn test_voting_engine_add_votes() {
    ...
    // 60% yes
    engine.cast_vote(&mut proposal, "zion1v1", VoteChoice::Yes,     6_000_000).unwrap();
    engine.cast_vote(&mut proposal, "zion1v2", VoteChoice::No,      3_000_000).unwrap();
    engine.cast_vote(&mut proposal, "zion1v3", VoteChoice::Abstain, 1_000_000).unwrap();
...
fn test_voting_engine_deduplication() {
    ...
    engine.cast_vote(&mut proposal, "zion1v1", VoteChoice::Yes, 1_000_000).unwrap();
    // Same voter — should be rejected
    let r = engine.cast_vote(&mut proposal, "zion1v1", VoteChoice::No, 1_000_000);
    assert!(r.is_err());
    assert_eq!(proposal.voter_count, 1);
}
...
fn test_voting_engine_rejected_if_closed() {
    let mut proposal = make_proposal_treasury(12);
    proposal.status = ProposalStatus::Passed; // already closed
    let mut engine = VotingEngine::new();

    let r = engine.cast_vote(&mut proposal, "zion1v1", VoteChoice::Yes, 1_000_000);
    assert!(r.is_err());
}
...
#[test]
fn test_quorum_treasury_met() {
    let circulating_supply = 100_000_000_000_000u64; // 100M ZION (atomic)
    let mut proposal = make_proposal_treasury(20);
    let mut engine = VotingEngine::new();
    let checker = QuorumChecker::new(circulating_supply);

    // 15 voters × 1M ZION = 15M = 15% > 15% quorum for Treasury
    for i in 1..=15 {
        engine
            .cast_vote(&mut proposal, &format!("zion1v{:03}", i), VoteChoice::Yes, 1_000_000_000_000)
            .unwrap();
    }

    // Treasury quorum = 15%
    assert!(checker.is_quorum_met(&proposal));
}

#[test]
fn test_quorum_not_met() {
    let circulating_supply = 100_000_000_000_000u64;
    let mut proposal = make_proposal_treasury(21);
    let mut engine = VotingEngine::new();
    let checker = QuorumChecker::new(circulating_supply);
...
fn test_quorum_emergency_higher() {
    ...
    // 15% voted — passes standard (10%) but NOT emergency (20%)
    for i in 1..=15 {
        engine
            .cast_vote(&mut proposal, &format!("zion1v{:03}", i), VoteChoice::Yes, 1_000_000_000_000)
            .unwrap();
    ...
    assert!(!checker.is_quorum_met(&proposal), "Emergency needs 20%, got 15%");
}
...
fn test_executor_treasury_spend() {
    ...
    let mut timelock = Timelock::new_expired(); // already passed
    let mut treasury = make_treasury(100_000_000_000_000); // 100M ZION
    let mut config = DaoConfig::default();
...
fn test_executor_humanitarian_grant() {
    ...
    let mut timelock = Timelock::new_expired();
    let mut treasury = make_treasury(100_000_000_000_000);
    let mut config = DaoConfig::default();
...
fn test_executor_parameter_change() {
    ...
    let mut timelock = Timelock::new_expired();
    let mut treasury = make_treasury(0); // not needed for param change
    let mut config = DaoConfig::default();
    assert_eq!(config.quorum_percent, 10.0);
...
fn test_executor_parameter_invalid_value() {
    ...
    assert!(apply_parameter_change(&mut cfg, "quorum_percent", "abc").is_err());
    assert!(apply_parameter_change(&mut cfg, "quorum_percent", "99.0").is_err());
    assert!(apply_parameter_change(&mut cfg, "nonexistent", "1").is_err());
}
...
#[test]
fn test_executor_rejects_non_timelocked() {
    let mut proposal = make_proposal_treasury(33);
    // status = Active (not Timelocked)
    let mut timelock = Timelock::new_expired();
    let mut treasury = make_treasury(100_000_000_000_000);
    let mut config = DaoConfig::default();
...
fn test_executor_rejects_active_timelock() {
    ...
    let mut timelock = Timelock::new_active(); // still locked!
    let mut treasury = make_treasury(100_000_000_000_000);
    let mut config = DaoConfig::default();
...
fn test_full_e2e_treasury_proposal() {
    ...
    let mut proposal = make_proposal_treasury(100);
    let mut engine = VotingEngine::new();
    let circulating_supply = 100_000_000_000_000u64; // 100M ZION
    let checker = QuorumChecker::new(circulating_supply);
    let mut treasury = make_treasury(100_000_000_000_000);
    let mut config = DaoConfig::default();
    ...
    // 3. Vote — 20 voters × 1M ZION = 20M = 20% > 15% quorum
    for i in 1..=20 {
        let choice = if i <= 15 { VoteChoice::Yes } else { VoteChoice::No };
        let voter = format!("zion1voter{:03}", i);
        engine.cast_vote(&mut proposal, &voter, choice, 1_000_000_000_000).unwrap();
        db.record_vote(100, &voter, choice, 1_000_000, None).unwrap();
    }

    // 4. Check quorum
    assert!(checker.is_quorum_met(&proposal), "Quorum must be met");

    // 5. Mark Passed
    proposal.status = ProposalStatus::Passed;
    let row = db.get_proposal(100).unwrap().unwrap();
    let mut updated_row = row;
    updated_row.status = "Passed".into();
    db.update_proposal_status(&updated_row).unwrap();

    // 6. Timelock — skip with new_expired()
    let mut timelock = Timelock::new_expired();
    proposal.status = ProposalStatus::Timelocked;
...
fn test_full_e2e_parameter_proposal() {
    ...
    // Vote with 12% of supply (> 10% standard quorum)
    for i in 1..=12 {
        engine
            .cast_vote(&mut proposal, &format!("zion1p{:02}", i), VoteChoice::Yes, 1_000_000_000_000)
            .unwrap();
    }
