    ...
    match parameter_name {
        ...
        "daily_spend_limit" => {
            let v: u64 = proposed_value.parse().map_err(|_| {
                DaoError::Internal(format!("daily_spend_limit must be u64 (atomic), got '{}'", proposed_value))
            })?;
            // Max 500M ZION/day = 500_000_000_000_000 atomic units
            if v > 500_000_000_000_000 {
            ...
            Ok(format!("daily_spend_limit: {} → {} (atomic ZION)", old, v))
        }
        "multisig_threshold" => {
            let v: u32 = proposed_value.parse().map_err(|_| {
        ...
        "proposal_threshold" => {
            let v: u64 = proposed_value.parse().map_err(|_| {
                DaoError::Internal(format!("proposal_threshold must be u64 (atomic), got '{}'", proposed_value))
            })?;
