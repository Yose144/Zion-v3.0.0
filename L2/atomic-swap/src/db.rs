...
impl SwapDb {
    ...
    pub fn insert_htlc(&self, rec: &HtlcRecord) -> SwapResult<()> {
        ...
        conn.execute(
            r#"INSERT INTO htlc_locks
               (hash_hex, locker_address, amount_atomic, lock_tx_id, lock_block_height,
    ...
    pub fn get_htlc(&self, hash_hex: &str) -> SwapResult<Option<HtlcRecord>> {
        ...
        let res = conn
            .query_row(
                r#"SELECT hash_hex, locker_address, amount_atomic, lock_tx_id,
...
fn row_to_record(row: &rusqlite::Row<'_>) -> rusqlite::Result<HtlcRecord> {
    ...
    Ok(HtlcRecord {
        ...
        amount_atomic: row.get::<_, i64>(2)? as u64,
...
mod tests {
    ...
    fn insert_and_get() {
        ...
        assert_eq!(fetched.amount_atomic, 5_000_000);
    }
