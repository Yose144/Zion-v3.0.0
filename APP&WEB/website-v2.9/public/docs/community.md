# Komunita & Ekosystém

---

## Odkazy

- **Web**: [zionterranova.com](https://www.zionterranova.com)
- **GitHub**: [github.com/Zion-TerraNova](https://github.com/Zion-TerraNova)
- **Discord**: [discord.gg/zion-terranova](https://discord.gg/zion-terranova)
- **Explorer**: [zionterranova.com/explorer](https://www.zionterranova.com/explorer)

---

## Repozitáře

| Repo | Popis |
|------|-------|
| [2.9.5-NativeAwakening](https://github.com/Zion-TerraNova/2.9.5-NativeAwakening) | Nativní binárky, Docker, deployment |
| [v3-Mainnet](https://github.com/Zion-TerraNova/v3-Mainnet) | Mainnet příprava |
| [2.9-QuantumLeap](https://github.com/Zion-TerraNova/2.9-QuantumLeap) | Python miner a nástroje |
| [Zion-TestNet2.8.5](https://github.com/Zion-TerraNova/Zion-TestNet2.8.5) | Historický testnet |

---

## Jak přispět

1. **Těžbou** — každý node posiluje decentralizaci sítě
2. **Kódem** — forkni repo, pošli PR
3. **Reportováním bugů** — vytvoř issue na GitHubu
4. **Dokumentací** — pomoz vylepšit tuto dokumentaci
5. **Komunitou** — šiř povědomí, pomáhej nováčkům

---

## Roadmap highlights

| Milník | Stav |
|--------|------|
| Rust core (v2.9.5) | ✅ Hotovo |
| Cosmic Harmony v3 | ✅ Hotovo |
| Testnet multi-node | ✅ Běží |
| Nativní binárky | ✅ Hotovo |
| v2.9.6 Decade Decay emise | ✅ Hotovo |
| 6-Layer "On the Star" architektura | ✅ Hotovo |
| L6 ZION Issobella naming | ✅ Hotovo |
| Dual-mining ZION + VRSC | ✅ Hotovo |
| Desktop wallet | 🔄 Ve vývoji |
| Mobile wallet | 🔄 Ve vývoji |
| Mainnet launch | 📅 31. 12. 2026 |

Kompletní roadmap viz [Roadmap →](#whitepaper-roadmap).

---

## ZION Oasis

ZION Oasis je ekonomický ekosystém (L4) zahrnující:

- **Golden Egg** — odměny pro early adoptery
- **DAO Treasury** — komunitní správa fondů
- **Humanitární fond** — 5% block reward + 1,44 mld ZION premine
- **L5/L6 Issobella Fund** — 5% block reward pro Free World + vesmírnou stanici
- **Infrastrukturní fond** — udržitelný rozvoj

Více v [ZION Oasis →](#zion-oasis).

---

## Kontakt

- Otevři issue na [GitHub](https://github.com/Zion-TerraNova)
- Přidej se na [Discord](https://discord.gg/zion-terranova)

---

*ZION TerraNova v2.9.6*
