  ...
  if (blocks.length === 0) {
    return (
      <div className="flex items-center justify-center h-16 text-xs text-gray-500">
        {cs ? 'Čekání na bloky…' : 'Waiting for blocks…'}
      </div>
    );
  }
    ...
    <section className="zion-rainbow-card p-8" style={{ '--rc': '6, 182, 212' } as React.CSSProperties}>
      <div className="flex flex-col gap-2 mb-6">
        ...
        <p className="text-sm text-gray-400">
          {cs
            ? 'Živé metriky konsenzu: finalita, reorg detekce, indikátor forku, orphan rate a propagace bloků.'
            : 'Live consensus metrics: finality, reorg detection, fork indicator, orphan rate, and block propagation.'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <StatCard
          ...
          label={cs ? 'Reorg (1h)' : 'Reorgs (1h)'}
          value={`${reorgCount}`}
          sub={reorgCount === 0 ? (cs ? 'Žádné reorg' : 'No reorgs') : (cs ? 'Detekováno!' : 'Detected!')}
        ...
        <StatCard
          ...
          label={cs ? 'Fork' : 'Fork'}
          value={metrics.forkDetected ? (cs ? 'Ano' : 'Yes') : (cs ? 'Ne' : 'No')}
          sub={`${metrics.altBlocks} ${cs ? 'alt bloků' : 'alt blocks'}`}
        ...
        <StatCard
          ...
          label={cs ? 'Orphan rate' : 'Orphan Rate'}
          value={`${metrics.orphanRate.toFixed(2)}%`}
          sub={cs ? 'Alt / celkem bloků' : 'Alt / total blocks'}
        ...
        <StatCard
          ...
          label={cs ? 'Diff adj.' : 'Diff Adj.'}
          value={`${metrics.estNextChangePct > 0 ? '+' : ''}${metrics.estNextChangePct.toFixed(1)}%`}
          sub={cs ? 'Odhad dalšího bloku' : 'Next block est.'}
        ...
        <StatCard
          ...
          label={cs ? 'Poslední blok' : 'Last Block'}
          value={lastBlockAge != null ? fmtAgo(lastBlockAge) : '—'}
          sub={`${cs ? 'Průměr' : 'Avg'}: ${metrics.avgBt.toFixed(0)}s ±${metrics.stdDev.toFixed(0)}s`}
      ...
      <div className="zion-rainbow-sub p-5" style={{ '--rc': '6, 182, 212' } as React.CSSProperties}>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-3 text-[10px]">
            <div className="flex items-center gap-1">
              ...
              <span className="text-gray-500">{cs ? 'Finalizováno' : 'Finalized'}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-2 h-2 bg-purple-500 rounded-sm" />
              <span className="text-gray-500">{cs ? 'Čeká' : 'Pending'}</span>
            </div>
            <div className="flex items-center gap-1">
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div>
            <span className="text-gray-500">{cs ? 'Finalita od' : 'Finality from'}: </span>
            <span className="font-mono text-emerald-400">#{fmtNum(metrics.finalityHeight)}</span>
          </div>
          <div>
            <span className="text-gray-500">{cs ? 'Obtížnost' : 'Difficulty'}: </span>
            <span className="font-mono text-purple-400">{fmtLarge(stats.difficulty)}</span>
          </div>
          <div>
            <span className="text-gray-500">{cs ? 'Cíl bloku' : 'Block target'}: </span>
