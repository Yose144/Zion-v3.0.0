...
export default function Pool24hCharts({
  cs,
  hashrateData,
  acceptRateData,
  activeMinersData,
  ...
  cs: boolean;
  hashrateData: number[];
  acceptRateData: number[];
  activeMinersData: number[];
}) {
  const charts: ChartData[] = useMemo(
    ...
    [hashrateData, acceptRateData, activeMinersData]
  );
  ...
  return (
    <section className="zion-rainbow-card p-8" style={{ '--rc': '228, 30, 43' } as React.CSSProperties}>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {charts.map((chart) => (
          <div key={chart.label} className="zion-rainbow-sub p-5 flex flex-col" style={{ '--rc': '228, 30, 43' } as React.CSSProperties}>
