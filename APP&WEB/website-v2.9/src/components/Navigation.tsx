...
import {
  ...
  Coins, Wallet, BookOpen, Github, Layers, Network as NetworkIcon,
} from 'lucide-react';
    ...
    <nav className="fixed top-20 sm:top-24 md:top-28 left-0 right-0 z-50">
      ...
      <div className="relative mx-auto w-[min(96vw,1080px)] py-1" data-nav-desktop>
        {/* ═══════════════════════════════════════════════════
            ...
            ═══════════════════════════════════════════════════ */}
        <div className="relative rounded-2xl border border-white/10 bg-black/50 backdrop-blur-xl p-[1px] shadow-[0_16px_60px_rgba(0,0,0,0.35)]">
          {/* Rasta gradient top accent line — thicker and narrower */}
          <div className="pointer-events-none absolute -top-px left-12 right-12 h-1 rounded-t-2xl bg-linear-to-r from-emerald-400/60 via-zion-gold/70 to-red-400/60" />
          <div className="relative flex items-center justify-between gap-1.5 rounded-[15px] bg-black/70 px-2 sm:px-3 py-1"
            style={{
        ...
        {/* ═══ COMPACT TRANSPARENT FLOOR 2 — GROUP DROPDOWNS ONLY ═══ */}
        <div className="mt-1 hidden md:flex justify-center">
          <div className="flex items-center gap-1 rounded-2xl border border-white/[0.07] bg-black/20 backdrop-blur-md px-2 py-1 shadow-none">
            {navGroups.map((group) => {
              ...
              return (
                <div key={group.title} className="relative shrink-0">
                  <button
                    ...
                    className={`group inline-flex items-center gap-1 rounded-xl border px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] transition-all ${
                      isActive
                        ? 'border-zion-gold/40 bg-zion-gold/10 text-white shadow-[0_0_12px_rgba(251,191,36,0.12)]'
                        : groupHasActiveChild
                          ? 'border-emerald-500/30 bg-white/[0.04] text-white hover:border-zion-gold/40'
