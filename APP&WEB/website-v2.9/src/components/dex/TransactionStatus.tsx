...
const STATUS_CONFIG: Record<string, { icon: any; color: string; bg: string; label: string; spin?: boolean }> = {
  ...
  executing: { icon: Loader2, color: 'text-amber-400', bg: 'bg-amber-500/10', label: 'Executing', spin: true },
  completed: { icon: CheckCircle2, color: 'text-green-400', bg: 'bg-green-500/10', label: 'Completed' },
  failed: { icon: XCircle, color: 'text-red-400', bg: 'bg-red-500/10', label: 'Failed' },
  refunded: { icon: ArrowRight, color: 'text-orange-400', bg: 'bg-orange-500/10', label: 'Refunded' },
};
          ...
          return (
            <div key={i} className="flex items-center gap-3 p-2 bg-zinc-800/40 rounded-lg">
              ...
              <div className="flex-1">
                ...
                {step.error && (
                  <div className="text-xs text-red-400 mt-0.5">{step.error}</div>
                )}
              </div>
