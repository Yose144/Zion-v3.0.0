    ...
    <div className="flex flex-col gap-1.5" ref={ref}>
      ...
      <div className="relative">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 bg-zinc-900/80 border border-zinc-700/50 rounded-xl hover:border-amber-500/50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          ...
          <div className="absolute z-50 mt-1 w-full bg-zinc-900 border border-zinc-700/50 rounded-xl shadow-2xl max-h-64 overflow-y-auto">
            {tokens.map(token => (
              ...
              >
                ...
                {token.symbol === value && <Check className="w-4 h-4 text-amber-500" />}
              </button>
            ))}
          </div>
        )}
