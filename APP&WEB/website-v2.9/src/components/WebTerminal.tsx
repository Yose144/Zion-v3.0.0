        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-gradient-to-r from-zion-purple/10 to-zion-cyan/10 px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1.5">
              <div className="h-3 w-3 rounded-full bg-zion-purple/70" />
              <div className="h-3 w-3 rounded-full bg-zion-gold/70" />
              <div className="h-3 w-3 rounded-full bg-zion-cyan/70" />
            </div>
            ...
            <div className="ml-2 flex items-center gap-1" title={nodeOnline === null ? 'Checking…' : nodeOnline ? 'Node online' : 'Node offline'}>
              <motion.div
                className={`h-2 w-2 rounded-full ${nodeOnline === null ? 'bg-gray-500' : nodeOnline ? 'bg-zion-cyan' : 'bg-zion-purple'}`}
                  ...
                  {line.type === 'output' && (
                    <div className="relative">
                      ...
                      <button
                        ...
                        title="Copy output"
                      >
                        {copiedId === line.id ? <Check className="h-3 w-3 text-zion-cyan" /> : <Copy className="h-3 w-3" />}
                      </button>
                    </div>
                  )}
                  {line.type === 'error' && (
                    <div className="relative">
                      <div className="flex items-start gap-1.5 whitespace-pre-wrap break-words text-zion-purple">
                        <span className="shrink-0 text-zion-purple">✗</span>
                        <span>{line.text}</span>
                      </div>
                      <button
                        onClick={(e) => { e.stopPropagation(); copyLine(line.id, line.text); }}
