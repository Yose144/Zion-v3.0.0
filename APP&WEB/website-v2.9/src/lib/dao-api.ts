/**
 * ZION DAO API Client
 * Connects to the Rust zion-dao daemon (axum, port 8080).
 *
 * Endpoint base (override via NEXT_PUBLIC_DAO_API_URL):
 *   production  → https://zionterranova.com/dao-api  (nginx proxy → Helsinki:8080)
...
const DAO_BASE =
  ...
  (typeof window !== 'undefined' && window.location.hostname !== 'localhost'
    ? '/dao-api'           // nginx proxy to Helsinki:8080 in production
    : 'http://localhost:8080');
