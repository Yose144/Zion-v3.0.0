...
const t = {
  cs: {
    ...
    heroStats: [
      ...
      { label: 'Status', value: 'Ve vývoji', descriptor: 'Launch odložen' },
    ],
    ...
    phase4Title: 'Veřejný launch & Security Closure (odloženo)',
    phase4Period: 'Odloženo (TBD)',
    phase4Priority: '⏸️ P0 Blocker → On Hold',
    phase4Desc: 'Veřejný launch 31. 12. 2026 se odkládá. Phase 1 Foundation je kompletní, ale pro veřejný start potřebujeme projít Maturity Gate (Maturitou), zajistit základní likviditu a sestavit tým dobrovolných vývojářů. Do té doby bude síť nadále v aktivním vývoji a testování.',
    ...
    genesisReserveSub: '16,78B ZION · veřejný souhrn',
    security: 'Bezpečnost',
    securityTitle: 'Launch Readiness Security Checklist',
    securityCompleted: 'dokončeno',
    ctaTitle: 'Public Launch Gate — odloženo',
    ctaSubtitle: 'L1 je srdce, které stavíme zdola nahoru. Mainnet běží, L2 je live, ale veřejný launch je odložen, dokud neprojdeme Maturity Gate, nezajistíme likviditu a nenajdeme tým dobrovolných vývojářů.',
  ...
  en: {
    ...
    heroStats: [
      ...
      { label: 'Status', value: 'In development', descriptor: 'Launch postponed' },
    ],
    ...
    phase4Title: 'Public Launch & Security Closure (postponed)',
    phase4Period: 'Postponed (TBD)',
    phase4Priority: '⏸️ P0 Blocker → On Hold',
    phase4Desc: 'The public launch on 31 Dec 2026 is postponed. Phase 1 Foundation is complete, but a public start requires passing the Maturity Gate, securing basic liquidity, and assembling a team of volunteer developers. Until then the network remains in active development and testing.',
    ...
    genesisReserve: 'Genesis Reserve',
    genesisReserveSub: '16.78B ZION · public summary',
    security: 'Security',
    securityTitle: 'Launch Readiness Security Checklist',
    securityCompleted: 'completed',
    ctaTitle: 'Public Launch Gate — postponed',
    ctaSubtitle: 'L1 is the heart we build bottom-up. Mainnet is live, L2 is live, but the public launch is postponed until we pass the Maturity Gate, secure liquidity, and find a team of volunteer developers.',
...
const constitution = [
  ...
  { param: 'Mining Supply', value: '127,220,000,000 ZION' },
  { param: 'Genesis Reserve', value: '16,780,000,000 ZION' },
  ...
  const phases: PhaseData[] = [
    {
      ...
      sprints: [
        ...
        { id: '0.1', title: cs ? 'Emise & Genesis — 5 400 ZION/blok, 16,78B reserve' : 'Emission & Genesis — 5,400 ZION/block, 16.78B reserve', done: true },
      ...
      exitCriteria: [
        ...
        { text: cs ? 'Genesis produkuje 16,78B reserve' : 'Genesis produces 16.78B reserve', done: true },
      ...
      <div className="zion-container max-w-7xl space-y-14">
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-3 text-xs">
                ...
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-gray-200">
                  <ShieldCheck className="h-3 w-3 text-zion-cyan" /> {copy.testsPassing}
                </span>
              </div>
            </div>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        <div className="h-2 w-24 rounded-full bg-white/10">
                          <div
                            className={`h-2 rounded-full ${comp.readiness >= 85 ? 'bg-zion-cyan' : comp.readiness >= 70 ? 'bg-zion-gold' : 'bg-zion-purple'}`}
        ...
        >
          ...
          <div className="relative">
            <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-zion-cyan via-zion-purple to-zion-gold hidden md:block" />
            <div className="space-y-6">
              {phases.map((phase, idx) => {
                ...
                const statusBadge =
                  phase.status === 'done'
                    ? { text: copy.done, cls: 'border-zion-cyan/30 bg-zion-cyan/10 text-emerald-200' }
                    : phase.status === 'active'
                      ...
                      <div className="mt-4 flex items-center gap-4">
                        <div className="h-2 flex-1 rounded-full bg-white/10">
                          <motion.div
                            ...
                            className={`h-2 rounded-full ${phase.status === 'done' ? 'bg-zion-cyan' : 'bg-gradient-to-r from-zion-purple via-zion-cyan to-zion-purple'}`}
                          />
                        </div>
                      ...
                      <div className="mt-5 grid gap-2 md:grid-cols-2">
                        {phase.sprints.map((sprint) => (
                          <div key={sprint.id} className="flex items-start gap-2 text-sm">
                            {sprint.done ? (
                              <CheckCircle2 className="h-4 w-4 text-zion-cyan mt-0.5 shrink-0" />
                            ) : (
                        ...
                        <div className="mt-3 grid gap-1.5 md:grid-cols-2">
                          {phase.exitCriteria.map((ec) => (
                            <div key={ec.text} className="flex items-start gap-2 text-sm">
                              {ec.done ? (
                                <CheckCircle2 className="h-3.5 w-3.5 text-zion-cyan mt-0.5 shrink-0" />
                              ) : (
        ...
        >
          ...
          <div className="mt-6 zion-rainbow-sub p-5" style={{ '--rc': '228, 30, 43' } as React.CSSProperties}>
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              {[
                { n: '1️⃣', label: 'Base / Arbitrum (Uni v3)', cls: 'text-zion-cyan' },
                { n: '2️⃣', label: 'BNB Chain (PancakeSwap)', cls: 'text-zion-gold' },
                { n: '3️⃣', label: 'CoinGecko + CMC', cls: 'text-zion-purple' },
                { n: '4️⃣', label: 'Tier-3 CEX (MEXC, XT)', cls: 'text-zion-purple' },
        ...
        >
          ...
          <div className="mt-4 flex items-center gap-3 text-sm text-gray-400">
            <span className="font-mono text-zion-cyan">{secDone}</span>
            <span>/</span>
            ...
            <div className="h-2 flex-1 max-w-xs rounded-full bg-white/10">
              <div className="h-2 rounded-full bg-zion-cyan" style={{ width: `${(secDone / secTotal) * 100}%` }} />
            </div>
          </div>
        </motion.section>
