...
const docsPageCopy = {
  ...
  overviewNotice: {
    cs: 'MainNet Genesis 11. 6. 2026 úspěšný — aktuální public line je 3.0.4 From Genesis to the Star nad kanonickou runtime v3.0.4 Deeksha/Ekam. Decimal fork 1e12→1e6 (6-decimal flowers) dokončen. Edge server topologie běží, pool aktivní, mining live. Veřejný launch zůstává na 31. prosince 2026 (Silvestr). Live Index na /docs#live-index je kanonický vstup pro síťový snapshot, baseline repa 2.9.6 a mapu release/runtime.',
    en: 'MainNet Genesis 11 Jun 2026 successful — the current public line is 3.0.4 From Genesis to the Star over the canonical v3.0.4 Deeksha/Ekam runtime. Decimal fork 1e12→1e6 (6-decimal flowers) complete. Edge server topology is running, the pool is active, and mining is live. Public launch remains 31 December 2026 (New Year\'s Eve). Live Index at /docs#live-index is the canonical entry for the network snapshot, 2.9.6 repo baseline, and release/runtime map.',
  },
    ...
    categories: [
      {
        ...
        icon: Rocket,
        docs: [
          { id: 'v301-readme', title: 'v3.0.4 Current Line Overview', file: 'v3.0.1/README.md' },
          { id: 'v301-launch-sequence', title: 'MainNet Launch Sequence', file: 'v3.0.1/MAINNET_LAUNCH_SEQUENCE.md' },
          { id: 'v301-status', title: 'v3.0.4 Status & Runtime Track', file: 'v3.0.1/StatusV3.md' },
          { id: 'v301-roadmap', title: 'v3.0.4 Roadmap Track', file: 'v3.0.1/ROADMAP.md' },
        ]
      },
    ]
  },
...
const sections: Section[] = [
  {
    ...
    docs: [
      ...
      { id: 'live-mainnet', title: 'Genesis 3.0.4 Status', file: 'mainnet/README.md' },
      { id: 'v301-launch-sequence', title: 'MainNet Launch Sequence', file: 'v3.0.1/MAINNET_LAUNCH_SEQUENCE.md' },
    ],
  },
  {
    ...
    docs: [
      { id: 'v301-genesis', title: 'v3.0.4 — Current Public Line', file: 'v3.0.1/README.md' },
...
export default function DocsPage() {
  const [activeVersion, setActiveVersion] = useState('v3.0.4');
  ...
  const [expandedVersions, setExpandedVersions] = useState<Record<string, boolean>>({ 'v3.0.4': true, 'v3.0.0': false, 'v2.9.9': false, 'v2.9.8': false, 'v2.9.7': false, 'v2.9.6': false, 'v2.9.5': false, 'v2.9': false, 'v2.8.x': false, 'live-ops': true, 'ai-native': true, 'whitepaper': false, 'architecture': false, 'mainnet': false, 'listing': false, 'legal': false, 'lumi-language': false });
  const [sidebarTab, setSidebarTab] = useState<'resources' | 'history'>('resources');
  const { lang } = useLang();
  const currentLang = lang === 'cs' ? 'cs' : 'en';
  const primaryVersions = versions.filter((version) => version.id === 'v3.0.4');
