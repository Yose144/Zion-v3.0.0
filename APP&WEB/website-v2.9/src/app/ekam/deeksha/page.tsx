...
export default function EkamDeekshaPage() {
  ...
  const chapters = [
    { id: 'predmluva',  number: '',   title: cs ? 'Předmluva' : 'Preface',               subtitle: cs ? 'Druhý dech Quantové Revoluce' : 'Second Breath of the Quantum Revolution' },
    { id: 'kapitel-1', number: '1',  title: cs ? 'Konec odděleného člověka' : 'The End of the Separate Human', subtitle: cs ? 'Krize odděleného self' : 'Crisis of the Separate Self' },
    { id: 'kapitel-2', number: '2',  title: cs ? 'Proč staré mapy nestačí' : 'Why Old Maps Are Not Enough',  subtitle: cs ? 'Čtyři limity moderního hledání' : 'Four Limits of Modern Seeking' },
    { id: 'kapitel-3', number: '3',  title: cs ? 'Historická linie' : 'Historical Lineage',         subtitle: 'Jeevashram · Satyaloka · Oneness' },
    { id: 'kapitel-4', number: '4',  title: 'Amma a Bhagavan',          subtitle: cs ? 'Dvojjediny princip lásky a vhledu' : 'Dual-Unity Principle of Love and Insight' },
    { id: 'kapitel-5', number: '5',  title: 'Deeksha',                  subtitle: cs ? 'Přenos jako technologie vědomí' : 'Transmission as Consciousness Technology' },
    { id: 'kapitel-6', number: '6',  title: 'Ekam a Golden Orb',        subtitle: cs ? 'Chrám a zlaté centrum' : 'Temple and Golden Center' },
    { id: 'kapitel-7', number: '7',  title: 'Hiranyagarbha',            subtitle: cs ? 'Zlatý zárodek — kosmologie stvoření' : 'Golden Seed — Cosmology of Creation' },
    { id: 'kapitel-8', number: '8',  title: 'Ekam Deeksha PoW',         subtitle: cs ? 'Algoritmus jako zlatý zárodek — Cosmic Harmony v3' : 'Algorithm as Golden Seed — Cosmic Harmony v3' },
    { id: 'kapitel-9', number: '9',  title: cs ? 'Algoritmus Zlatého věku' : 'Algorithm of the Golden Age',  subtitle: cs ? 'Deset kroků od separace k jednotě' : 'Ten Steps from Separation to Unity' },
    { id: 'zaver',     number: '',   title: cs ? 'Závěr' : 'Conclusion',                    subtitle: cs ? 'Kdy začíná Zlatý věk' : 'When the Golden Age Begins' },
  ];

  const algoSteps = [
    { id: 0, name: 'Hiranyagarbha', color: 'text-amber-300',   border: 'border-amber-300/40',   bg: 'bg-amber-300/10',   desc: cs ? 'Inicializace zlatého zárodku — seed z hlavičky bloku se rozvíjí v primordial state.' : 'Initialization of the golden seed — block header seed expands into primordial state.' },
    { id: 1, name: 'Brahma',        color: 'text-violet-300',  border: 'border-violet-300/40',  bg: 'bg-violet-300/10',  desc: cs ? 'Expanze stvoření — čtení z 256 KiB scratchpadu rozviřuje prostor stavů.' : 'Expansion of creation — reading from 256 KiB scratchpad swirls the state space.' },
    { id: 2, name: 'Yantra',        color: 'text-sky-300',     border: 'border-sky-300/40',     bg: 'bg-sky-300/10',     desc: cs ? '4 průchody × 256 náhodných čtení — geometrie posvátného vzorce, doslova nelze parallelizovat.' : '4 passes × 256 random reads — geometry of sacred pattern, literally cannot be parallelized.' },
    { id: 3, name: 'Karma',         color: 'text-emerald-300', border: 'border-emerald-300/40', bg: 'bg-emerald-300/10', desc: cs ? 'Zákon příčiny — epoch NPU váhy (rotace každých 2016 / 100 bloků) mění výsledek každou epochou.' : 'Law of cause — epoch NPU weights (rotation every 2016 / 100 blocks) change the result each epoch.' },
    { id: 4, name: 'Chit',          color: 'text-rose-300',    border: 'border-rose-300/40',    bg: 'bg-rose-300/10',    desc: cs ? 'Čisté vědomí — finální mix vah s hash state.' : 'Pure consciousness — final mix of weights with hash state.' },
    { id: 5, name: 'Samadhi',       color: 'text-amber-200',   border: 'border-amber-200/40',   bg: 'bg-amber-200/10',   desc: cs ? 'Sjednocení — 32bajtový výsledek srovnaný s target difficulty. PoW je dokončen.' : 'Unification — 32-byte result compared with target difficulty. PoW is complete.' },
  ];

  const algoTiers = [
    { tier: 'Tier 1', title: '256 KiB Scratchpad', desc: cs ? 'Paměťově vázaná práce — každý průchod skenerem musí procházet celou 256 KiB oblastí. ASIC nemůže vynechat paměťový latency.' : 'Memory-bound work — every scanner pass must traverse the entire 256 KiB region. ASIC cannot skip memory latency.' },
    { tier: 'Tier 2', title: cs ? 'Epoch NPU váhy' : 'Epoch NPU Weights',     desc: cs ? 'Každých 2016 bloků (nebo 100 bloků v testnet) se rotují NPU vahové matice. Speciální hardware zastarává každou epochou.' : 'Every 2016 blocks (or 100 blocks in testnet) the NPU weight matrices rotate. Specialized hardware becomes obsolete every epoch.' },
  ];
        ...
        <div className="zion-container py-20 relative">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-amber-300/30 bg-amber-300/10 mb-6">
              ...
              <span className="text-sm text-amber-200 font-semibold">{cs ? 'Navazuje na Quantovou Revoluci' : 'Continues the Quantum Revolution'}</span>
            </div>
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold mb-6 text-gradient">
              Ekam Deeksha
            </h1>
            <p className="text-xl text-gray-400 mb-4">
              {cs ? 'Algoritmus jednoty — filozofie, kosmologie a praxe Zlatého věku' : 'Algorithm of unity — philosophy, cosmology and practice of the Golden Age'}
            </p>
          ...
          {mobileMenuOpen && (
            <div className="mt-4 border border-white/10 rounded-2xl p-4 bg-black/80 backdrop-blur-xl space-y-1">
              {chapters.map((ch) => (
                ...
                >
                  {ch.number && <span className="text-xs opacity-60">{cs ? 'Kap. ' : 'Ch. '}{ch.number}: </span>}
                  {ch.title}
                </button>
              ))}
            </div>
            ...
            <div className="sticky top-24 border border-white/10 rounded-2xl p-5 bg-black/60 backdrop-blur-xl">
              ...
              <nav className="space-y-1">
                {chapters.map((ch) => (
                  ...
                  >
                    {ch.number && <span className="text-xs opacity-50 block">{cs ? 'Kap. ' : 'Ch. '}{ch.number}</span>}
                    {ch.title}
                  </button>
                ))}
              </nav>
          ...
          <div className="flex-1 max-w-3xl">
            ...
            <section id="predmluva" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <h2 className="text-4xl md:text-5xl font-bold text-amber-300 mb-3">{cs ? 'Předmluva' : 'Preface'}</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Druhý dech Quantové Revoluce' : 'Second Breath of the Quantum Revolution'}</p>
              </div>
              <div className="space-y-6 text-center">
                <p className="text-xl leading-relaxed text-gray-300">
                  {cs ? 'Quantová Revoluce pojmenovala civilizační práh.' : 'The Quantum Revolution named the civilizational threshold.'}<br />
                  {cs ? 'Ukázala, že starý svět se nevyčerpává náhodou —' : 'It showed that the old world does not exhaust itself by accident —'}<br />
                  {cs ? 'ale proto, že dosáhl mezí vlastního odděleného vědomí.' : 'but because it has reached the limits of its own separated consciousness.'}
                </p>
                <p className="text-base leading-8 text-gray-400">
                  {cs ? 'Tato kniha na tu mapu navazuje. Ale nerozšiřuje ji přidáním nové vrstvy teorie.' : 'This book follows that map. But it does not expand it by adding a new layer of theory.'}<br />
                  {cs ? 'Obrací pohled ' : 'It turns the gaze ' }<strong className="text-amber-300">{cs ? 'dovnitř' : 'inward'}</strong>.
                </p>
                <p className="text-base leading-8 text-gray-400">
                  {cs ? 'Ptá se přesněji: co se musí stát v člověku, aby oddělený svět opravdu skončil?' : 'It asks more precisely: what must happen in a human being for the separated world to truly end?'}<br />
                  {cs ? 'Nestačí lepší systémy. Nestačí kvantový slovník. Nestačí motivační spiritualita.' : 'Better systems are not enough. A quantum vocabulary is not enough. Motivational spirituality is not enough.'}<br />
                  {cs ? 'Je třeba konec jedné struktury vnímání — a zrození jiné.' : 'What is needed is the end of one structure of perception — and the birth of another.'}
                </p>
                <div className="my-10 p-8 rounded-2xl border border-amber-300/20 bg-amber-300/5 backdrop-blur-xl">
                  <p className="text-lg italic text-gray-300 leading-relaxed">
                    {cs ? 'Ekam znamená „jedno".' : 'Ekam means "one".'}<br />
                    {cs ? 'Deeksha znamená „přenos, zasvěcení, milost".' : 'Deeksha means "transmission, initiation, grace".'}<br />
                    {cs ? 'Tato kniha je o tom, jak se toto jedno přenáší v čase,' : 'This book is about how this oneness is transmitted in time,'}<br />
                    {cs ? 'prostoru, filozofii — a v algoritmech blockchainu.' : 'space, philosophy — and in blockchain algorithms.'}
                  </p>
                </div>
                <p className="text-gray-400 text-sm leading-7">
                  {cs ? 'Za touto knihou stojí linie Jeevashram → Satyaloka → Oneness → Ekam.' : 'Behind this book stands the lineage Jeevashram → Satyaloka → Oneness → Ekam.'}<br />
                  {cs ? 'Za touto linií stojí Amma a Bhagavan Sri Kalki.' : 'Behind this lineage stand Amma and Bhagavan Sri Kalki.'}<br />
                  {cs ? 'A za tím vším stojí nejstarší kosmologický obraz Véd:' : 'And behind all of this stands the oldest cosmological image of the Vedas:'}<br />
                  <strong className="text-amber-300">Hiranyagarbha</strong> — {cs ? 'zlatý zárodek, z něhož se rodí vesmír.' : 'the golden seed from which the universe is born.'}
                </p>
              </div>
            </section>
            ...
            <section id="kapitel-1" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 1' : 'Chapter 1'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">{cs ? 'Konec odděleného člověka' : 'The End of the Separate Human'}</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Krize odděleného self' : 'Crisis of the Separate Self'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                ...
                <div className="rounded-2xl border border-sky-300/20 bg-sky-300/5 p-6 my-6">
                  <p className="text-sky-200 font-semibold mb-2 flex items-center gap-2"><Layers className="w-4 h-4" /> {cs ? 'Základní teze' : 'Core Thesis'}</p>
            ...
            <section id="kapitel-2" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 2' : 'Chapter 2'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">{cs ? 'Proč staré mapy nestačí' : 'Why Old Maps Are Not Enough'}</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Čtyři limity moderního hledání' : 'Four Limits of Modern Seeking'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
            ...
            <section id="kapitel-3" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 3' : 'Chapter 3'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">{cs ? 'Historická linie' : 'Historical Lineage'}</h2>
            ...
            <section id="kapitel-4" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 4' : 'Chapter 4'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">Amma a Bhagavan</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Dvojjediny princip lásky a vhledu' : 'Dual-Unity Principle of Love and Insight'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="grid sm:grid-cols-2 gap-4 my-6">
                  <div className="rounded-2xl border border-rose-300/20 bg-rose-300/5 p-6">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">{cs ? 'Amma — princip lásky' : 'Amma — Principle of Love'}</h3>
                    <p className="text-sm leading-7 text-gray-300">
                      {cs ? 'Amma ztělesňuje bezpodmínečné přijetí. Láskyplnou přítomnost, která nehodnotí a neodmítá. V linii Ekam je tato energie popsána jako sídlo soucitu — schopnost přijmout člověka přesně tam, kde je, bez požadavku, že nejprve uspěje.' : 'Amma embodies unconditional acceptance. A loving presence that does not judge and does not reject. In the Ekam lineage, this energy is described as the abode of compassion — the ability to accept a person exactly where they are, without the demand that they first succeed.'}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-violet-300/20 bg-violet-300/5 p-6">
                    <Sun className="w-6 h-6 text-violet-300 mb-3" />
                    <h3 className="text-xl font-bold text-white mb-2">{cs ? 'Bhagavan — princip vhledu' : 'Bhagavan — Principle of Insight'}</h3>
                    <p className="text-sm leading-7 text-gray-300">
                      {cs ? 'Bhagavan Sri Kalki reprezentuje pronikající jasnost, která vidí skrz iluze. Kde Amma zahřeje, Bhagavan osvítí. Dohromady tvoří pole, v němž může proměna vejít do obou svých pohybů: přijetí a osvobození.' : 'Bhagavan Sri Kalki represents penetrating clarity that sees through illusions. Where Amma warms, Bhagavan illuminates. Together they create a field in which transformation can enter both of its movements: acceptance and liberation.'}
                    </p>
                  </div>
                </div>
                <p>
            ...
            <section id="kapitel-5" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 5' : 'Chapter 5'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">Deeksha</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Přenos jako technologie vědomí' : 'Transmission as Consciousness Technology'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="rounded-2xl border border-amber-300/20 bg-amber-300/5 p-6 my-6">
                  <p className="text-amber-200 font-semibold mb-3">{cs ? 'Co Deeksha tvrdí, že dělá' : 'What Deeksha Claims to Do'}</p>
                  <ul className="space-y-2 text-sm text-gray-300">
                    {[
                      cs ? 'Ztišuje neustálý mentální hluk odděleného já' : 'Quiets the constant mental noise of the separate self',
                      cs ? 'Otevírá prostor pro přímou zkušenost jednoty' : 'Opens space for direct experience of unity',
                      cs ? 'Uvolňuje emocionální uzly vázané na oddělené vnímání' : 'Releases emotional knots bound to separated perception',
                      cs ? 'Usnadňuje přechod od identifikace s myslí k prožívání přítomnosti' : 'Facilitates the transition from identification with mind to experiencing presence',
                      cs ? 'Nevytváří nový duchovní výkon — rozpouští potřebu ho vytvářet' : 'Does not create a new spiritual performance — dissolves the need to create one',
                    ].map((item) => (
            ...
            <section id="kapitel-6" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 6' : 'Chapter 6'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">Ekam a Golden Orb</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Chrám a zlaté centrum' : 'Temple and Golden Center'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="text-center py-6">
                  <p className="text-2xl text-amber-300 font-bold mb-3">{cs ? 'Zlatý zárodek vědomí je v centru každého člověka.' : 'The golden seed of consciousness is in the center of every human being.'}</p>
                  <p className="text-gray-400">{cs ? 'Zlatý věk není budoucnost, která přijde sama od sebe.' : 'The Golden Age is not a future that will come by itself.'}<br />{cs ? 'Je to stav vědomí, který musí být probuzen.' : 'It is a state of consciousness that must be awakened.'}</p>
                </div>
                <p>
                  {cs
            ...
            <section id="kapitel-7" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 7' : 'Chapter 7'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">Hiranyagarbha</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Zlatý zárodek — kosmologie stvoření' : 'Golden Seed — Cosmology of Creation'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="rounded-2xl border border-amber-300/20 bg-amber-300/5 p-6 my-6 text-center">
                  ...
                  <p className="text-amber-200 italic text-lg">
                    {cs ? '„Na počátku bylo zlaté vejce, zárodek vesmíru.' : '"In the beginning was the golden egg, the seed of the universe.'}<br />
                    {cs ? 'Z něho se zrodil veškerý svět — nebe, země, světlo, tma."' : 'From it was born the whole world — heaven, earth, light, darkness."'}
                  </p>
                  <p className="text-gray-500 text-sm mt-2">{cs ? '— Rigvéda, hymnus na Hiranyagarbhu' : '— Rigveda, Hymn to Hiranyagarbha'}</p>
                </div>
                <p>
                  {cs
                ...
                <div className="grid sm:grid-cols-3 gap-4 my-4">
                  {([
                    { dim: cs ? 'Védská kosmologie' : 'Vedic Cosmology', t: cs ? 'Zlaté vejce pluje v primordálních vodách. Nese zárodek celého stvořeného světa.' : 'The golden egg floats in primordial waters. It carries the seed of the entire created world.' },
                    { dim: cs ? 'Linie Ekam' : 'Ekam Lineage',        t: cs ? 'Golden Orb jako centrum chrámu — kondenzovaný symbol zárodku nového vědomí.' : 'Golden Orb as the center of the temple — condensed symbol of the seed of new consciousness.' },
                    { dim: cs ? 'ZION PoW' : 'ZION PoW',          t: cs ? 'Ekam Deeksha hash pipeline začíná fází Hiranyagarbha — inicializací zlatého zárodku z hlavičky bloku.' : 'Ekam Deeksha hash pipeline begins with the Hiranyagarbha phase — initialization of the golden seed from the block header.' },
                  ] as const).map(({ dim, t }) => (
            ...
            <section id="kapitel-8" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 8' : 'Chapter 8'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">Ekam Deeksha PoW</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Algoritmus jako zlatý zárodek — Cosmic Harmony v3' : 'Algorithm as Golden Seed — Cosmic Harmony v3'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="my-6 space-y-2">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{cs ? '6-stupňový pipeline Ekam Deeksha' : '6-Stage Ekam Deeksha Pipeline'}</p>
                ...
                <p className="text-base font-semibold text-white mt-6 mb-3">{cs ? 'ASIC rezistence — dvouvrstvá ochrana' : 'ASIC Resistance — Two-Layer Protection'}</p>
            ...
            <section id="kapitel-9" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <div className="text-sm text-amber-300 uppercase tracking-wider mb-2">{cs ? 'Kapitola 9' : 'Chapter 9'}</div>
                <h2 className="text-4xl md:text-5xl font-bold text-white mb-3">{cs ? 'Algoritmus Zlatého věku' : 'Algorithm of the Golden Age'}</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Deset kroků od separace k jednotě' : 'Ten Steps from Separation to Unity'}</p>
              </div>
              <div className="space-y-5 text-base leading-8 text-gray-300">
                <p>
                  {cs
                ...
                <div className="space-y-3 my-6">
                  {([
                    { n: 1,  step: cs ? 'Zastavení hluku' : 'Stopping the Noise',          desc: cs ? 'Vědomá pauza — přerušení automatického proudu myšlenek, notifikací a reakcí. Bez ticha nelze nic rozpoznat.' : 'Conscious pause — interruption of the automatic stream of thoughts, notifications and reactions. Without silence nothing can be recognized.' },
                    { n: 2,  step: cs ? 'Rozpoznání separace' : 'Recognizing Separation',       desc: cs ? 'Vidět, kdy jednáme z oddělenosti — ze strachu, obrany, potřeby potvrzení. Samotné rozpoznání začíná měnit vzorec.' : 'To see when we act from separation — from fear, defense, need for confirmation. Recognition itself begins to change the pattern.' },
                    { n: 3,  step: cs ? 'Otevření srdce' : 'Opening the Heart',            desc: cs ? 'Pohyb od hlavy jako jediného centra k celému tělu jako poli vnímání. Srdce jako druhý mozek.' : 'Movement from the head as the only center to the whole body as a field of perception. The heart as a second brain.' },
                    { n: 4,  step: cs ? 'Aktivace citlivosti' : 'Activating Sensitivity',       desc: cs ? 'Schopnost být skutečně přítomen — s druhým člověkem, s přírodou, s vlastním tělem. Bez hodnocení.' : 'The ability to be truly present — with another person, with nature, with one\'s own body. Without judgment.' },
                    { n: 5,  step: cs ? 'Přenos a Deeksha' : 'Transmission and Deeksha',          desc: cs ? 'Prolomení izolace sebe-interpretace přijetím impulzu z pole vědomí, které je hlubší než individuální mysl.' : 'Breaking the isolation of self-interpretation by receiving an impulse from a field of consciousness that is deeper than the individual mind.' },
                    { n: 6,  step: cs ? 'Stabilizace' : 'Stabilization',               desc: cs ? 'Integrace nového stavu do každodenního života. Bez stabilizace je každé probuzení jen dočasná výjimka.' : 'Integration of the new state into everyday life. Without stabilization, every awakening is only a temporary exception.' },
                    { n: 7,  step: cs ? 'Služba' : 'Service',                    desc: cs ? 'Obrat od sebestřednosti k péči. Probuzení, které neslouží druhým, se uzavírá zpět do rafinovanějšího ega.' : 'Turn from self-centeredness to care. Awakening that does not serve others closes back into a more refined ego.' },
                    { n: 8,  step: cs ? 'Vytváření lokálních polí' : 'Creating Local Fields',  desc: cs ? 'Tvoření prostorů a komunit, kde jsou podmínky pro proměnu vědomí přítomny. Každý takový uzel je buňkou nové civilizace.' : 'Creating spaces and communities where conditions for transformation of consciousness are present. Every such node is a cell of a new civilization.' },
                    { n: 9,  step: cs ? 'Síťování' : 'Networking',                  desc: cs ? 'Propojení lokálních polí do vzájemně podporující se sítě. Od izolovaných ostrovů k archipelagu vědomí.' : 'Connecting local fields into a mutually supporting network. From isolated islands to an archipelago of consciousness.' },
                    { n: 10, step: cs ? 'Civilizační manifestace' : 'Civilizational Manifestation',   desc: cs ? 'Nové vzdělávání, nová ekonomika, nová politika — ne jako ideologický program, ale jako přirozený výsledek dostatečně mnoha proměněných lidí.' : 'New education, new economy, new politics — not as an ideological program, but as a natural result of sufficiently many transformed people.' },
            ...
            <section id="zaver" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                <h2 className="text-4xl md:text-5xl font-bold text-amber-300 mb-3">{cs ? 'Závěr' : 'Conclusion'}</h2>
                <p className="text-gray-400 text-lg italic">{cs ? 'Kdy začíná Zlatý věk' : 'When the Golden Age Begins'}</p>
              </div>
              <div className="space-y-6 text-center">
                <p className="text-xl leading-relaxed text-gray-300">
                  {cs ? 'Zlatý věk nezačíná, když se změní vláda.' : 'The Golden Age does not begin when the government changes.'}<br />
                  {cs ? 'Nezačíná, když se vymyslí dostatečně dobrý systém.' : 'It does not begin when a sufficiently good system is invented.'}<br />
                  {cs ? 'Nezačíná ani tehdy, když technologie dosáhne dostatečné složitosti.' : 'It does not even begin when technology reaches sufficient complexity.'}
                </p>
                <p className="text-base leading-8 text-gray-400">
                  {cs ? 'Zlatý věk začíná vždy, když se jeden konkrétní člověk' : 'The Golden Age always begins when one particular human being'}<br />
                  {cs ? 'přestane prožívat jako uzavřený ostrov.' : 'ceases to experience themselves as a closed island.'}<br />
                  {cs ? 'A toto je přesně to, co linie Ekam Deeksha hledá' : 'And this is precisely what the Ekam Deeksha lineage seeks'}<br />
                  {cs ? 'od prvních dnů Jeevashram až po dnešní pole chrámu Ekam.' : 'from the first days of Jeevashram to today\'s field of the Ekam temple.'}
                </p>
                <div className="my-10 p-8 rounded-2xl border border-amber-300/30 bg-amber-300/5 backdrop-blur-xl">
                  ...
                  <p className="text-lg italic text-gray-300 leading-relaxed">
                    {cs ? 'Quantová Revoluce ukázala, že civilizace z vnějšku nemůže být zachráněna bez vnitřní proměny člověka.' : 'The Quantum Revolution showed that civilization cannot be saved from the outside without the inner transformation of the human being.'}<br /><br />
                    {cs ? 'Ekam Deeksha ukazuje, jak tato vnitřní proměna vypadá —' : 'Ekam Deeksha shows what this inner transformation looks like —'}<br />
                    {cs ? 'v historii, v filozofii, v prostoru chrámu, v Deeksha přenosu,' : 'in history, in philosophy, in the space of the temple, in Deeksha transmission,'}<br />
                    {cs ? 'v zlatém zárodku Hiranyagarbhy' : 'in the golden seed of Hiranyagarbha'}<br />
                    {cs ? 'a v každém bloku ZION blockchainu pojmenovaném po tomto přechodu.' : 'and in every block of the ZION blockchain named after this transition.'}
                  </p>
                </div>
                <p className="text-gray-400 text-sm">
                  {cs ? 'Obě knihy jsou jednou knihou.' : 'Both books are one book.'}<br />
                  {cs ? 'Jedna mluví o světě. Druhá mluví o člověku, který ho tvoří.' : 'One speaks about the world. The other speaks about the human who creates it.'}<br />
                  {cs ? 'Bez obou polí žádná mapa nestačí.' : 'Without both fields, no map is sufficient.'}
                </p>
                <p className="text-center text-amber-300 font-semibold text-2xl mt-10">
                  {cs ? 'Ekam. Jedno. Tady. Teď. Ty.' : 'Ekam. One. Here. Now. You.'}
                </p>
              </div>
            </section>
            ...
            <div className="mb-20 border-t border-white/10 pt-28 text-center space-y-4">
              <p className="text-gray-400 text-sm">{cs ? 'Prozkoumej dál' : 'Explore further'}</p>
              <div className="flex flex-wrap justify-center gap-3">
                <Link href="/ekam" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/15 bg-white/8 text-sm font-semibold text-white hover:border-amber-300/30 transition-colors">
                  {cs ? 'Ekam — virtuální prohlídka' : 'Ekam — Virtual Tour'}
                </Link>
