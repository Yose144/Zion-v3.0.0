    ...
    def start_mining(self) -> bool:
        ...
        if self.afterburner_enabled and self.afterburner is None:
            try:
                ...
                logger.info("[OK] AI Afterburner online (HUD every 10s)")
            except Exception as ab_err:
                logger.warning(f"Afterburner init failed: {ab_err}")
        return True
    
    def _mining_loop(self):
        """Main mining loop - optimized with TURBO batch sizes"""
        try:
            nonce = 0
            # TURBO: Use much larger batch for 86x speedup (8+ GH/s)
            batch_size = 500_000  # 500K minimum for GPU efficiency
            self.stats['batch_size'] = batch_size
            
            # Try to use TURBO miner if available
            turbo_miner = None
            try:
                from mining.cosmic_harmony_v1_turbo import CosmicHarmonyV1Turbo, GPU_AVAILABLE
                if GPU_AVAILABLE:
                    turbo_miner = CosmicHarmonyV1Turbo(
                        batch_size=50_000_000,  # 50M batch for max throughput
                        num_buffers=2,
                        use_pinned_memory=True,
                        verbose=False
                    )
                    batch_size = turbo_miner.batch_size
                    self.stats['batch_size'] = batch_size
                    logger.info(f"[TURBO] Using optimized GPU (8+ GH/s) | Batch: {batch_size:,}")
            except ImportError:
                logger.info("[INFO] TURBO miner not available, using standard GPU")
