/**
 * ZION Presale Dashboard - Frontend Logic v2.9.5 (EN Version)
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 ZION Presale Dashboard v2.9.5 initialized (EN)');
...
function updatePhaseIndicators(stats) {
  ...
  const phases = [
    {id: 'phase-1-status', phase: 1, activeText: 'Active', completedText: 'Completed'},
    {id: 'phase-2-status', phase: 2, activeText: 'Active', completedText: 'Completed', waitingText: 'Waiting'},
    {id: 'phase-3-status', phase: 3, activeText: 'Active', waitingText: 'Waiting'}
  ];
...
function updateRecentOrdersTable(orders) {
  ...
  const ordersHtml = orders.slice(0, 10).map(order => {
    const statusColor = order.status === 'completed' ? 'var(--rasta-green)' : 
                        ...
                        'var(--rasta-red)';
    const statusText = order.status === 'completed' ? 'Paid' :
    ...
    tableBody.innerHTML = `
      <tr>
        <td colspan="5" style="text-align: center; padding: 40px;">
          ...
          <div style="color: #999;">Backend is unavailable</div>
...
async function lookupWallet() {
  ...
  if (!query) {
    alert('Please enter Order ID or Wallet ID');
    return;
  }
  ...
  try {
    ...
    if (!isOrderId && !isWalletId) {
      throw new Error('Invalid format. Use PRESALE-XXX or ZION_XXX');
    }
    ...
    errorDiv.innerHTML = `
      ...
      <div style="font-size: 1.1em; margin-bottom: 5px;">Not found</div>
...
function displayLookupResult(data, resultDiv) {
  ...
  let html = `
    <h4 style="color: var(--rasta-gold); margin-bottom: 20px;">
      <i class="fa-solid fa-check-circle"></i> Order found
    </h4>
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
      ...
      <div>
        <div style="color: #999; font-size: 0.9em; margin-bottom: 5px;">Tokens</div>
        <div style="color: white; font-weight: bold;">${formatNumber(order.tokens_total || 0)} ZION</div>
      </div>
      <div>
        <div style="color: #999; font-size: 0.9em; margin-bottom: 5px;">Price</div>
