/**
 * ZION Token Presale - JavaScript (English/EUR version)
...
const PRESALE_CONFIG = {
    ...
    currency: 'EUR', // EUR for English version
    tokenPrice: 0.004, // EUR per ZION token (same as 0.1 CZK at 25 rate)
    tokenPriceEur: 0.004,
    eurToCzk: 25,
    minAmount: 20, // Min 20 EUR
    maxAmount: 1200, // Max 1200 EUR
    bonusTiers: [
        { min: 0, max: 98, bonus: 0 },
        { min: 99, max: 498, bonus: 0.10 }, // +10% above 99 EUR
        { min: 499, max: 998, bonus: 0.20 }, // +20% above 499 EUR
        { min: 999, max: Infinity, bonus: 0.30 } // +30% above 999 EUR
    ],
    packages: {
        starter: {
            name: 'PIZZA Pack',
        ...
        standard: {
            ...
            priceCzk: 12490,
            priceEur: 499,
            baseTokens: 124750, // 499 EUR / 0.004
            bonus: 0.20
        },
        premium: {
            name: 'Guardian Pack',
            priceCzk: 24990,
            priceEur: 999,
            baseTokens: 249750, // 999 EUR / 0.004
            bonus: 0.30
        }
    }
};
...
function addBookToCart() {
    const book = {
        id: 'book-001',
        name: 'Quantum Revolution & MAINNET ALPHA',
        price: 5.80,
        priceEur: 5.80,
        tokens: 144,
        bonus: 144,
...
function updateCalculator() {
    ...
    // Update slider display value (amount in EUR)
...
function selectPackage(packageId) {
    ...
    currentOrder = {
        ...
        price: PRESALE_CONFIG.currency === 'CZK' ? pkg.priceCzk : pkg.priceEur,
        currency: PRESALE_CONFIG.currency,
...
function buyCustomAmount() {
    ...
    if (amount > PRESALE_CONFIG.maxAmount) {
        ...
        alert(`For amounts over ${maxDisplay} please contact us directly.`);
        return;
    }
    
    // Calculate EUR equivalent for backend
    let eurAmount = amount;
    if (PRESALE_CONFIG.currency === 'CZK') {
        eurAmount = Math.round(amount / PRESALE_CONFIG.eurToCzk * 100) / 100;
    }
    
    const bonusRate = getBonusRate(eurAmount);
    // Token price is based on EUR value for calculation consistency
    const tokenPriceEur = PRESALE_CONFIG.tokenPriceEur || 0.004;
    const baseTokens = Math.floor(eurAmount / tokenPriceEur);
    const totalTokens = Math.floor(baseTokens * (1 + bonusRate));
    
    currentOrder = {
        packageId: 'custom',
        packageName: 'Custom Amount',
        priceEur: eurAmount,
        priceCzk: PRESALE_CONFIG.currency === 'CZK' ? amount : (amount * PRESALE_CONFIG.eurToCzk),
        price: amount,
        currency: PRESALE_CONFIG.currency,
        baseTokens,
        bonusRate,
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 30px;">
                ...
                <div style="background: rgba(0,0,0,0.3); padding: 25px; border-radius: 16px; border: 2px solid var(--rasta-gold); text-align: left;">
                    ...
                    <div style="color: #ddd; line-height: 2;">
                        <p><strong>Recipient:</strong><br/>ZION TerraNova</p>
                ...
                <div style="background: rgba(0,0,0,0.3); padding: 25px; border-radius: 16px; border: 2px solid var(--rasta-green);">
                    ...
                    ${qrUrl ? `
                        ...
                        <p style="color: #888; font-size: 0.85rem; margin-top: 15px;">Scan to connect to the ZION network</p>
