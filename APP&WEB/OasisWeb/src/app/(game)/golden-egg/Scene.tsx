  ...
  return (
    <group>
      ...
      <mesh scale={[1.15, 1.15, 1.15]}>
        <sphereGeometry args={[1.1, 64, 64]} />
        <meshStandardMaterial color="#fcd116" emissive="#fcd116" emissiveIntensity={0.2} transparent opacity={0.35} />
      </mesh>
      <pointLight position={[0, 0, 0]} intensity={2} color="#fcd116" distance={18} />
    </group>
  );
}
      ...
      <group position={zonePos} ref={groupRef}>
        ...
        <Html transform center distanceFactor={10} position={[-3.5, 2.5, 0]} className="pointer-events-none hidden sm:block">
          <GlassPanel className="px-3 py-2 text-xs">
            ...
            <p className="mt-2 text-[10px] text-white/70">Discovered: {discovered}/108</p>
          </GlassPanel>
        </Html>
      </group>
  );
...
function ClueDetail({ clue, onClose }: { clue: Clue; onClose: () => void }) {
  ...
  return (
    <div className="space-y-2 text-sm">
      ...
      <p className="text-white/80">{clue.hint}</p>
