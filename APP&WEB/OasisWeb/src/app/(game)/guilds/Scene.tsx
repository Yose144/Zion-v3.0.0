...
const GUILD_COLORS = ['#e41e2b', '#e41e2b', '#078930', '#fcd116', '#078930', '#e41e2b'];
      ...
      <group position={zonePos}>
        <group ref={platformRef}>
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.1, 0]}>
            ...
            <meshStandardMaterial color="#1a1a1a" emissive="#e41e2b" emissiveIntensity={0.15} roughness={0.4} metalness={0.6} />
          </mesh>
          <mesh position={[0, -0.15, 0]} rotation={[Math.PI / 2, 0, 0]}>
            <torusGeometry args={[2.2, 0.04, 16, 64]} />
            <meshStandardMaterial color="#e41e2b" emissive="#e41e2b" emissiveIntensity={0.3} transparent opacity={0.8} />
          </mesh>
        </group>
...
function GuildDetail({
  ...
  return (
    <div className="space-y-2 text-sm">
      ...
      <p className="text-white/80">{guild.description || 'No description.'}</p>
      ...
      <div className="mt-3 space-y-2 border-t border-white/10 pt-3">
        <input
          ...
          className="w-full rounded-xl bg-black/30 px-3 py-2 text-xs text-white outline-none placeholder:text-white/60"
        />
        <button
          onClick={join}
      ...
      <button onClick={onClose} className="mt-1 text-xs text-white/70 hover:text-white">Close</button>
    </div>
  );
}
