...
| Růže v Zahradě Genesis: narativní epilog + OASIS Rose lore / fyzické zasazení / pilot `GENESIS-ROSE-001`. | **MÝTUS** (příběh a lore) · **ŽIVÉ** (existence OASIS metadat) · **NEDOLOŽENO** (fyzická růže) · **HORIZONT / NEZAHÁJENO** (pilot) | [`Ruze/Epilog-Zahrada-Genesis.md`](../Ruze/Epilog-Zahrada-Genesis.md); [`BodhiGaia/12`](../BodhiGaia/12-Ruze-v-Zahrade-Genesis.md); `APP&WEB/OasisWeb/src/domain/config/worlds.ts` (`GENESIS_GARDEN`, lore označena MYTH); [`BodhiGaia/pilots/GENESIS-ROSE-001.md`](../BodhiGaia/pilots/GENESIS-ROSE-001.md). | Splnění Gate A–C + Day 0 packet, pak kontroly D30/D90/D365; nenahrazuje M5 exit gate. |
...
| OASIS zrcadlí tři L5 projekty a svět Dharma Temple. | **ŽIVÉ** (klient) | `APP&WEB/OasisWeb/src/components/WorldPanel.tsx` (`NOVA_ZEME_PROJECTS`); `src/domain/config/worlds.ts` (`DHARMA_TEMPLE_LA_PALMA`, `layer: 3` — má být L5). | Statusy napojené na L5 API; oprava `layer`. Detail: [`BodhiGaia/07`](../BodhiGaia/07-Zahrada-v-Oasis-a-na-Webu.md). |
...
7. Příběh [`Ruze/`](../Ruze/00-README.md) (*Příběh Růže — Co nelze vlastnit*, DRAFT) je celý literární **MÝTUS** — bajka o Lumi a růži u řeky za otevřenou bránou, bez technického protějšku. Její existence **nepovyšuje** starší materiál kolem avatara „Meriam Rose“ (`V31/L4/oasis/data/avatars.json`, id 18) ani esoterické nároky dokumentu `docs/docs2.9/ZION_OASIS/SACRED_TRINITY/18_MERIAM_ROSE.md` na historickou, duchovní, technickou či provozní pravdu; jejich hranice drží [`Ruze/10-Kotva-Pribehu.md`](../Ruze/10-Kotva-Pribehu.md) a `Miriam/13`. Totéž platí pro její epilog v Zahradě Genesis: scéna s Lumi a Anou je **MÝTUS**, existence metadat světa `GENESIS_GARDEN` v `APP&WEB/OasisWeb/src/domain/config/worlds.ts` je **ŽIVÝ** klientský údaj s obsahem výslovně označeným MÝTUS, fyzické zasazení růže v Algarve je **NEDOLOŽENO** a pilot [`GENESIS-ROSE-001`](../BodhiGaia/pilots/GENESIS-ROSE-001.md) je **HORIZONT / NEZAHÁJENO** — nesmí začít bez Gate A–C a nenahrazuje M5 exit gate.
