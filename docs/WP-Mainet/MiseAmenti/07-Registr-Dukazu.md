...
| L5 fond dnes vyplácí granty. | **NEPLATNÝ NÁROK** (dnes) | Gate G10 (`docs/3.2/L5_L6_ACTIVATION_PLAN.md`): žádná automatická výplata; `V31/L2/dao/src/runtime.rs` exekuce je summary-only. | DAO UI, výplatní tx flow, guardian multisig, první impact packet. |
| Zahrada Genesis (Algarve, Portugalsko) je fyzické místo v aktivním rozvoji. | **STAVBA** | `public/V3/L5/docs/COMMUNITIES/genesis-garden.md` (Fáze 0 ✅, Fáze 1 🟡); web `/terranova/genesis`; OASIS panel *Active*. | Pozemek, registrovaná entita, GPS, nezávislé ověření. Detail: [`BodhiGaia/03`](../BodhiGaia/03-Zahrada-Genesis.md). |
| Růže v Zahradě Genesis: narativní epilog + OASIS Rose lore / fyzické zasazení / pilot `GENESIS-ROSE-001`. | **MÝTUS** (příběh a lore) · **ŽIVÉ** (existence zdrojového řádku) / **STAVBA** (nová klientská metadata; build prošel, deploy neproběhl) · **NEDOLOŽENO** (fyzická růže) · **HORIZONT / NEZAHÁJENO** (pilot) | [`Ruze/Epilog-Zahrada-Genesis.md`](../Ruze/Epilog-Zahrada-Genesis.md); [`BodhiGaia/12`](../BodhiGaia/12-Ruze-v-Zahrade-Genesis.md); `APP&WEB/OasisWeb/src/domain/config/worlds.ts` (`GENESIS_GARDEN`, lore označena MYTH); [`BodhiGaia/pilots/GENESIS-ROSE-001.md`](../BodhiGaia/pilots/GENESIS-ROSE-001.md). | Deploy + ověření klienta pro nová metadata; splnění Gate A–D + Day 0 packet, pak kontroly D30/D90/D365; nenahrazuje M5 exit gate. |
| Dharma Temple (La Palma) — Merkaba, 7 kopulí, Strom života (Nová Bodhi Gaia). | **STAVBA (dokumentace) / HORIZONT (fyzicky)** | `public/V3/L5/docs/COMMUNITIES/dharma-temple.md`; `public/docs/terranova/dharma-temple.{cs,en}.md`; 3D koncept na `/terranova/dharma-temple`; OASIS svět `DHARMA_TEMPLE_LA_PALMA`. | Pozemek, entita, 5 zakládajících strážců, financování; žádná kopule nestojí. Detail: [`BodhiGaia/04`](../BodhiGaia/04-Dharma-Temple-Nova-Bodhi-Gaia.md). |
| Te Pīko Ora (Raiatea, Francouzská Polynésie) — živý pupek, wayfinding, mořská permakultura. | **HORIZONT** | `public/V3/L5/docs/COMMUNITIES/te-piko-ora.md` (Fáze 0 2026–2027); web `/terranova/te-piko-ora`. | Polynéský partner, ostrov, entita, tým. Detail: [`BodhiGaia/05`](../BodhiGaia/05-Te-Piko-Ora.md). |
| Komunity L5 provozují Guardian Node (split 90/10). | **NEPLATNÝ NÁROK** (dnes) | Všechny tři dokumenty: instalace Fáze 2 (2027 / 2028 / 2028). | Hardware, konektivita, pokladna. |
| OASIS zrcadlí pět L5 projektů a čtyři světy. | **ŽIVÉ** (klient) | `APP&WEB/OasisWeb/src/components/WorldPanel.tsx` (`NOVA_ZEME_PROJECTS` — pět projektů); `src/domain/config/worlds.ts` (`GENESIS_GARDEN`, `DHARMA_TEMPLE_LA_PALMA`, `GOLDEN_REPUBLIC_BOHEMIA`, `BODHI_LANKA`; všechny `layer: 5`). | Statusy napojené na L5 API; ověření deploy nových metadat (Rose lore je ve zdroji, deploy neproběhl). Detail: [`BodhiGaia/07`](../BodhiGaia/07-Zahrada-v-Oasis-a-na-Webu.md). |
...
| L5 má ověřený dopad (studna, strom, projekt). | **NEDOLOŽENO** | Žádný impact packet dosud nevznikl; exit gate M5 = jeden pilot celým cyklem. | První impact packet, nezávislý report. Detail: [`BodhiGaia/06`](../BodhiGaia/06-Protokoly-Zeme.md) a [`BodhiGaia/09`](../BodhiGaia/09-Kotva-Pravdy-a-Hranice.md). |
...
7. Příběh [`Ruze/`](../Ruze/00-README.md) (*Příběh Růže — Co nelze vlastnit*, DRAFT) je celý literární **MÝTUS** — bajka o Lumi a růži u řeky za otevřenou bránou, bez technického protějšku. Její existence **nepovyšuje** starší materiál kolem avatara „Meriam Rose“ (`V31/L4/oasis/data/avatars.json`, id 18) ani esoterické nároky dokumentu `docs/docs2.9/ZION_OASIS/SACRED_TRINITY/18_MERIAM_ROSE.md` na historickou, duchovní, technickou či provozní pravdu; jejich hranice drží [`Ruze/10-Kotva-Pribehu.md`](../Ruze/10-Kotva-Pribehu.md) a `Miriam/13`. Totéž platí pro její epilog v Zahradě Genesis: scéna s Lumi a Anou je **MÝTUS**, existence zdrojového řádku světa `GENESIS_GARDEN` v `APP&WEB/OasisWeb/src/domain/config/worlds.ts` je **ŽIVÝ** repozitářový fakt, nová klientská metadata jsou **STAVBA** (build prošel, deploy neproběhl) s obsahem výslovně označeným MÝTUS, fyzické zasazení růže v Algarve je **NEDOLOŽENO** a pilot [`GENESIS-ROSE-001`](../BodhiGaia/pilots/GENESIS-ROSE-001.md) je **HORIZONT / NEZAHÁJENO** — nesmí začít bez Gate A–D a nenahrazuje M5 exit gate.
