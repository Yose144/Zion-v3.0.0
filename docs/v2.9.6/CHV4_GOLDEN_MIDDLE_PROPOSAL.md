...
5. **Starší PC bez NPU musí zůstat plně podporované (CPU fallback).**
...
Poznámka: „NPU step povinně" znamená povinnou matematickou fázi v pipeline.  
**NPU hardware není povinný** — zařízení bez NPU používají deterministický CPU INT8 fallback.
...
2. NPU mixing zůstává povinnou fází (hardware NPU pouze akcelerace).
...
## 9) Navazující rollout specifikace

Pro konkrétní aktivaci, kompatibilitu se staršími PC a release checklist viz:
[CHV4_1_ROLLOUT_SPEC.md](CHV4_1_ROLLOUT_SPEC.md)
