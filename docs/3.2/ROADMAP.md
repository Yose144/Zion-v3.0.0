...
| E2 | AuxPoW real-pool E2E | core/miner | ≥2 GPU + ≥2 CPU coins accepted upstream on Edge pool | ✅ Complete — ZANO (GPU ProgPoW) + VRSC (CPU VerusHash) accepted on Edge pool 2026-08-10, 100% accept rate, 22-24 MH/s total; see `StatusV3.md` and `REPORT_2026-08-10_VRSC_REJECTION_FIX_AND_TUNING.md` |
| E3 | Profit switching live test | core/miner | switches within 15% hysteresis, PRL never selected | 🔄 Code complete and unit-tested; hysteresis 15% and PRL exclusion by no kernel + not in `CoinProfile::defaults` are in place. Live E2E evidence and known limitations tracked in `docs/3.2/AUTONOMOUS_PROFIT_ROUTER.md` |
...
| F2 | Complete 24h transaction fuzz evidence | QA | ✅ Complete — 24h run finished 2026-08-24 21:37 UTC against `127.0.0.1:9445`: 305 170 requests @ 10 workers, 99.4 % RPC success (errors = malformed payloads as designed), **0 health fails**, node stayed responsive. Report: [`docs/3.2/REPORTS/fuzz_logs/tx_fuzz_24h_report_20260823_233724.md`](./REPORTS/fuzz_logs/tx_fuzz_24h_report_20260823_233724.md) |
