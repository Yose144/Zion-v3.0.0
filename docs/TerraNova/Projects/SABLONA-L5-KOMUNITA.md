# Terra Nova Projects — Šablona L5 Komunity

Tento adresář obsahuje projektové listy pro fyzické komunity sítě Terra Nova — L5 vrstva (fyzické místo a soběstačnost).

Každý projekt je "živý dokument" — průběžně se zpřesňuje s tím, jak projekt fyzicky roste.

## Aktivní projektové listy

| Projekt | Lokace | Status | Soubor |
|---------|--------|--------|--------|
| **Zahrada Genesis** | Algarve, Portugalsko | 🟡 Aktivní rozvoj | [ZAHRADA-GENESIS-PORTUGAL.md](ZAHRADA-GENESIS-PORTUGAL.md) |
| **Dharma Temple** | La Palma, Kanárské ostrovy | 🔵 Příprava | [DHARMA-TEMPLE-LA-PALMA.md](DHARMA-TEMPLE-LA-PALMA.md) |
| **Te Pīko Ora** | Raiatea / Tahiti, Francouzská Polynésie | 🔵 Vize / Příprava | [TE-PIKO-ORA.md](TE-PIKO-ORA.md) |
| **Golden Republic Bohemia** | Čechy, Česká republika | 🔵 Vize / Příprava | [GOLDEN-REPUBLIC-BOHEMIA.md](GOLDEN-REPUBLIC-BOHEMIA.md) |
| **Bodhi Lanka** | Srí Lanka | 🔵 Vize / Příprava | [BODHI-LANKA.md](BODHI-LANKA.md) |
| **LUMI — Nová Amerika** | Kostarika | 🔵 Vize / Příprava | [NOVA-AMERIKA-KOSTARIKA.md](NOVA-AMERIKA-KOSTARIKA.md) |
| **Projekt Venus** | TBD | 🔵 Konceptuální fáze | [PROJEKT-VENUS.md](PROJEKT-VENUS.md) |

## Archetypová role každého uzlu

```
Zahrada Genesis          →  Base Camp   →  Země / Kořen   →  Pohyb, práce, oceán, farma
Dharma Temple            →  Sanctuary   →  Oheň / Kmen    →  Ticho, meditace, dharma, sopka
Te Pīko Ora              →  Paradise    →  Voda / Koruna  →  Plnost, mořská permakultura, wayfinding
Golden Republic Bohemia  →  Governance  →  Vzduch / Srdce →  Governance lab, kruh rozhodnutí, česká moudrost
Bodhi Lanka              →  Bhakti      →  Akasha / Éter  →  Láska, Sri Maha Bodhi, Rama Setu, Ayurveda
LUMI (Nová Amerika)      →  Most        →  Paměť          →  Nativní kultury, Kostarika, středobod Amerik (+ L6 pozemní stanice)
Projekt Venus            →  Regenerace  →  Ženský princip →  Byliny, tvořivost, obnova krajiny
```

## Sdílené protokoly (všechny L5 uzly)

- **ZION Node** — Guardian uzel sítě, 10 % odměn do humanitárního fondu
- **Seed Library** — výměna osiva mezi uzly, focus na lokální odrůdy
- **Medical Table** — zdravotní a wellness protokoly komunity
- **LoRa / Meshtastic** — off-grid komunikace v areálu i mezi uzly
- **Governance** — kompatibilní modely (sociokracie + ZION DAO)

---

## Šablona: Off-Grid Community — Projektový List

```
# [Název projektu] — L5 Komunita Terra Nova

## Základní data
- **Lokace:** [Stát, region, GPS nebo obec]
- **Klimatická zóna:** [Mediteránní / Atlantická / Kontinentální / Tropická]
- **Status:** [Vize / Příprava / Aktivní / Zavedená]
- **Verze listu:** [datum]
- **Guardian(i):** [jméno nebo alias]

---

## Vize a záměr
[1–3 věty o tom, proč projekt existuje a komu slouží]

---

## Fyzická infrastruktura

### Energie
- Zdroj: [solar / vítr / hydro / kombinace]
- Kapacita: [kWh/den, orientačně]
- Záložní: [baterie, diesel backup, jiné]

### Voda
- Zdroj: [studna / déšť / obecní / řeka]
- Sběr dešťové vody: [ano/ne, kapacita m³]
- Čištění: [filtr / membrana / UV]

### Jídlo
- Zahrada: [m² / ha]
- Metody: [permakultura / biodynamika / syntropic]
- Soběstačnost cíl: [% kalorií z vlastní produkce]

### Ubytování
- Kapacita: [počet osob]
- Typy: [trvalé bydlení / hosté / retreaty]

---

## Governance

- **Model:** [Sociokracie / DAO / hybridní]
- **Rozhodování:** [konsensuální / prahové hlasování]
- **ZION DAO:** [node ID nebo "plánováno"]
- **Minimální buňka:** [počet zakládajících členů]

---

## ZION Integrace

- **ZION Node:** [ano / plánováno / ne]
- **Wallet Guardian fondu:** [adresa nebo TBD]
- **Humanitární příspěvek:** [% z node odměn]
- **Medical Table:** [plánováno / aktivní / ne]
- **LoRa / Mesh:** [plánováno / aktivní / ne]

---

## Klíčové aktivity

| Aktivita | Status | Poznámka |
|----------|--------|----------|
| | | |

---

## Fáze rozvoje

| Fáze | Cíl | Časový rámec |
|------|-----|--------------|
| 0 — Zárodek | Právní základ, první Guardian | |
| 1 — Kořeny | Energie, voda, základní jídlo | |
| 2 — Komunita | Trvalí obyvatelé, governance | |
| 3 — Síť | Propojení s dalšími L5 uzly | |
| 4 — Výzařování | Hosté, retreaty, vzdělávání | |

---

## Poznámky / Otevřené otázky

- [ ] ...

---

## Kontakt / Zdroje

- Web:
- Koordinátor:
- Dokumenty:
```
