# 🌍 OASIS Worlds — Světová dokumentace

Tento adresář obsahuje koncepty jednotlivých světů, dimenzí a prostorů OASIS.

> **Hlavní vizi najdeš v:** [`APP&WEB/OasisWeb/OASIS_UNIVERSE.md`](../../../APP&WEB/OasisWeb/OASIS_UNIVERSE.md)  
> **Technická pravidla pro UE5 import:** [`AGENTS.md`](../../../AGENTS.md)

---

## Cíl

Každý soubor `.md` popisuje jeden svět. Může to být:

- hvězdný systém (Sirius, Arcturus, Pleiady …),
- planeta / herní žánr (Doom, GTA, Cyberpunk …),
- filmový / kulturní svět (Star Wars, Star Trek, Matrix, starověký Egypt …),
- dimenze / časová linie / hráčský svět.

Cílem je maximálně rozvinout možnosti OASIS multiverse — každý svět má svou atmosféru, obyvatele, mechaniky, questy a vztah k honbě za Zlatým vejcem.

---

## Struktura souborů

Pro nové světy použij šablonu [`TEMPLATE.md`](TEMPLATE.md).

---

## Kategorie světů

### Hvězdné systémy
- `SIRIUS.md`
- `ALPHA_CENTAURI.md`
- `ORION.md`
- `PLEIADES.md`
- `ARCTURUS.md`
- *(další: Vega, Antares, Betelgeuse, Tau Ceti, Proxima Centauri …)*

### Žánrové planety
- `PLANET_DOOM.md`
- `PLANET_GTA.md`
- `PLANET_QUAKE.md`
- `PLANET_WOW.md`
- `PLANET_LOL.md`
- `PLANET_CYBERPUNK.md`
- `PLANET_ATARI.md`

### Filmové / space-opera sektory
- `SECTOR_STAR_WARS.md`
- `SECTOR_STAR_TREK.md`
- `SECTOR_MATRIX.md`
- `SECTOR_BLADE_RUNNER.md`
- `SECTOR_DUNE.md`

### Kulturní / historické světy
- `WORLD_ANCIENT_EGYPT.md`
- `WORLD_ATLANTIS.md`
- `WORLD_LEMURIA.md`
- `WORLD_NORSE_CELTIC.md`
- `WORLD_MAYA.md`
- `WORLD_INDIA.md`
- `WORLD_JAPAN.md`
- `WORLD_CHINA.md`
- `WORLD_AFRICA.md`
- `WORLD_ABORIGINAL_AUSTRALIA.md`
- `WORLD_AOTEAROA.md`
- `WORLD_INDONESIA.md`

### Dimenzní / meta světy
- `DIMENSION_8BIT.md`
- `DIMENSION_HORROR.md`
- `DIMENSION_WESTERN.md`
- `DIMENSION_AI_FUTURE.md`

---

> **Toto je živý index.** Světy se budou neustále přidávat. Každý soubor má být samostatný koncept, který se dá později převést do `src/domain/config/worlds.ts` a UE5.
