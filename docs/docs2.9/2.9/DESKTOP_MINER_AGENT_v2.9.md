...
- 📷 **QR Wallet Import** - Skenování presale QR z emailu pomocí webcam
- 🔐 **Secure Mnemonic Storage** - AES-256 encrypted local storage
- 📲 **Mobile Sync** - Sync wallet mezi desktop a mobile app

---

## 📷 QR Wallet Import System (PLÁNOVÁNO)

**Priorita:** P2 | **Timeline:** Q1 2026 | **Effort:** ~16 hodin

### Popis
Presale zákazníci obdrží email s QR kódem obsahujícím jejich wallet mnemonic. Desktop Mining Agent bude schopen tento QR kód naskenovat přímo z webkamery a automaticky importovat wallet pro mining.

### User Flow
```
┌──────────────────────────────────────────────────────────────────────────┐
│  1. Uživatel spustí ZION Miner                                           │
│  2. Klikne "Import Wallet from Email"                                   │
│  3. Povolí přístup k webkameře                                          │
│  4. Namíří kameru na QR kód v emailu/vytištěný                          │
│  5. App automaticky:                                                     │
│     • Parsuje ZION URI (zion://import?mnemonic=...)                     │
│     • Zobrazí preview wallet adresy                                     │
│     • Šifrovaně uloží mnemonic (AES-256)                                │
│  6. Wallet je ready → Start Mining!                                      │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│   │    ║              [Webcam Preview Area]                    ║  │    │
│   │    ║                                                       ║  │    │
│   │    ║      Point your camera at the QR code from your      ║  │    │
...
│   │   └─────────────────────────────────────────────────────────┘ │    │
│   │                                                               │    │
│   │   [ Enter Mnemonic Manually ]                  [ Cancel ]     │    │
│   │                                                               │    │
│   └───────────────────────────────────────────────────────────────┘    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
**QR Scanner Component (Electron Renderer):**
```javascript
// src/renderer/components/QRWalletImport.jsx
import { Html5Qrcode } from 'html5-qrcode';
import { parseZionUri } from '../utils/zionUri';

const QRWalletImport = ({ onWalletImported }) => {
  ...
  const startScanning = async () => {
    ...
    await html5QrCode.start(
      ...
      { fps: 10, qrbox: 250 },
      (decodedText) => {
        const wallet = parseZionUri(decodedText);
        if (wallet.canImport) {
...
**ZION URI Parser (shared with mobile-app):**
```javascript
// src/renderer/utils/zionUri.js
// Reuse from mobile-app/src/utils/zionUri.js

export function parseZionUri(input) {
  if (input.startsWith('zion://import?')) {
    const params = new URLSearchParams(input.split('?')[1]);
    const mnemonic = params.get('mnemonic')?.replace(/\+/g, ' ');
    const network = params.get('network') || 'mainnet';
    return {
      type: 'import',
      mnemonic,
      network,
      canImport: mnemonic?.split(' ').length === 12
    };
  }
...
```javascript
// main.js - Wallet storage
const Store = require('electron-store');
const crypto = require('crypto');

const store = new Store({
  encryptionKey: getMachineKey(), // Hardware-bound key
});

function saveWallet(wallet) {
  const encrypted = encrypt(wallet.mnemonic);
  store.set('wallet', {
...
| Soubor | Repo | Popis |
|--------|------|-------|
| `zionUri.js` | `mobile-app/src/utils/` | ZION URI parser (reuse) |
| `QRScanner.js` | `mobile-app/src/components/` | Mobile reference |
| `mobile_qr_generator.py` | Server `~/api/` | QR generator backend |

### Úkoly (TODO)
- [ ] Implementovat QR scanner component
- [ ] Port `zionUri.js` z mobile-app
- [ ] Přidat webcam permission dialog
- [ ] Implementovat drag & drop QR image
- [ ] Secure mnemonic storage (AES-256)
- [ ] UI design (WARP style)
- [ ] Testing na všech platformách (Win/Mac/Linux)
