...
///
/// Decred BLAKE3 (DCP-0011) and GhostRider (RTM) return the PoW hash in
/// little-endian byte order (byte 0 = LSB, byte 31 = MSB), as per Bitcoin's
/// uint256 convention.  The target bytes are big-endian (byte 0 = MSB), as
/// produced by `difficulty_to_target` or received from a stratum pool.
///
