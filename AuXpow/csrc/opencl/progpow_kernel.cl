// ProgPow (EPIC — Epic Cash) OpenCL kernel — REAL implementation from EpicCash/progpow-rust
//
// ProgPow v0.9.3 (EIP-1057) with EPIC parameters:
//   PROGPOW_PERIOD = 50, PROGPOW_CNT_CACHE = 12, PROGPOW_CNT_MATH = 20
//   ETHASH_DATASET_PARENTS = 256
//   NO "RAVENCOINKAWPOW" constant (unlike KawPow/Ravencoin)
//
...
typedef struct __attribute__ ((aligned (PROGPOW_DAG_LOADS * 4))) {uint32_t s[PROGPOW_DAG_LOADS];} dag_t;

// ── progPowLoop function placeholder ─────────────────────────────────
// This function is generated by the host-side random math generator
// (port of ProgPow::getKern) and injected at compile time.
// The generated code includes: dag_t, progPowLoop function with
// random cache accesses, random math operations, and DAG data loads.
PROGPOW_INCLUDE_PROGPOW_LOOP
...
)
{
    ...
    for (uint32_t h = 0; h < PROGPOW_LANES; h++)
    {
		...
		if (h == lane_id)
		{
            ...
            for (int i = 0; i < 8; i++)
			{
                digest.uint32s[i] = digest_temp.uint32s[i];
			}
        }
    }
    ...
    if (keccak_f800(g_header, seed, digest) <= target)
    {
        ...
        // Also write to original g_output for compatibility
		uint slot = atomic_inc(&g_output[0]) + 1;
        if(slot < (MAX_OUTPUTS+1)){
            g_output[slot] = gid;
			g_output[2] = digest.uint32s[0];
			g_output[3] = digest.uint32s[1];
			g_output[4] = digest.uint32s[2];
			g_output[5] = digest.uint32s[3];
			g_output[6] = digest.uint32s[4];
			g_output[7] = digest.uint32s[5];
			g_output[8] = digest.uint32s[6];
			g_output[9] = digest.uint32s[7];
		}
    }
}
