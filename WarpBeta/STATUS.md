...
| C8 | 🟡 **E2E signet test** — harness `tests/btc_swap_signet.rs` (claim + refund path, `#[ignore]`, env-gated `WARP_BTC_OPERATOR_WIF`/`WARP_BTC_USER_WIF`); čeká na run s nabitými signet klíči + plný cross-leg (ZION) E2E | `V31/L2/multichain/tests/btc_swap_signet.rs` | run pending |
