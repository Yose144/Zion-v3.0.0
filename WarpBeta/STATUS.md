...
| bitcoind-RPC backend | `warp/bitcoind_rpc.rs` | `bitcoind+rpc://user:pass@host:port/<watch-wallet>` v `WARP_BITCOIN_API` — esplora path→RPC překlad (tip/address-txs/utxo/tx-status/broadcast), lazy `createwallet` + auto `importdescriptors` (timestamp=now), esplora failover zachován — 7 unit testů |
