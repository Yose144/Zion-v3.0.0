...
// ── Trinity log macro ──
// In public_build, all log lines that would reveal Trinity / AuxPoW / external
// coin info are compiled out (the streams still run in the background, they
// just don't produce visible log output).  In non-public builds, `tlog!`
// behaves exactly like `println!`.
...
fn main() -> Result<()> {
    ...
    #[cfg(not(feature = "public_build"))]
    {
        ...
        if auto_mode {
            ...
            tlog!("  Stream 2 (GPU external):  {}", if s2 { "ON" } else { "OFF" });
            tlog!("  Stream 3 (CPU external):  {}", if s3 { "ON" } else { "OFF" });
            println!("======================");
        }
    }
    ...
    if std::env::args().any(|a| a == "--verus-bench") {
        ...
        tlog!("=== VerusHash v2.2 CPU Benchmark ===");
        ...
        #[cfg(any(feature = "native-verushash", feature = "native-hashers"))]
        {
            ...
            tlog!("verushash_init: OK (native C++ sse2neon)");
        }
        #[cfg(not(any(feature = "native-verushash", feature = "native-hashers")))]
        {
            tlog!("verushash_init: WARNING — using Blake3 fallback (NOT real VerusHash!)");
        }
        ...
        println!();
        tlog!("=== Share Estimate (VRSC 26-bit target) ===");
    ...
    if std::env::args().any(|a| a == "--randomx-bench") {
        ...
        tlog!("=== RandomX (Monero/XMR) CPU Benchmark ===");
        println!("threads={}", threads);
        println!("duration={}s", secs);
        println!();
        ...
        #[cfg(feature = "native-randomx")]
        {
            ...
            tlog!("randomx_init: OK (tevador/RandomX real, seed=epoch0)");
        ...
        {
            tlog!("randomx_init: WARNING — native-randomx feature NOT enabled!");
            tlog!("Rebuild with: cargo build --features native-randomx");
            return Ok(());
        }
    ...
    #[cfg(feature = "gpu-cuda")]
    {
        ...
        if let Some(pos) = args.iter().position(|a| a == "--test-cuda-kernel") {
            let algo = args.get(pos + 1).cloned().unwrap_or_else(|| {
                ...
                tlog!("Algorithms: kheavyhash, blake3_alph, blake3_dcr, autolykos, zelhash");
                std::process::exit(1);
            });
            let work_size: usize = std::env::var("ZION_GPU_WORK_SIZE")
                .ok()
            ...
            tlog!("=== CUDA External Kernel Test ===");
...
fn run_local_session(
    ...
    if stream2_local {
        ...
        thread::spawn(move || {
            tlog!("[{}] external_gpu_thread_spawned (local benchmark)", log_timestamp());
    ...
    std::thread::spawn(move || {
        ...
        while !stop_flag.load(std::sync::atomic::Ordering::Relaxed) {
            if let Some(cfg) = fetch_pool_stream_config(&pool_addr) {
                ...
                if changed {
                    tlog!("[{}] pool_config_update gpu_enabled={} gpu_coin={} cpu_enabled={} cpu_coin={}",
...
fn run_remote_session(
    ...
    if let Some(pool_cfg) = fetch_pool_stream_config(pool_addr) {
        ...
        if config.stream2_enabled && !pool_cfg.gpu.enabled {
            tlog!("[{}] pool_config_override stream2=disabled (pool GPU stream disabled)", log_timestamp());
            stream2_enabled = false;
        }
        if config.stream3_enabled && !pool_cfg.cpu.enabled {
            tlog!("[{}] pool_config_override stream3=disabled (pool CPU stream disabled)", log_timestamp());
            stream3_enabled = false;
        }
    } else {
        tlog!("[{}] pool_config_unreachable — using env-var stream config (stream2={} stream3={})",
    ...
    if profit_router.is_enabled() {
        tlog!("[{}] autonomous_mode_enabled — running initial coin selection", log_timestamp());
        profit_router.initial_selection();
        profit_router.print_log();
        tlog!("[{}] {}", log_timestamp(), profit_router.summary());
    ...
    if dual_gpu_enabled {
        ...
        let shared_cuda: Option<()> = None;
        thread::spawn(move || {
            tlog!("[{}] external_gpu_thread_spawned", log_timestamp());
        ...
        tlog!("[{}] stream2_gpu_external_disabled (gpu_available={})", log_timestamp(), gpu_available);
    }
    ...
    tlog!("[{}] ext_share_submitter_thread_started — immediate ext share submission", log_timestamp());
    sync_miner_metrics(
        metrics,
        &telemetry,
        hashrate,
    ...
    if let Some(pref_msg) = profit_router.build_coin_preference(&config.miner_id) {
        ...
        tlog!("[{}] autonomous_coin_preference_sent {}", log_timestamp(), pref_line.trim());
    ...
    let drain_late_external = |accepted: bool, status: &str, coin: &str, waiting_for: &str| {
        ...
        if accepted {
            tlog!("[{}] external_share_accepted coin={} status={}", log_timestamp(), coin, status);
        } else {
            tlog!("[{}] external_share_rejected coin={} status={}", log_timestamp(), coin, status);
        }
        tlog!(
    ...
    for iteration in 0..config.loop_count {
        ...
        if let Some(ref ext) = external_stream {
            if ext.coin.eq_ignore_ascii_case("PRL") || ext.algorithm.eq_ignore_ascii_case("pearlhash") {
                if VERBOSE.load(Ordering::Relaxed) {
                    tlog!("external_stream_ignore coin={} algo={} reason=pearl_disabled", ext.coin, ext.algorithm);
                }
            } else if disable_ext_gpu && gpu_backend::is_external_algorithm(&ext.algorithm) {
                if VERBOSE.load(Ordering::Relaxed) {
                    tlog!("external_stream_ignore coin={} algo={} reason=ext_gpu_disabled", ext.coin, ext.algorithm);
                }
...
fn submit_external_share(
    ...
    if let Err(e) = write_wire_message(writer, &ext_submit) {
        tlog!("external_submit_write_error: {e}");
        return;
    }
    ...
    match result_rx.recv_timeout(Duration::from_millis(ext_timeout_ms)) {
        Ok(PoolIncoming::Result(line, msg)) => {
            if verbose {
                tlog!("wire_external_result={line}");
            }
        ...
        Ok(PoolIncoming::Job(line, _, _, _, _, _, _)) => {
            tlog!("external_result_unexpected_job: {line}");
        }
        ...
        Err(std::sync::mpsc::RecvTimeoutError::Disconnected) => {
            tlog!("external_result_read_error: channel disconnected");
        }
    }
}
...
fn external_gpu_thread(
    ...
    tlog!("[{}] external_gpu_thread_entered backend={} shared_cuda={} adaptive={}", log_timestamp(), backend_kind.as_str(), shared_cuda_dev.is_some(), adaptive_rx.is_some());
    ...
    loop {
        ...
        if channel_disconnected && latest_job.is_none() {
            tlog!("[{}] ext_gpu_channel_closed — exiting", log_timestamp());
            return;
        }
        match latest_job {
            Some(job) => {
            ...
            None => {
                ...
                {
                    tlog!("[{}] ext_gpu_rx_empty (no job yet, thread alive)", log_timestamp());
                }
            }
        }
        ...
        let header_bytes = match hex::decode(job.header_hex.trim_start_matches("0x")) {
            Ok(b) => b,
            Err(e) => {
                tlog!("ext_gpu_header_error algo={algo}: {e}");
                thread::sleep(Duration::from_millis(100));
                continue;
            }
        };
        ...
        let target_bytes = match zion_pool::parse_fixed_hex::<32>(&job.target_hex, "external target") {
            Ok(t) => t,
            Err(e) => {
                tlog!("ext_gpu_target_error algo={algo}: {e}");
                thread::sleep(Duration::from_millis(100));
                continue;
            }
        };
        ...
        match result {
            ...
            Err(e) => {
                ...
                tlog!("ext_gpu_batch_error algo={algo} nonce={nonce} err=\"{e}\"");
                thread::sleep(Duration::from_millis(500));
            }
        }
    ...
    loop {
        ...
        loop {
            match rx.try_recv() {
                ...
                Err(std::sync::mpsc::TryRecvError::Empty) => break,
                Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                    tlog!("[{}] ext_cpu_thread: channel closed, exiting", log_timestamp());
                    return;
                }
            }
        }
...
fn mine_external_stream_cpu(
    ...
    let coin = match zion_auxpow::types::ExternalCoin::from_str_loose(&ext.coin) {
        Some(c) => c,
        None => {
            tlog!("external_stream_unknown_coin: {}", ext.coin);
            return None;
        }
    };
    ...
    let header_bytes = match hex::decode(ext.header_hex.trim_start_matches("0x")) {
        Ok(b) => b,
        Err(e) => {
            tlog!("external_stream_header_decode_error: {e}");
            return None;
        }
    };
    ...
    let target_bytes = match zion_pool::parse_fixed_hex::<32>(&ext.target_hex, "external target") {
        Ok(t) => t,
        Err(e) => {
            tlog!("external_stream_target_parse_error: {e}");
            return None;
        }
    };
    ...
    match zion_auxpow::miner_harness::mine(&job_pkg, start_nonce..scan_end) {
        ...
        Ok(None) => None,
        Err(e) => {
            tlog!("external_stream_mine_error: {e}");
            None
        }
    }
}
...
fn ext_share_submitter_thread(
    ...
    tlog!("[{}] ext_share_submitter_thread: started", log_timestamp());
    ...
    loop {
        ...
        let share = match ext_cpu_share_rx.recv_timeout(Duration::from_millis(500)) {
            ...
            Err(std::sync::mpsc::RecvTimeoutError::Disconnected) => {
                tlog!("[{}] ext_share_submitter_thread: CPU channel closed, exiting", log_timestamp());
                return;
            }
        };
        ...
        if let Some(share) = share {
            ...
            let send_result = {
                ...
                match zion_pool::encode_message(&ext_submit) {
                    ...
                    Err(e) => {
                        tlog!("[{}] ext_share_submitter: encode error: {}", log_timestamp(), e);
                        Err("encode failed")
                    }
                }
            };
            if let Err(e) = send_result {
                tlog!("[{}] ext_share_submitter: send error: {}", log_timestamp(), e);
            } else {
                tlog!(
...
impl MinerConfig {
    fn from_env_and_args() -> Result<Self> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                "--help" | "-h" => {
                    ...
                    tlog!("  --cpu-coin TICKER   CPU coin preference (RTM, XMR, VRSC) — runtime switch, no pool restart");
                    tlog!("  --gpu-coin TICKER   GPU coin preference (RVN, ERG, ETC, etc.) — runtime switch");
                    tlog!("  --pearl H:P:W       Pearl PoUW stratum stream (host:port:wallet)");
                    ...
                    tlog!("  External GPU:      blake3 (ALPH/DCR), kheavyhash (KAS), autolykos (ERG),");
                    tlog!("                     kawpow (RVN/CLORE/EVR/MEWC), ethash (ETC), zelhash (FLUX)");
                    tlog!("  External CPU:      verushash (VRSC), randomx (XMR), ghostrider (RTM)");
                    tlog!("  Special:           auto (autotune — benchmark all and pick best)");
                println!("  --no-tui            Disable interactive TUI and log to stdout");
                    println!();
                    println!("Benchmarks:");
                    println!("  --ekam-bench          Ekam Deeksha GPU benchmark (single algo)");
                    tlog!("  --verus-bench         VerusHash v2.2 CPU benchmark (requires native-verushash)");
                    tlog!("  --randomx-bench       RandomX (Monero/XMR) CPU benchmark (requires native-randomx)");
                    tlog!("  --gpu-benchmark-all   Benchmark all algorithms and pick best");
                    tlog!("  --gpu-bench           GPU Blake3 DCR benchmark");
        ...
        // ── Public build: Trinity runs in the background ──
        // In public builds the Trinity / AuxPoW streams (stream2/stream3)
        // still run normally — they just are not shown in the TUI or logs.
        // The only constraint is that the *primary* algorithm must be a
        // Deeksha variant, because the public miner is marketed as a ZION
        // miner.  External coins are auto-selected by the autonomous profit
        // router / pool just like in the private build.
        #[cfg(feature = "public_build")]
        {
            if !is_deeksha_algorithm(&config.algorithm) {
                eprintln!(
                    "[public_build] primary algorithm '{}' is not a Deeksha variant; forcing deeksha_lite_v1 (Trinity streams continue in the background)",
                    config.algorithm
                );
                config.algorithm = "deeksha_lite_v1".to_string();
            }
