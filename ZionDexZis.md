...
> **Status (2026-08-29):** Fáze 0–7 jsou ve `main` implementovány (merge `feat/ziondex-zis-multichain-wallet` v `9bfede06c`). `cargo test --workspace` prochází, `cargo clippy --workspace` je čisté (pouze pre-existing warnings). Rust E2E `deposit → swap → withdraw` a Playwright smoke test `/wallet/multichain` jsou v repo. Přidány Playwright E2E testy `e2e/multichain-flow.spec.ts` pro `deposit → withdraw` a `e2e/multichain-dex-flow.spec.ts` pro `/multichain#dex` swap quote → execute wZION → USDT; oba s mockovaným ZIS a multichain backendem. Failing `miner-stats` test opraven (health endpoint může vrátit `down`/`degraded` lokálně bez RPC/pool). UI swap widget `CrossChainSwapWidget` byl opraven — chyba `Cannot access 'es' before initialization` byla způsobena TDZ proměnné `minOutput` v dependency array `executeSwap` před její deklarací; `minOutput` přesunut před callback a `slippageBps` přidán do dependency array. Všechny web E2E testy (`e2e/`) nyní procházejí lokálně (8/8 passed). Reálný cross-chain HTLC workflow a on-chain testnet/mainnet E2E vyžadují deploy, autentizovaný test účet a operátorskou dokumentaci.
>
