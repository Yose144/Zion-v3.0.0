...
> **Status (2026-08-29):** Fáze 0–7 jsou ve `main` implementovány (merge `feat/ziondex-zis-multichain-wallet` v `9bfede06c`). `cargo test --workspace` prochází, `cargo clippy --workspace` je čisté (pouze pre-existing warnings). Rust E2E `deposit → swap → withdraw` a Playwright smoke test `/wallet/multichain` jsou v repo. Přidán Playwright E2E test `e2e/multichain-flow.spec.ts` pro `deposit → withdraw` flow s mockovaným backendem; všechny web E2E testy (`e2e/miner-stats.spec.ts`, `e2e/wallet-multichain.spec.ts`, `e2e/multichain-flow.spec.ts`) procházejí lokálně. Failing `miner-stats` test opraven (health endpoint může vrátit `down`/`degraded` lokálně bez RPC/pool). UI swap widget na `/dex` zůstává nepoužitelný v produkčním buildu kvůli `next.config.ts` redirectu na `/multichain#dex` a runtime chybě dynamic chunku 1007 (`Cannot access 'es' before initialization`) — vyžaduje samostatný fix. Reálný cross-chain HTLC workflow a plnohodnotný UI swap E2E vyžadují testnet/mainnet deploy, autentizovaný test účet a operátorskou dokumentaci.
>
