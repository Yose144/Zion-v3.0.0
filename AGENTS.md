...
> **⚠️ TRINITY MINING E2E 2026-08-10 (LATEST):** V31 Trinity Mining (ZION+ZANO+VRSC) je **PRODUCTION** na Edge pool. **2026-08-10 deploy fix:** na Edge běžel stale `zion-pool` binár bez posledních `pop_job()` / `touch_job_timestamp` změn, takže se VRSC/ZANO share odmítaly upstream (`job not found`, `Duplicate share`, `Job expired`). Fresh build z `V31/`, redeploy a restart `zion-v31-pool`; upstream nyní acceptuje VRSC i ZANO share. AuxPoW worker name zjednodušen na čisté `zion-pool` bez coin suffixu (`a5b3aa2c6`). Místní miner běží přes `~/Desktop/Start.sh --bg` a připojil se k poolu. Post-fix accept rate: ZION 100%, ZANO 100%, VRSC acceptováno upstream (~2 MH/s LuckPool worker `noname` z důvodu normalizace, ZANO ~13 MH/s HeroMiners worker `zion-pool`). Miner build: `cargo build --release --bin zion-miner --features gpu-cuda,native-all,tui` (native-verushash REQUIRED pro VRSC). Pool build: `cargo build --release -p zion-pool`. Miner launcher: `~/Desktop/Start.sh` (TUI) nebo `~/Desktop/Start.sh --bg` (background). Systemd service: `~/.config/systemd/user/zion-gpu-miner.service`. Report 2026-08-09: [`REPORT_2026-08-09_TRINITY_TUNING_FINAL.md`](./REPORT_2026-08-09_TRINITY_TUNING_FINAL.md); aktuální stav viz [`StatusV3.md`](./StatusV3.md).
>
