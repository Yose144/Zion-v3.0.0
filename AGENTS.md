...
- **Live tested:** 2026-07-12 — RVN/KawPow fully operational with real BTC wallet, shares counted under `src_kawpow` and forwarded to 2miners. **2026-07-13 — XMR/RandomX** pool connected, authorized, and queuing external RandomX jobs. **2026-07-15 — RandomX native build** — `tevador/RandomX` C++ library integrated via `native-randomx` feature, per-thread VM (lock-free, 175 H/s on M1 interpreted VM), seed_hash epoch plumbing, `--randomx-bench` benchmark. See [`RandomXReport.md`](./RandomXReport.md). **2026-07-13 — VRSC/VerusHash v2.2** C++ native build integrated (`native-verushash` feature), ZcashStratum protocol support active for LuckPool.
