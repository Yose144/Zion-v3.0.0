...
- **PoC-lab Fáze 2 — DharmaValidator, NclReputationRegistry, ConsciousnessLevel, multi-epoch stress testy, CLI (2026-07-08):** [`PoC-lab/`](./PoC-lab/) — standalone Rust workspace (mimo `V3/`) implementující Proof-of-Care prototyp. **Fáze 1 (commit `2936fcb1`)** — kompletní základ: INT8 NPU VM (`poc-npu`), multi-backend cross-validace (`poc-verifier`), validator registry + Sefirot Vow lifecycle (`poc-registry`), reward split + slashing (`poc-economics`), E2E síťový simulátor s guardian/Bodhisattva Vow demo (`poc-sim`). **Fáze 2 (commit `5d0aefea`)** — 119 testů PASS: `DharmaValidator` (5-pilířový pipeline: non-harm / authenticity / benefit / consciousness-alignment / temporal-coherence) + `HiranAwareVerifier` + `VerificationReport` v `poc-verifier`; `NclReputationRegistry` stub (in-memory, mirroring `V3/L3/ai-native/src/ncl`, ban_threshold=20.0, `compute_score = 100×success_rate×(1+level×0.05)`) + NCL bonus tiers (0–100 škála: ≥95→+5%, ≥85→+3.5%) v `poc-economics`; `ConsciousnessLevel` enum (L0 Dormant → L6 Grok, `can_compute()`, `ncl_bonus_factor()`) v `poc-core`, integrovaný do `poc-registry` (`ValidatorRecord.consciousness_level`, `can_do_poc_tasks()`, `upgrade_consciousness_level()`); `run_epochs(start, count)` + 8 multi-epoch stress testů (100 epoch, lazy validator rejection, payout cap) v `poc-sim`; CLI přes `clap` (`--epochs`, `--validators 1-64`, `--hiran-url`, `--block-reward`, `--min-stake`, `--min-care-score`, `--verbose`) v `poc-sim/src/main.rs`. Spec: [`docs/3.0.4/POC_HIRAN_INTEGRATION_SPEC.md`](./docs/3.0.4/POC_HIRAN_INTEGRATION_SPEC.md). Konceptuální základ: [`docs/3.0.4/PoC_CONCEPT.md`](./docs/3.0.4/PoC_CONCEPT.md). **Netýká se L1 consensus — čistě výzkumný prototyp.**
