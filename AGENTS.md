...
- **ZionDex backend** currently lives in `ZionDex/`; planned V3.1 location is `V3/L2/ziondex/` (moved into the workspace under L2 alongside bridge/warp/dao). See `ZionDex/README.md` (current) and [`V3.1_ARCHITECTURE_REORG_PLAN.md`](./V3.1_ARCHITECTURE_REORG_PLAN.md) for the reorg plan. **Status: Live Beta** — Router integrated with WARP API (port 8453), web UI live at `/dex` and `/ziondex` on zionterranova.com. Cross-chain AMM routing implemented (`aggregator.rs`). Multi-path quote API: `GET /quote/multi`. Pending: custom AMM deploy, intent-based execution. ZionDex Router service LIVE on Edge (port 8454, 7 chains). Contract addresses template: [`docs/3.0.5/CONTRACT_ADDRESSES.md`](./docs/3.0.5/CONTRACT_ADDRESSES.md).
...
- **V3.1 tree reorganization is planned:** WARP moves from `V3/L3/warp/` to `V3/L2/warp/`, ZionDex moves from `ZionDex/` to `V3/L2/ziondex/`, AuxPoW merges into `V3/L1/miner/`, and `L3` becomes AI/orchestration/automation/NCL/PoC. See [`V3.1_ARCHITECTURE_REORG_PLAN.md`](./V3.1_ARCHITECTURE_REORG_PLAN.md). Until the reorg is executed, keep using current paths.
