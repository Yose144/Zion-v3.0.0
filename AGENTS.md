...
> **⚠️ 2026-09-19 WARP BTC-SWAP STATE + EDGE ACCESS/REDEPLOY:** Nativní ZION↔BTC atomic swap (`BtcSwapFlow` v `V31/L2/multichain`) je implementovaný a audit-prep hotový — viz [`WarpBeta/STATUS.md`](./WarpBeta/STATUS.md) a [`WarpBeta/AUDIT_PREP.md`](./WarpBeta/AUDIT_PREP.md). Na Edge **zůstává disabled** (`WARP_BTC_SWAP_ENABLED` unset) do externího auditu. `warpd` redeployed na HEAD `50df05d54` (build na Edge v `/root/build/V31` — NE lokálně, glibc!). **Edge SSH:** `62.171.141.136:2222`, klíč `~/.ssh/zion-edge-post-wipe-2026-07-29`, user `root`. Lokální `~/.ssh/config` alias `zion` ukazuje na **mrtvou IP `91.98.122.165`** (reimaged cizí stroj — ignorovat). Novinky: R3 dedikovaný WARP ZION wallet (`WARP_BTC_SWAP_ZION_SECRET`/`_MNEMONIC` — vlastní keyring+adapter+coordinator, bez env → bridge keyring fallback s warn), coordinator restart hydration fix, solvency withdrawal double-count fix (verify_withdrawal počítal user claim dvakrát — full-balance withdrawals failovaly). `warp.db` perms 640. Pending před enable: externí audit, R3 ops (vygenerovat+nabít klíč), R4 vlastní esplora/bitcoind (Edge disk ~93G free — pruned node možný), signet run (čeká na funded adresy).
>
