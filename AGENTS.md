...
> **⚠️ 2026-09-20 ROBINHOOD CHAIN (L2):** Robinhood Chain (Arbitrum Orbit L2, mainnet od 2026-07-01, chain ID **4663**, testnet **46630**, ETH gas, ~100ms bloky, RPC `https://rpc.mainnet.chain.robinhood.com`, explorer `robinhoodchain.blockscout.com`) je registrována v L2 stacku: `ChainId::Robinhood` v `V31/L1/types`, EVM adaptery (`warp/adapter/evm.rs`, `chain/adapters/evm.rs`), `warp/registry` (22 chains), executor, deposit watcher (finality 12, native ETH), `contracts.rs`, `service.rs`/`db.rs` name maps, `warp.toml` configs (repo + Edge `/etc/zion/warp.toml` — **enabled=false**), web `ChainSelector`/`TokenSelector`/`dex-api`. **Deterministický deploy ověřen:** deployer `0xdde17506…` má na Robinhood nonce 0 → `CREATE@nonce0` = wZION `0x0c49…`, `CREATE@nonce1` = bridge `0xa5a0…` (stejné kanonické adresy jako jinde). **Pending před enable:** deploy `wZION.sol` + `ZIONBridge.sol` z validator.key (pořadí: wZION pak bridge, threshold 4/5 — na starších chainech je 5/5), nabít deployer ETH na 4663, doplnit `wzion_contract("robinhood")` v `warp/adapter/evm.rs` + `BRIDGE_CONTRACTS_ROBINHOOD` v `bridge-api.ts`, `enabled=true` + restart `zion-v31-multichain`. Testy: `zion-multichain` 655 pass, `zion-l1-types` 11 pass.
>
> **⚠️ 2026-09-19 WARP BTC-SWAP STATE + EDGE ACCESS/REDEPLOY:** Nativní ZION↔BTC atomic swap (`BtcSwapFlow` v `V31/L2/multichain`) je implementovaný a audit-prep hotový — viz [`WarpBeta/STATUS.md`](./WarpBeta/STATUS.md) a [`WarpBeta/AUDIT_PREP.md`](./WarpBeta/AUDIT_PREP.md). Na Edge **zůstává disabled** (`WARP_BTC_SWAP_ENABLED` unset) do externího auditu. `warpd` deployed `50df05d54` — **⚠️ postrádá R2 offer TTL `d8fe0f97d` + R4 esplora failover `d16922977`, redeploy potřeba před pilotem** (build na Edge v `/root/build/V31` — NE lokálně, glibc!). **Edge SSH:** `62.171.141.136:2222`, klíč `~/.ssh/zion-edge-post-wipe-2026-07-29`, user `root`. Lokální `~/.ssh/config` alias `zion` ukazuje na **mrtvou IP `91.98.122.165`** (reimaged cizí stroj — ignorovat). Novinky: R3 dedikovaný WARP ZION wallet **FUNDED 500 ZION** (tx `abe521fc…`, blok 49544), coordinator restart hydration fix, solvency withdrawal double-count fix. `warp.db` perms 640. **2026-09-20: testnet3 rehearsal STALLED — faucety nedoručily funding (0 tx na HTLC `tb1qwps3v5w…` ani wallet `tb1qmy6czt…`), offer TTL-failuje; doporučen mainnet dust pilot (~100k sats na `bc1q53gn9…5n6k`).** Pending před enable: externí audit, produkční `WARP_BTC_RELAY_KEY` (mainnet WIF), R4 vlastní esplora/bitcoind (Edge ~93G free — pruned node možný), Edge redeploy na HEAD.
>
...
>
> **⚠️ 2026-08-27 EDGE CPU MINER RE-ACTIVATION:** `zion-v31-miner` je znovu aktivní na Edge v CPU-only režimu. Služba běží jako `zion` user, `--pool 127.0.0.1:8444 --wallet ${ZION_MINER_WALLET} --worker ${ZION_MINER_WORKER} --threads 4 --no-gpu --no-cpu`, `ZION_GPU_BACKEND=cpu`, `Nice=19`, `CPUWeight=10`, `IOSchedulingClass=idle`, `OOMScoreAdjust=900`, `CPUQuota=400%` (drop-in `99-zion-tuning.conf` — miner běží jen na volných cyklech, ostatní služby mají přednost). Aktuálně ~140–155 H/s (reálné PoW evaluace po hashrate fixu 2026-09-13; starší reportované ~1.0–1.2 MH/s byly nadhodnocené `batch_size/elapsed`), `accept_rate=100%`. Peněženka a worker se nastavují v `/etc/zion/edge-environment.sh` (`ZION_MINER_WALLET`, `ZION_MINER_WORKER`). Systemd template: `V31/deploy/systemd/zion-v31-miner.service`. Použitá binárka: `/opt/zion/V31/target/release/zion-miner`.
>
