...
> **⚠️ 2026-09-25 DAO PARLAMENT + TREASURY TRUTH-SYNC (DEPLOYED):** Edge `zion-dao` a web nyní fail-closed rozlišují treasury UTXO balance od spendable: ověřeno při height 57 826 = 1.5B ZION UTXO, ale `unlock_height=144000`, time-lock aktivní, `admin_unlock_known=false` (produkční L1 zatím nemá `getAdminUnlocks`) a `spendable=0`. Nikdy neprezentovat 1.5B jako okamžitě dostupných; `treasury_sigs` jsou pouze approval/audit records, ne kryptografické podpisy ani broadcast. `/dao` má VISION Parlament se 4 komorami, Hiranem jako advisory draft zrcadlem (0 hlasů/0 klíčů/lidský sponsor) a Sítí Země — 144 000 je symbolický cíl, ne cap/live registry/treasury role. Live L2 registry: jen `zion-l1` + `base`; BTC swap zůstává disabled; Hiran a Guardian registry zůstávají offline/501. Legacy `/dex/:path*` se permanentně přesměrovává na `/multichain#dex`; hardening skryté liquidity stránky není změna routingu. Backups: `/opt/zion/V31/target/release/zion-dao.bak-20260925T152109Z-dao-parliament`, `/opt/zion/APP&WEB/website-v2.9/.next.bak-20260925T152109Z-dao-parliament`.
>
> **⚠️ 2026-09-25 POOL STREAM 3 (VRSC) OUTAGE + FIX (DEPLOYED):** Streamy 2/3 na `zion-v31-miner` (Edge Trustee) jsou záměrně vypnuté (`--no-gpu --no-cpu`; VPS nemá GPU a CPU držíme pro ZION bloky). Skutečný bug byl pool-side: **upstream `eu.luckpool.net:3956` servroval ~7h zamrzlý job `4e42216`** (50 reconnectů = pokaždé stejný template, žádné nové notify; ověřeno i z čistého testovacího socketu z jiné IP — upstream outage, ne náš klient). Failover: `ZION_POOL_AUXPOW_POOL_VRSC="pool.verus.io:9999"` v `/etc/zion/edge-environment.sh` (backup `.bak-vrsc-failover-*`; per-coin override je `ZION_POOL_AUXPOW_POOL_<COIN>` v `auxpow_runtime.rs`). **Protokol fix (commit níže):** `forward_share_to_upstream` posílal pro verushash `params[3]` jako `en2_size` nul (4B) — správně je `(32 − en1_len)` nulových bajtů (en1 + suffix = 32B header nonce; miner iteruje nonceSpace uvnitř solution, field zůstává en1+zeros). Empiricky validováno proti `pool.verus.io`: 4B → `incorrect size of nonce`; 28B + solution bez en1 → `invalid solution version`/`pool nonce missing`; 28B + solution s en1 v nonceSpace tail → `low difficulty share` (= validní formát, jen slabý PoW). Deploy: `zion-pool` rebuild na Edge (backup `zion-pool.bak-20260925-vrsc-nonce`), po restartu okamžitě **první VRSC share Accepted**; accept-rate ~23 acc / 17 `job not found` za 10 min (verus.io rotuje joby co pár sekund — churn je inherentní). Stream 2 (ZANO GPU) fungoval celou dobu (fresh jobs, 1204 accepted). Pozn.: Verus stratum submit = `[worker, job_id, ntime, nonce_suffix(28B), solution]`; `mining.notify` ntime = `params[5]`, nbits = `params[6]`; řešení = varint + 1344B, nonceSpace = posledních 15B solution (`[en1][nonce 4B LE][pad]`).
>
> **⚠️ 2026-09-21 PREMINE UNLOCK PATH (IMPLEMENTED, PENDING DEPLOY):** V31 nyní umí legitímně odemknout admin-lock premine slotů on-chain: transakce utrácející ≥1 UTXO od **každé** ze 3 kanonických admin adres (`v3_compat::ADMIN_L1_ADDRESSES`) s memo `ZION:ADMIN_UNLOCK:v1:<premine_adresa>[:<ref>]` se při `apply_transaction` zapíše do `UtxoSet.admin_unlocked` — deterministicky replay-derived přes `apply_block_unchecked`, restart-safe. `Node::validate_premine_and_maturity_for_tx` i mempool/template cesty nově konzultují tento set místo hardcoded `_no_admin`. Builder: `v31_wallet::build_admin_unlock_tx` (každý admin podepisuje stejný `signing_hash` offline). RPC: `getAdminUnlocks`. Memo `<ref>` má nést `AdminProposal.proposal_id` / DAO proposal id jako audit link (DAO hlas je off-chain, L1 vynucuje jen 3-of-3 multisig). Unlock je **trvalý a per-adresa** — není re-lock; po spendu sweepnout zbytek na fresh adresu. **Deploy order:** nejdřív updatovat VŠECHNY validující nody (starý binary unlock tx akceptuje jako normální spend, ale následný premine spend zamítne → stall), teprve pak broadcast unlock tx. Full doc: [`PREMINE_UNLOCK.md`](./PREMINE_UNLOCK.md). Souvisí: `docs/3.1/REPORTS/REPORT_2026-08-22_PREMINE_LOCK_BYPASS.md`, `ZION_SOFT_FORK_ACTIVATION=13100` na Edge.
>
...
>
> **⚠️ 2026-09-14 DAO TREASURY + UI LIVE + ZIS-GATED GOVERNANCE:** `zion-dao` API má nové treasury endpointy: `GET /api/dao/treasury` (live L1 UTXO součet přes `treasury_addresses` z configu — genesis sloty 7+8 = **1.5B ZION**; slot 6 je L6 Issobella, ne DAO), `POST /api/dao/treasury/submit|sign|execute` (guardian multisig 5-of-7, `treasury_sigs` tabulka v `/opt/zion/data/dao.db`). Governance mutace (vote/create proposal) akceptují `zion_session` cookie — DAO backend resolvuje identitu přes ZIS `/api/auth/me` (`zis_url` v configu, default `127.0.0.1:8096`) a váhu hlasu počítá z L1 balance ve snapshot bloku; client-supplied `voter`/`weight` se u session cesty ignorují. `X-DAO-Key` zůstává pro operátory. `/dao` je opět reálná stránka (redirect odstraněn z `next.config.ts`), používá `useAuth()`. Produkční config `/etc/zion/dao-mainnet.toml` se načítá přes `Environment="DAO_CONFIG=..."` v `zion-v31-dao.service`; `l1_rpc_url` je override přes `DAO_L1_RPC=127.0.0.1:9445`. První produkční návrh #1 = ratifikace treasury multisig. Solana adapter: deposit watcher kompletní (ATA discovery, outer+inner SPL parsing, slot confirmations, DB dedup), chain zůstává `enabled=false` do deploye reálného ZION SPL mintu; outbound `Unsupported`. Známé blokéry vyžadující klíče: dedikovaný Base RPC do `warp.toml` (public `mainnet.base.org` sdílí rate-limit budget i s interním retry/backoff v `EvmAdapter`), npm publish `zion-wallet-sdk` (expired token).

This file provides operating guidance to Devin, WARP, Copilot, and future automated agents working in this repository.

> **⚠️ 2026-09-02 ZIONDEX MAINNET + MINER HASHRATE FIX + TOKEN ICONS + DASHBOARD ZIS (HISTORICAL — DEPRECATED):** ZIONDex AMM byla **LIVE na Base Mainnet** s reálnou likviditou a E2E swapem. **ZISGate** deploynut na `0x55160347B33Bb56F0ea99499072Ba5bf8D2862A5` (tx `0x0cff45...`, cost ~0.0000035 ETH). **AMM pair tZION/tUSDT** vytvořen na `0x1fE64df93226b8434877D5826aE2DCEda171e39E` s 100k tZION + 1k tUSDT likviditou. **E2E swap prošel**: 1000 tZION → 9.87 tUSDT (tx `0x69eda7...`, gas 149,005). Test tokeny na Base: tZION `0xC5E79b8C6475137aC3a982651097a219B63b0c33` (18 dec), tUSDT `0x677693fbFDe6a9EeA655033fffF93054B559552C` (6 dec), tWETH `0xcE5Df8e83B87f462835b51Ac6B2A4c53fafA620F` (18 dec). Deployer wallet: `0xdde17506BC2D2dCE1d594bD1D85B0BAbb389D186` (validator-1 key `/etc/zion/keys/validator.key`). **Miner hashrate fix**: `parallel_zion_find_nonce` v `V31/L1/miner/src/runtime.rs` počítal `nonces_searched = found_nonce + 1` místo `batch_size` → hashrate se podreportoval. Opraveno v pool i solo mining cestě. Po redeploy na Edge: **84.45 kH/s** (dříve podreport). **Token icons**: ZION logo (128x128 PNG) integrováno napříč webem i marketplace přes `src/lib/token-icons.ts` + `src/components/dex/TokenIcon.tsx`; dostupné na `https://app.zionterranova.com/tokens/zion.png` a `https://market.zionterranova.com/tokens/zion.png`. **Dashboard ZIS auth (J4)**: `ZION_OS/dashboard/app.py` `_check_auth()` prioritizuje ZIS SSO cookie, fallback na Basic Auth; nový `/api/me` endpoint vrací `{"authenticated":true,"source":"zis"|"basic","role":"operator"}`. **Marketplace ZIS (J2)**: `AuthContext` + `ConnectButton` integrovány se ZIS clientem. Všechny Edge služby active: `zion-v31-multichain` (s test tokeny), `zion-v31-miner` (84.45 kH/s), `zion-website`, `zion-marketplace`, `zion-edge-python-dashboard`. npm publish wallet SDK přeskočeno (npm token expired). Report: [`docs/3.1/REPORTS/REPORT_2026-09-02_ZIONDEX_MAINNET_HASHRATE_FIX.md`](./docs/3.1/REPORTS/REPORT_2026-09-02_ZIONDEX_MAINNET_HASHRATE_FIX.md).
>
> **⚠️ 2026-08-27 EDGE CPU MINER RE-ACTIVATION:** `zion-v31-miner` je znovu aktivní na Edge v CPU-only režimu. Služba běží jako `zion` user, `--pool 127.0.0.1:8444 --wallet ${ZION_MINER_WALLET} --worker ${ZION_MINER_WORKER} --threads 4 --no-gpu --no-cpu`, `ZION_GPU_BACKEND=cpu`, `IOSchedulingClass=idle`, `OOMScoreAdjust=900`, `CPUQuota=400%` (drop-in `99-zion-tuning.conf`). **2026-09-21 retuning — miner má prioritu (Trustee = produkční miner):** `Nice=5`, `CPUWeight=150` (dříve `Nice=19`/`CPUWeight=10` = jen volné cykle; node má 200). Zároveň `zion-bitcoind.service` **STOPPED** kvůli CPU/IO spotřebě IBD (byl na ~55 %, pokračuje po `systemctl start zion-bitcoind` tam, kde skončil; do WARP zatím není zapojený). Stejný limit IBD případně přes drop-in `zion-bitcoind.service.d/50-ibd-nice.conf` (`CPUWeight=20`, `CPUQuota=60%`, `IOWeight=10`). Pozor: na Edge právě běží `cargo build` warpd v `/root/build/V31` (build je tam správně kvůli glibc — reniced na 19, nekillovat). Reálný ceiling mineru ~140–155 H/s jen při volných 4 jádrech; pod zátěží/buildem ~50–75 H/s. Starší reportované ~84–90 kH/s i ~1.0–1.2 MH/s byly nadhodnocené metriky (přepočet 2026-09-02/13). `accept_rate=100%`. Peněženka a worker se nastavují v `/etc/zion/edge-environment.sh` (`ZION_MINER_WALLET`, `ZION_MINER_WORKER`). Systemd template: `V31/deploy/systemd/zion-v31-miner.service`. Použitá binárka: `/opt/zion/V31/target/release/zion-miner`.
>
> **📋 2026-08-23 ZIONDEX + ZIS MULTICHAIN WALLET PLAN (SUPERSEDED 2026-09-03):** Active end-to-end plan for a ZIS-backed multichain wallet, real DEX settlement, and per-user deposit/ledger tracking is captured in [`ZionDexZis.md`](docs/3.2/ZionDexZis.md). It supersedes the historical ZionDex beta scope below and is the authoritative implementation plan for the DEX → wallet → settlement flow. **Update:** tento plán byl 2026-09-03 nahrazen zjednodušeným [`ZIS_WALLET_PLAN.md`](./ZIS_WALLET_PLAN.md) — bez vlastního AMM, swapy přes Uniswap V3; kanonický registr kontraktů je [`L2contracts.md`](./L2contracts.md).
>
> **⚠️ NODE REWARD SYSTEM 2026-08-23 (LATEST):** Implementován protocol-level reward systém pro full user nody. Po `node_reward_activation_height` se 1% block subsidy (dříve burns jako `POOL_FEE_PCT`) mintuje do node-reward poolu (`node_reward_address`) místo burn. L1 coinbase má post-activation 4 výstupy: 0=miner, 1=humanitarian, 2=issobella, 3=node reward. Kanonická adresa poolu je `zion_core::v3_compat::MAINNET_CANONICAL_NODE_REWARD_WALLET`. CLI `zion node start` a binárka `zion-node` podporují `--node-reward-address` a `--node-reward-activation-height` (default `u64::MAX` = disabled). L2 multichain (`V31/L2/multichain/src/node_rewards.rs`) registruje nody, sbírá podepsané heartbeaty, score per epoch a každých `epoch_blocks=10080` blocks rozešle batch payout z poolu aktivním nodům přes `build_batch_payout`. HTTP API endpointy: `POST /v1/nodes/register` (vyžaduje ZIS auth), `POST /v1/nodes/heartbeat` (podepsaný payload), `GET /v1/nodes` a `GET /v1/nodes/payouts`. Desktop agent v `APP&WEB/desktop-agent` automaticky registruje lokální node a posílá heartbeaty po startu node, když je uživatel přihlášen přes ZIS. CLI příkazy pro headless nody: `zion node register` a `zion node heartbeat`. Signer mnemonic pro sweep payoutů se konfiguruje v `node_rewards.signer_mnemonic` (TOML) nebo env `ZION_NODE_REWARD_MNEMONIC`; pokud neodpovídá `reward_address`, sweep se vypne.
>
> **⚠️ 2026-08-22 ZIS DEPLOY + UTXO V2 WALLET/CLI/POOL (LATEST):** ZION Identity Service (`APP&WEB/identity/`) je nasazen na Edge jako `zion-zis.service` na `https://auth.zionterranova.com` (nginx proxy, port 8096). Build vyžaduje `@fastify/rate-limit ^9.1.0` pro Fastify 4 (`^10.x` je pro Fastify 5 a padá s `FST_ERR_PLUGIN_VERSION_MISMATCH`); systemd `ExecStart` musí být `/usr/bin/node /opt/zion/identity/server.js` (ne `dist/server.js`, protože tsc emituje do kořene projektu). UTXO v2 transakce (length-prefixed BLAKE3 hash s `ZION_TX_V2\0` domain tag) jsou kanonické pro V3 wallet/CLI/pool. Wallet SDK (`APP&WEB/zion-wallet-sdk/src/core/transaction.ts`) a CLI (`V31/cli/src/main.rs`) nyní používají `zion_core::v3_wallet::build_and_sign` a odesílají přes `submitUtxoTransaction`. Pool payouty (`V31/L1/pool/src/payout.rs` a `deferred_payout.rs`) používají `zion_core::v31_wallet` pro V31-native `crate::Transaction` a správně extrahují `result` z JSON-RPC response (`tx_id` je uvnitř `result`, ne top-level).
>
> **⚠️ 2026-08-17 LWMA + CPU-ONLY MINER FIX:** Nasazeny nové konstanty LWMA difficulty v `V31/L1/core/src/difficulty.rs` (`MIN_SOLVE_TIME=6`, `MAX_SOLVE_TIME=360`, ±50% per-block clamp) a nová binárka `zion-node` na Edge. Chain se zotavil z 6944–6945 stallu, height 7000+, difficulty kleslo z ~21k na ~16k a block time se ustálil kolem 60s. Zároveň opraven `zion-miner` enumerace GPU v `V31/L1/miner/src/gpu/mod.rs` — `ZION_GPU_BACKEND=cpu` nyní short-circuituje OpenCL/CUDA/Metal a miner běží stabilně na CPU-only Edge serveru. Aktuální stav: všechny V31 služby active, `zion-v31-miner` CPU-only běží s 100% ZION accept rate. Historické reporty: [`docs/3.1/REPORTS/README.md`](./docs/3.1/REPORTS/README.md).
>
> **⚠️ 2026-08-11 EDGE AUDIT & HARDENING:** Dokončen audit a opravy z [`REPORT_2026-08-10_EDGE_V31_AUDIT.md`](./docs/3.1/REPORTS/REPORT_2026-08-10_EDGE_V31_AUDIT.md). Nasazeny nové binárky `zion-node` (getStatus V31), `zion-pool` (HTTP API `127.0.0.1:8080`), `zion-dao` (`127.0.0.1:8456`), `zion-oasis` (fix parsingu `subsidy_zion`/`reward`). `zion-v31-multichain` restartován a active. `zion-v31-miner` zastaven a disabled na Edge (CPU-only server bez OpenCL/CUDA). Edge secrets zabezpečeny (`chmod 600`), DAO `api_key` přesunut z TOML do env, nginx `/api/dao` proxy opraven na port 8456, operátorské IP (`46.135.81.225`, `2a00:11b1:10e2:af49:b90b:20ed:4eee:b48b`) přidány do `fail2ban` a `ufw`. **ANKR API key** je legacy pro starý V3 bridge (`public/V3/L2/bridge/src/ankr.rs`) — `zion-v31-multichain` ho nepotřebuje, běží na veřejných RPC endpointech. Doporučeno odstranit `ANKR_API_KEY` z `/etc/zion/edge-environment.sh` a 1Password. Výjimka z instrukce: nebyly commitnuty necommitnuté změny na Edge (`/opt/zion`) — nasazeny jen nové binárky z lokálního V31 workspace.
>
> **⚠️ TRINITY MINING E2E 2026-08-10:** V31 Trinity Mining (ZION+ZANO+VRSC) je **PRODUCTION** na Edge pool. **2026-08-10 VRSC rejection fix + perf tuning:** dva bugy způsobovaly VRSC share reject (~86-97% accept rate): (1) race condition na sdíleném `ext_result_rx` kanálu — VRSC a ZANO výsledky se zaměňovaly; fix: per-coin kanály (`vrsc_result_rx`/`zano_result_rx`) + stale result draining v `V31/L1/miner/src/v3_pool_client.rs`. (2) `skip_stale` check `job_rx.has_changed()` se aktivoval při každém novém V3 job bundlu (i ZION-only) → validní VRSC share přeskočeny; fix: kontrola pouze když se změnil `cpu_external.job_id` v `V31/L1/miner/src/runtime.rs`. Performance tuning v `Start.sh`: `ZION_EXT_GPU_GAP_MS=0` (yield_now místo 50ms sleep — CUDA driver scheduling, +57% ZION, +13% ZANO), `ZION_NONCE_COUNT=10M`, `ZION_EXT_CPU_NONCE_COUNT=10M` (+12% VRSC). **Výsledek: 22-24 MH/s total (ZION 3.14 + ZANO 9.13 + VRSC 11.77), 100% accept rate napříč všemi 3 streamy.** `work_size=8192` testován ale horší (18-20 MH/s, VRAM bandwidth pressure) — 4096 je optimální pro 1070 Ti GPU sharing. GPU power limit 140W (throttle na 1493 MHz místo 1683 MHz) — pro increase na 170W potřebný sudo. Commity: `9cbaa6302` (VRSC fix), `4f6f8dd89` (perf tuning). Report: [`REPORT_2026-08-10_VRSC_REJECTION_FIX_AND_TUNING.md`](./docs/3.1/REPORTS/REPORT_2026-08-10_VRSC_REJECTION_FIX_AND_TUNING.md). **2026-08-10 deploy fix:** na Edge běžel stale `zion-pool` binár bez posledních `pop_job()` / `touch_job_timestamp` změn, takže se VRSC/ZANO share odmítaly upstream (`job not found`, `Duplicate share`, `Job expired`). Fresh build z `V31/`, redeploy a restart `zion-v31-pool`; upstream nyní acceptuje VRSC i ZANO share. AuxPoW worker name zjednodušen na čisté `zion-pool` bez coin suffixu (`a5b3aa2c6`). Místní miner běží přes `~/Desktop/Start.sh --bg` a připojil se k poolu. **ZANO duplicate-share fix 2026-08-10:** miner nyní resetuje Stream 2 (GPU external) nonce cursor na unikátní 64-bit base pro každý nový job (`wallet + worker + job_id + PID + counter` → blake3), takže více rigů na stejném upstream `eth_getWork` jobu nenachází stejné první nonce. Fix v `V31/L1/miner/src/runtime.rs`. Post-fix accept rate: ZION 100%, ZANO 100%, VRSC acceptováno upstream (~2 MH/s LuckPool worker `noname` z důvodu normalizace, ZANO ~13 MH/s HeroMiners worker `zion-pool`). Miner build: `ZION_CPU_TARGET=native cargo build --release -p zion-miner --bin zion-miner --features gpu-cuda,native-all,tui` (native-verushash REQUIRED pro VRSC; `ZION_CPU_TARGET=native` compiles VerusHash C++ with `-march=native` for max local hashrate). Pool build: `cargo build --release -p zion-pool`. Miner launcher: `~/Desktop/Start.sh` (TUI) nebo `~/Desktop/Start.sh --bg` (background). Systemd service: `~/.config/systemd/user/zion-gpu-miner.service`. CUDA je defaultní backend (`--gpu cuda`); OpenCL je možné přes `ZION_GPU_BACKEND=opencl`. **CUDA DAG disk cache 2026-08-10:** `V31/L1/miner/src/gpu/cuda_external.rs` persists generated DAGs to `~/.zion/dag-cache/{algo}_epoch{N}.bin` (override with `ZION_DAG_CACHE_DIR`; disable with `ZION_DAG_CACHE_DISABLE=1`) so subsequent starts load from disk instead of regenerating the ~2 GB ProgPoW DAG on GPU. Validated: first run generated and saved `progpow_epoch126.bin` (2.0 GB) in 2.7 s; restart loaded the cache and began mining ZANO immediately. Monitoring: ZANO 1 accepted / 0 rejected (high unique nonce `11049022021416437794`), no duplicate rejects. Report 2026-08-09: [`REPORT_2026-08-09_TRINITY_TUNING_FINAL.md`](./docs/3.1/REPORTS/REPORT_2026-08-09_TRINITY_TUNING_FINAL.md); aktuální stav viz [`StatusV3.md`](./StatusV3.md).
>
> **⚠️ V3.2 ONE LOVE GENESIS RESET 2026-08-06 (LATEST):** Kompletní rotace všech klíčů s BIP39 mnemonics — premine (14), canonical wallets (5), admin (3), DAO guardian (7), EVM validator (5) + escrow. Nové genesis hashe: **V3 compat `4cf7560f9140deb9376fa6567e76eacaa8bd1b733ca3c91b00830a08f332ef71`**, **V31 native `96109423298542a836edc10b9ba5ff9b29a1970418db543c2ee5cd952fe35bdb`**. Předchozí genesis hashe (`08a94fb0...` V3 compat, `21e2b274...` V31 native z first reset, `4f75a0df...` z 2026-07-20) jsou **historické**. Všechny adresy aktualizovány v kódu i na Edge serveru (config files + env files + systemd service files). V31 služby active: `zion-v31-node`, `zion-v31-node2`, `zion-v31-node3`, `zion-v31-pool`, `zion-v31-multichain`, `zion-v31-dao`, `zion-v31-oasis`, `zion-edge-python-dashboard`, `zion-website`, `zion-oasis-web`, `zion-marketplace`. `zion-v31-miner` je na Edge zastaven (CPU-only server, žádné OpenCL/CUDA zařízení). Watchdog fix: height=0 je validní pro fresh chain (nerestartuje node). Payout confirmation sweep s UTXO fallback je nasazen a potvrzuje výplaty on-chain. Miner fix: `--no-gpu --no-cpu` flagy odstraněny. Backup fix: V31 DBs (`/opt/zion/data/v31/`) přidány do `backup-edge.sh`. Klíče uloženy v `~/Desktop/ZION_KEYS_GENESIS_V2_2026-08-06/` (chmod 700, owner-only). **Všech 35 keypairs má BIP39 24-word mnemonics** — klíče generovány přes `gen-all-keys-mnemonic.rs`. L1 wallet CLI používá Ed25519 SK přímo (mnemonic nepotřebuje). Multi-Chain SDK (`WalletClient::from_mnemonic()`) nyní funguje se všemi klíči. Kompletní procedura: [`HARD_RESET_PLAYBOOK.md`](docs/3.2/HARD_RESET_PLAYBOOK.md). Adresní mapping (old → new) viz playbook appendix.
>
...
- **V3.1 / V31 status (updated 2026-08-10):** `V31/` workspace (`3.1.0-beta`, protocol `3.1.0-alpha`) is **PRODUCTION** — V31 cutover complete. `cargo test --workspace` passes (2178 tests, 0 failures), `cargo clippy --workspace` clean (pre-existing warnings only). **Phase B+C+S COMPLETE**, **D.2 Cutover COMPLETE**, **D.3 Post-cutover D3.1-D3.4 PASS**. V31 node (`zion-v31-node.service`, P2P 8335, RPC 9445) is production — public RPC 8443 → V31, chain height 1000+. V31 pool (`zion-v31-pool.service`, Stratum 8444, HTTP API 8080) is production — shares accepted, payout confirmation sweep active; **2026-08-10: Edge pool binár rebuildnut a redeploynut s aktuálními `auxpow_bridge`/`auxpow_runtime` fixy, VRSC/ZANO share nyní acceptovány upstream (`a5b3aa2c6`). V31 multichain (`zion-v31-multichain.service`, port 8453/8454) — /health 200 OK. V31 DAO (`zion-v31-dao.service`, port 8456) and V31 OASIS (`zion-v31-oasis.service`, port 8094) are active. V31 miner binary (stratum v1 client) mining live. V3 pool/node disabled. Tag `pre-v31-cutover` marks last V3-production commit. `V3/`, `AuXpow/`, `ZionDex/` archived to `archive/`. **L2:** bridge/warp/atomic-swap/ziondex/swap-aggregator unified into **Multi-Chain** crate (`V31/L2/multichain`). See [`PLAN_TO_3.1.md`](./docs/3.1/PLAN_TO_3.1.md), [`V3.1_MIGRATION_PLAN.md`](./docs/3.0.6/V3.1_MIGRATION_PLAN.md) and [`V3.1_INTEGRATION_PLAN.md`](./docs/3.0.6/V3.1_INTEGRATION_PLAN.md).
...
- **Edge IPv6 + fail2ban lockout (2026-08-17):** Edge IPv6 je funkční fallback; při rychlých SSH auditech může `fail2ban` `sshd` jail zabanovat operátorskou IPv4/IPv6, pokud není v `ignoreip`. Aktuální operátorská IP byly přidány do `V31/AGENTS.md` `OPERATOR_IPS`. `zion-v31-multichain` je active; `zion-v31-miner` běží v CPU-only režimu po fixu enumerace GPU — viz aktuální `StatusV3.md`.
...
**Premine Distribution (16.78B total):**
- ZION OASIS (Slots 1-3): 1.65B each → 4.95B total (Slots 4 & 5 repurposed to L5 Free World Projects — 3.3B total)
- L6 Issobella — Orbital Station & Quantum Research (Slot 6): 2.5B (repurposed from DAO Treasury; LOCKED height 144,000)
- DAO Treasury - Grants (Slot 7): 1B (LOCKED height 144,000)
- DAO Treasury - Bootstrap (Slot 8): 0.5B (LOCKED height 144,000)
