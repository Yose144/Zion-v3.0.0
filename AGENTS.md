...
> **⚠️ TRINITY MINING E2E 2026-08-09 (LATEST):** V31 Trinity Mining (ZION+ZANO+VRSC) je **PRODUCTION** na Edge pool s celkovým accept rate **96.7%**. Kritický bug fix: `pop_job()` v `V31/L1/pool/src/auxpow_bridge.rs` vracel nejstarší job z `VecDeque` fronty místo nejnovějšího → VRSC "ntime out of range" a ZANO "Duplicate share" rejects. Fix: `pop_job()` nyní vrací `q.back()`, VRSC latest-only check používá `job_ids.last()`. VRSC share difficulty override na 180M (pool-side, `ZION_VRSC_MIN_DIFF=180000000` v `build_external_stream_cpu()`) + 10 CPU threadů (`ZION_EXT_CPU_THREADS=10`). Dashboard Trinity Mining panel opraven (auth-exempt routes + hashrate format). Stale thresholds: VRSC=50s, ZANO=28s. Miner build: `cargo build --release --bin zion-miner --features gpu-cuda,native-all,tui` (native-verushash REQUIRED pro VRSC). Pool build: `cargo build --release -p zion-pool`. Miner launcher: `~/Desktop/Start.sh` (TUI) nebo `~/Desktop/Start.sh --bg` (background). Systemd service: `~/.config/systemd/user/zion-gpu-miner.service`. Kompletní report: [`REPORT_2026-08-09_TRINITY_TUNING_FINAL.md`](./REPORT_2026-08-09_TRINITY_TUNING_FINAL.md).
>
