...
> **⚠️ TRINITY MINING E2E 2026-08-10 (LATEST):** V31 Trinity Mining (ZION+ZANO+VRSC) je **PRODUCTION** na Edge pool. **2026-08-10 deploy fix:** na Edge běžel stale `zion-pool` binár bez posledních `pop_job()` / `touch_job_timestamp` změn, takže se VRSC/ZANO share odmítaly upstream (`job not found`, `Duplicate share`, `Job expired`). Fresh build z `V31/`, redeploy a restart `zion-v31-pool`; upstream nyní acceptuje VRSC i ZANO share. AuxPoW worker name zjednodušen na čisté `zion-pool` bez coin suffixu (`a5b3aa2c6`). Místní miner běží přes `~/Desktop/Start.sh --bg` a připojil se k poolu. **ZANO duplicate-share fix 2026-08-10:** miner nyní resetuje Stream 2 (GPU external) nonce cursor na unikátní 64-bit base pro každý nový job (`wallet + worker + job_id + PID + counter` → blake3), takže více rigů na stejném upstream `eth_getWork` jobu nenachází stejné první nonce. Fix v `V31/L1/miner/src/runtime.rs`. Post-fix accept rate: ZION 100%, ZANO 100%, VRSC acceptováno upstream (~2 MH/s LuckPool worker `noname` z důvodu normalizace, ZANO ~13 MH/s HeroMiners worker `zion-pool`). Miner build: `cargo build --release -p zion-miner --bin zion-miner --features gpu-cuda,native-all,tui` (native-verushash REQUIRED pro VRSC). Pool build: `cargo build --release -p zion-pool`. Miner launcher: `~/Desktop/Start.sh` (TUI) nebo `~/Desktop/Start.sh --bg` (background). Systemd service: `~/.config/systemd/user/zion-gpu-miner.service`. CUDA je defaultní backend (`--gpu cuda`); OpenCL je možné přes `ZION_GPU_BACKEND=opencl`. DAG se pro CUDA stále generuje na GPU při každém startu. Report 2026-08-09: [`REPORT_2026-08-09_TRINITY_TUNING_FINAL.md`](./REPORT_2026-08-09_TRINITY_TUNING_FINAL.md); aktuální stav viz [`StatusV3.md`](./StatusV3.md).
>
