...
- Miner (connects to new server pool):
  ...
  - **ETC/Ethash CPU reference fix (2026-07-27):** The pure-Rust `hash_ethash` CPU audit path in `AuXpow/src/external_hashers.rs` was replaced with the `ethash` 0.4 reference crate for cache building and hashimoto, aligning it with chfast test vectors and the CUDA kernel. `zion-miner --test-cuda-kernel ethash` now reports `ETHASH_CPU_GPU_MATCH` (~121 MH/s on GTX 1070 Ti, epoch 1). Previously the hand-rolled Rust implementation produced a different final hash, causing `ETHASH_CPU_GPU_MISMATCH`.
