...
- **V3.1 migration is planned (starts after 3.0.9):** clean code is migrated into a brand-new `V31/` directory on a dedicated branch (NOT an in-place reorg of `V3/`). Target: WARP `V3/L3/warp/` → `V31/L2/warp/`, ZionDex `ZionDex/` → `V31/L2/ziondex/`, AuxPoW (root `AuXpow/`) merges into `V31/L1/miner/`, `L3` = AI/orchestration/automation/NCL/PoC, and **L4 Oasis / L5 Free World / L6 Issobella stay as their own superstructure layers**. See [`V3.1_MIGRATION_PLAN.md`](./V3.1_MIGRATION_PLAN.md). Until migration/cutover, keep using current `V3/`, `AuXpow/`, and `ZionDex/` paths.
