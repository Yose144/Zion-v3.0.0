...
| 3 | **DAO** | C1.4 `gen-dao-guardians` ported; D3.4 multichain `/health` OK | `zion-dao` je skeleton — žádné voting, treasury, humanitarian, L1 scanner, HTTP API | `zion-dao` má všechny V3 moduly (voting, quorum, timelock, treasury, humanitarian, db, l1_scanner, metrics, executor, consent, co_admin, cross_layer, prizes). `main.rs` spouští L1 scanner a `/metrics`. Zbývá plné propojení runtime ↔ db/scanner. |
