...
| 3 | **DAO** | C1.4 `gen-dao-guardians` ported; D3.4 multichain `/health` OK | `zion-dao` je skeleton — žádné voting, treasury, humanitarian, L1 scanner, HTTP API | `zion-dao` má všechny V3 moduly a runtime je napojený na `DaoDb`. `main.rs` otevírá DB, spouští L1 scanner, načítá existující návrhy/hlasy do runtime a persistuje nové. Zbývá plné propojení scanneru s runtime (nyní scanner zapisuje jen do DB). |
