...
#include "ZionOasis/Guild/GuildTypes.h"
...
public:
	...
	void JoinGuild(const FString& GuildId);

	UFUNCTION(BlueprintCallable, Category = "Guild")
	void CreateGuild(const FString& GuildName, EGuildOrder Order);
...
private:
	...
	UFUNCTION()
	void OnJoinGuildResponse(const FString& JsonBody, bool bSuccess);

	UFUNCTION()
	void OnCreateGuildResponse(const FString& JsonBody, bool bSuccess);

	UFUNCTION()
	void OnRefreshGuildResponse(const FString& JsonBody, bool bSuccess);
};
