...
void UConsciousnessComponent::SyncFromBackend(const FString& WalletAddress)
{
	if (UZionBlockchainBridge* Bridge = UZionBlockchainBridge::Get(GetWorld()))
	{
		...
		Callback.BindDynamic(this, &UConsciousnessComponent::OnSyncPlayerData);
		Bridge->GetPlayer(WalletAddress, Callback);
	}
}

void UConsciousnessComponent::OnSyncPlayerData(const FString& JsonBody, bool bSuccess)
{
	if (bSuccess) ApplyStateFromJson(JsonBody);
}

void UConsciousnessComponent::ApplyStateFromJson(const FString& JsonPayload)
{
	// Parse "total_xp" and "level" from REST JSON response
...
{
	// Walk from highest to lowest — mirrors consciousness.rs from_xp()
	for (int32 i = 8; i >= 0; i--)
	{
