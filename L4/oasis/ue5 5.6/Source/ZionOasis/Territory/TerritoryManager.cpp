...
#include "ZionOasis/Territory/TerritoryManager.h"
...
void ATerritoryManager::SyncFromBackend()
{
	...
	Callback.BindDynamic(this, &ATerritoryManager::OnTerritoryMapResponse);
	Bridge->GetTerritoryMap(Callback);
}

void ATerritoryManager::OnTerritoryMapResponse(const FString& Json, bool bSuccess)
{
	if (!bSuccess)
	{
...
ATerritoryManager* ATerritoryManager::Get(UWorld* World)
{
	...
	TActorIterator<ATerritoryManager> It(World);
