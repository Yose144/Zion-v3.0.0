...
void AZionPlayerController::ServerInteract_Implementation()
{
	...
	{
		// Delegate actual interaction to the hit actor's Blueprint event
		// (AAvatarNPC, ATerritoryMarker, etc. can respond via IZionInteractable)
		UE_LOG(LogTemp, Log, TEXT("[ZionPlayerController] Interact hit: %s"), *Hit.GetActor()->GetName());
	}
}

void AZionPlayerController::OpenAvatarQuestUI_Implementation(EAvatarID AvatarID)
{
...
void AZionPlayerController::SyncPlayerFromBackend()
{
	...
	Bridge->GetPlayer(ConnectedWallet, [this](bool bSuccess, const FString& Json)
	{
		if (!bSuccess)
		{
