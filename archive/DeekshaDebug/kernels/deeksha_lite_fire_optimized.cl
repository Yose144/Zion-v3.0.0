/*
 * DeekshaLite Fire Optimized — OpenCL Kernel (Thermal-Intensive, Winter Mode)
 *
 * Pipeline (must match CPU deeksha_lite_fire_optimized.rs exactly):
 ...
 *        Phase C: 32 random reads (reduced, focus on thermal loop)
 *   3. AES-128 CTR mix: 2 full rounds + 1 final round (AES_ROUNDS=3)
 *        key=s2[0..16], counter=nonce_le||s2[16..24]
 *        XOR result with s2[0..32]
 *   4. Thermal loop: 131072 iterations, 12 ulong chains
 *        — high ALU utilization, memory-dependent to prevent dead-code elim
 *   5. Keccak256(s3)  → final hash[32]
 *
 * Constants (must match deeksha_lite_fire_optimized.rs):
 *   SCRATCHPAD_SIZE = 128 KiB
 *   BLOCK_COUNT     = 4096
 *   RANDOM_READS    = 32
 *   AES_ROUNDS      = 3  (2 full + 1 final)
 *   THERMAL_ITERS   = 131072
 */
...
#define AES_FULL_ROUNDS  2
#define THERMAL_ITERS    131072

/* ========================================================================== */
/* Keccak-f[1600] — identical to deeksha_lite_optimized.cl                    */
/* ========================================================================== */
...
void aes_add_round_key(__private uchar s[16],__private const uchar k[16]) { for(int i=0;i<16;i++) s[i]^=k[i]; }
...
void fill_scratchpad(__private const uchar seed[32], __global uchar *pad)
{
    ...
    for (uint blk=0;blk<BLOCK_COUNT;blk++) {
        ...
        for (int i=0;i<32;i++) { pad[off+i]=out64[i]; state[i]=out64[i]; }
    }
}

/* ========================================================================== */
/* Step 2B: Sequential XOR passes                                              */
/* ========================================================================== */
...
void sequential_passes(__global uchar *pad)
{
    for (uint i=0;i<BLOCK_COUNT;i++) {
        ...
        uint cur=i*BLOCK_SIZE, prv=prev*BLOCK_SIZE;
        for (int j=0;j<BLOCK_SIZE;j++) pad[cur+j]^=pad[prv+j];
    }
    for (uint i=BLOCK_COUNT;i>0;i--) {
        uint idx=i-1;
        uint next=(idx+1==BLOCK_COUNT)?0:(idx+1);
        uint cur=idx*BLOCK_SIZE, nxt=next*BLOCK_SIZE;
        for (int j=0;j<BLOCK_SIZE;j++) pad[cur+j]^=pad[nxt+j];
    }
}

/* ========================================================================== */
/* Step 2C: Random read mix — 32 reads                                        */
/* ========================================================================== */
...
void aes128_mix_fire(__private const uchar seed[32], ulong nonce, __private uchar out[32])
{
    ...
    for (int r=0;r<AES_FULL_ROUNDS;r++) { aes_round(block0,key); aes_round(block1,key); }
...
/*                                                                             */
/* Matches CPU deeksha_lite_fire_optimized.rs step4_thermal_loop_enhanced()   */
...
void thermal_loop(__private uchar data[32], ulong nonce)
{
    ...
    for (uint iter=0; iter<THERMAL_ITERS; iter++) {
        ...
        /* Memory-dependent XOR — matches CPU data[iter & 0x1F] indexing */
        uint di = iter & 0x1Fu;
        a ^= (ulong)data[di];
        ...
        /* Additional rotations for ALU saturation */
        a = (a<<13)|(a>>51);
        b = (b<<23)|(b>>41);
        c = (c<<31)|(c>>33);
        d = (d<< 7)|(d>>57);
    ...
    /* Fold-back — matches CPU exactly
     * Rust a.rotate_right(17) == OpenCL rotate(a, 64-17) == rotate(a, 47)
     * Rust a.rotate_right(31) == OpenCL rotate(a, 64-31) == rotate(a, 33)
     * Rust c.rotate_right(13) == OpenCL rotate(c, 64-13) == rotate(c, 51)
     * Rust d.rotate_right(47) == OpenCL rotate(d, 64-47) == rotate(d, 17)
     * Rust e.rotate_right(23) == OpenCL rotate(e, 64-23) == rotate(e, 41)
     * Rust f.rotate_right(41) == OpenCL rotate(f, 64-41) == rotate(f, 23)
     */
    for (int i=0;i<16;i++) {
        data[i*2]  ^= (uchar)a;
...
/* ========================================================================== */
/* Main kernel — DeekshaLite Fire Optimized (Thermal, Winter Mode)            */
/* ========================================================================== */

__kernel void deeksha_lite_fire_optimized_mine(
