#ifndef GHOSTRIDER_CN_CL
#define GHOSTRIDER_CN_CL
...
// =============================================================================
// Section 1: AES tables and functions
// =============================================================================

static const __constant uchar cn_sbox[256] = {
...
static const __constant uchar cn_rcon[8] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40 };

static const __constant uint CN_AES0_C[256] = {
...
#define CN_BYTE(x, y) (((x) >> ((y) * 8U)) & 0xFFU)
#define CN_SubWord(inw) ((cn_sbox[CN_BYTE(inw, 3)] << 24) | (cn_sbox[CN_BYTE(inw, 2)] << 16) | (cn_sbox[CN_BYTE(inw, 1)] << 8) | cn_sbox[CN_BYTE(inw, 0)])

inline void AESExpandKey256(__private uint *keybuf)
{
    for (uint c = 8, i = 1; c < 40; ++c)
    {
        uint t = ((!(c & 7)) || ((c & 7) == 4)) ? CN_SubWord(keybuf[c - 1]) : keybuf[c - 1];
        keybuf[c] = keybuf[c - 8] ^ ((!(c & 7)) ? rotate(t, 24U) ^ as_uint((uchar4)(cn_rcon[i++], 0U, 0U, 0U)) : t);
    }
}

inline uint4 AES_Round(__local const uint *AES0, __local const uint *AES1,
                       __local const uint *AES2, __local const uint *AES3,
                       const uint4 X, uint4 key)
{
    key.s0 ^= AES0[CN_BYTE(X.s0, 0)] ^ AES1[CN_BYTE(X.s1, 1)] ^ AES2[CN_BYTE(X.s2, 2)] ^ AES3[CN_BYTE(X.s3, 3)];
    key.s1 ^= AES0[CN_BYTE(X.s1, 0)] ^ AES1[CN_BYTE(X.s2, 1)] ^ AES2[CN_BYTE(X.s3, 2)] ^ AES3[CN_BYTE(X.s0, 3)];
    key.s2 ^= AES0[CN_BYTE(X.s2, 0)] ^ AES1[CN_BYTE(X.s3, 1)] ^ AES2[CN_BYTE(X.s0, 2)] ^ AES3[CN_BYTE(X.s1, 3)];
    key.s3 ^= AES0[CN_BYTE(X.s3, 0)] ^ AES1[CN_BYTE(X.s0, 1)] ^ AES2[CN_BYTE(X.s1, 2)] ^ AES3[CN_BYTE(X.s2, 3)];
    return key;
}
                                   ...
                                   __local uint *AES2, __local uint *AES3)
{
    ...
    for (uint i = lid; i < 256; i += lws) {
        uint tmp = CN_AES0_C[i];
        AES0[i] = tmp;
...
// =============================================================================
// Section 2: Keccak-1600 (200-byte output, rate=136)
// =============================================================================
...
inline void keccak1600(__private const uchar *in, int inlen, __private uchar *md)
{
    ...
    while (inlen >= rsiz) {
        for (int i = 0; i < rsizw; ++i)
            st[i] ^= ((__private const ulong*)(in + offset))[i];
        keccakf1600(st);
        inlen -= rsiz;
        offset += rsiz;
    }

    __private uchar temp[136];
    for (int i = 0; i < inlen; ++i) temp[i] = in[offset + i];
    temp[inlen++] = 1;
    for (int i = inlen; i < rsiz; ++i) temp[i] = 0;
    temp[rsiz - 1] |= 0x80;

    for (int i = 0; i < rsizw; ++i)
        st[i] ^= ((__private const ulong*)temp)[i];

    keccakf1600(st);

    for (int i = 0; i < 25; ++i)
        ((__private ulong*)md)[i] = st[i];
}

// =============================================================================
// Section 3a: BLAKE-256 extra hash
// =============================================================================

static const __constant int blake_sigma[16][16] = {
...
static const __constant uint c_IV256[8] = {
...
static const __constant uint c_u256[16] = {
...
#define BLAKE_SWAP4(x) as_uint(as_uchar4(x).s3210)

#define BLAKE_GS(a,b,c,d,x) { \
    v[a] += (m[blake_sigma[r][x]] ^ c_u256[blake_sigma[r][x+1]]) + v[b]; \
    v[d] ^= v[a]; \
    v[d] = rotate(v[d], 16U); \
    v[c] += v[d]; \
    v[b] ^= v[c]; \
    v[b] = rotate(v[b], 20U); \
    v[a] += (m[blake_sigma[r][x+1]] ^ c_u256[blake_sigma[r][x]]) + v[b]; \
    v[d] ^= v[a]; \
    v[d] = rotate(v[d], 24U); \
    v[c] += v[d]; \
    v[b] ^= v[c]; \
...
void blake256_hash(__private uchar *output, __private const uchar *input, int len)
{
    ...
    for (uint i = 0; i < 3; ++i) {
        for (int x = 0; x < 16; ++x) {
            m[x] = ((__private const uint*)input)[i * 16 + x];
            m[x] = BLAKE_SWAP4(m[x]);
        }
        bitlen += 512;

        for (int x = 0; x < 8; ++x) v[x] = h[x];
        for (int x = 0; x < 8; ++x) v[8 + x] = c_u256[x];
        v[12] ^= bitlen;
        v[13] ^= bitlen;

        for (uint r = 0; r < 14; r++) {
            BLAKE_GS(0, 4, 0x8, 0xC, 0x0);
            BLAKE_GS(1, 5, 0x9, 0xD, 0x2);
            BLAKE_GS(2, 6, 0xA, 0xE, 0x4);
            BLAKE_GS(3, 7, 0xB, 0xF, 0x6);
            BLAKE_GS(0, 5, 0xA, 0xF, 0x8);
            BLAKE_GS(1, 6, 0xB, 0xC, 0xA);
            BLAKE_GS(2, 7, 0x8, 0xD, 0xC);
            BLAKE_GS(3, 4, 0x9, 0xE, 0xE);
        }

        for (int x = 0; x < 8; ++x) h[x] ^= v[x] ^ v[8 + x];
    }

    m[0]  = BLAKE_SWAP4(((__private const uint*)input)[48]);
    m[1]  = BLAKE_SWAP4(((__private const uint*)input)[49]);
    m[2]  = 0x80000000U;
    m[3]  = 0x00U;
    m[4]  = 0x00U;
    m[5]  = 0x00U;
    ...
    for (int i = 0; i < 8; ++i) h[i] = BLAKE_SWAP4(h[i]);
    for (int i = 0; i < 32; ++i) output[i] = ((__private uchar*)h)[i];
}
...
// =============================================================================
// Section 3b: Groestl-256 extra hash
// =============================================================================
...
#define GROESTL_B64_0(x)    ((x) & 0xFF)
#define GROESTL_B64_1(x)    (((x) >> 8) & 0xFF)
#define GROESTL_B64_2(x)    (((x) >> 16) & 0xFF)
#define GROESTL_B64_3(x)    (((x) >> 24) & 0xFF)
#define GROESTL_B64_4(x)    (((x) >> 32) & 0xFF)
#define GROESTL_B64_5(x)    (((x) >> 40) & 0xFF)
#define GROESTL_B64_6(x)    (((x) >> 48) & 0xFF)
#define GROESTL_B64_7(x)    ((x) >> 56)
...
#define GROESTL_ROUND_SMALL_P(a, r) do { \
    ...
    a[1] ^= GROESTL_PC64(0x10, r); \
    a[2] ^= GROESTL_PC64(0x20, r); \
    a[3] ^= GROESTL_PC64(0x30, r); \
    a[4] ^= GROESTL_PC64(0x40, r); \
...
#define GROESTL_ROUND_SMALL_Q(a, r) do { \
    ulong t[8]; \
    a[0] ^= GROESTL_QC64(0x00, r); \
    a[1] ^= GROESTL_QC64(0x10, r); \
    a[2] ^= GROESTL_QC64(0x20, r); \
    a[3] ^= GROESTL_QC64(0x30, r); \
    a[4] ^= GROESTL_QC64(0x40, r); \
...
#define GROESTL_PERM_SMALL_P(a) do { \
    for (int r = 0; r < 10; r++) GROESTL_ROUND_SMALL_P(a, r); \
} while (0)

#define GROESTL_PERM_SMALL_Q(a) do { \
...
void groestl256_hash(__private uchar *output, __private const uchar *input, int len)
{
    ...
    for (uint i = 0; i < 3; ++i) {
        for (int x = 0; x < 8; ++x)
            M[x] = ((__private const ulong*)input)[i * 8 + x];
        for (int x = 0; x < 8; ++x)
            H[x] = M[x] ^ State[x];
        GROESTL_PERM_SMALL_P(H);
        GROESTL_PERM_SMALL_Q(M);
        for (int x = 0; x < 8; ++x)
            State[x] ^= H[x] ^ M[x];
    }

    M[0] = ((__private const ulong*)input)[24];
    M[1] = 0x80UL;
    M[2] = 0UL;
    M[3] = 0UL;
    M[4] = 0UL;
    ...
    __private ulong tmp[8];
    for (int i = 0; i < 8; ++i)
        tmp[i] = State[i] ^= H[i] ^ M[i];

    GROESTL_PERM_SMALL_P(State);
...
#undef GROESTL_PERM_SMALL_Q

// =============================================================================
// Section 3c: JH-256 extra hash
// =============================================================================

static const __constant ulong JH_C[] = {
...
#define JH_Ceven_hi(r)   (JH_C[((r) << 2) + 0])
#define JH_Ceven_lo(r)   (JH_C[((r) << 2) + 1])
#define JH_Codd_hi(r)    (JH_C[((r) << 2) + 2])
#define JH_Codd_lo(r)    (JH_C[((r) << 2) + 3])

#define JH_Sb(x0, x1, x2, x3, c)   do { \
    x3 = ~x3; \
    x0 ^= (c) & ~x2; \
    tmp = (c) ^ (x0 & x1); \
    x0 ^= x2 & x3; \
...
#define JH_Lb(x0, x1, x2, x3, x4, x5, x6, x7)   do { \
    x4 ^= x1; \
    x5 ^= x2; \
    x6 ^= x3 ^ x0; \
    x7 ^= x0; \
...
#define JH_S(x0, x1, x2, x3, cb, r)   do { \
    JH_Sb(x0 ## h, x1 ## h, x2 ## h, x3 ## h, cb ## hi(r)); \
    JH_Sb(x0 ## l, x1 ## l, x2 ## l, x3 ## l, cb ## lo(r)); \
} while (0)

#define JH_L(x0, x1, x2, x3, x4, x5, x6, x7)   do { \
    JH_Lb(x0 ## h, x1 ## h, x2 ## h, x3 ## h, x4 ## h, x5 ## h, x6 ## h, x7 ## h); \
    JH_Lb(x0 ## l, x1 ## l, x2 ## l, x3 ## l, x4 ## l, x5 ## l, x6 ## l, x7 ## l); \
} while (0)

#define JH_Wz(x, c, n)   do { \
    ulong t = (x ## h & (c)) << (n); \
    x ## h = ((x ## h >> (n)) & (c)) | t; \
    t = (x ## l & (c)) << (n); \
    x ## l = ((x ## l >> (n)) & (c)) | t; \
...
#define JH_W0(x)   JH_Wz(x, SPH_C64(0x5555555555555555),  1)
#define JH_W1(x)   JH_Wz(x, SPH_C64(0x3333333333333333),  2)
#define JH_W2(x)   JH_Wz(x, SPH_C64(0x0F0F0F0F0F0F0F0F),  4)
#define JH_W3(x)   JH_Wz(x, SPH_C64(0x00FF00FF00FF00FF),  8)
#define JH_W4(x)   JH_Wz(x, SPH_C64(0x0000FFFF0000FFFF), 16)
#define JH_W5(x)   JH_Wz(x, SPH_C64(0x00000000FFFFFFFF), 32)
#define JH_W6(x)   do { ulong t = x ## h; x ## h = x ## l; x ## l = t; } while (0)

#define JH_SLu(r, ro)   do { \
    JH_S(h0, h2, h4, h6, JH_Ceven_, r); \
    JH_S(h1, h3, h5, h7, JH_Codd_, r); \
    JH_L(h0, h2, h4, h6, h1, h3, h5, h7); \
    JH_W ## ro(h1); \
    JH_W ## ro(h3); \
    JH_W ## ro(h5); \
    JH_W ## ro(h7); \
...
#define JH_E8   do { \
    JH_SLu( 0, 0); JH_SLu( 1, 1); JH_SLu( 2, 2); JH_SLu( 3, 3); \
    JH_SLu( 4, 4); JH_SLu( 5, 5); JH_SLu( 6, 6); JH_SLu( 7, 0); \
    JH_SLu( 8, 1); JH_SLu( 9, 2); JH_SLu(10, 3); JH_SLu(11, 4); \
    JH_SLu(12, 5); JH_SLu(13, 6); JH_SLu(14, 0); JH_SLu(15, 1); \
    JH_SLu(16, 2); JH_SLu(17, 3); JH_SLu(18, 4); JH_SLu(19, 5); \
    JH_SLu(20, 6); JH_SLu(21, 0); JH_SLu(22, 1); JH_SLu(23, 2); \
    JH_SLu(24, 3); JH_SLu(25, 4); JH_SLu(26, 5); JH_SLu(27, 6); \
    JH_SLu(28, 0); JH_SLu(29, 1); JH_SLu(30, 2); JH_SLu(31, 3); \
    JH_SLu(32, 4); JH_SLu(33, 5); JH_SLu(34, 6); JH_SLu(35, 0); \
    JH_SLu(36, 1); JH_SLu(37, 2); JH_SLu(38, 3); JH_SLu(39, 4); \
    JH_SLu(40, 5); JH_SLu(41, 6); \
} while (0)

#define JH_XOR(input) do { \
    h0h ^= input[0]; h0l ^= input[1]; \
    h1h ^= input[2]; h1l ^= input[3]; \
    h2h ^= input[4]; h2l ^= input[5]; \
    h3h ^= input[6]; h3l ^= input[7]; \
    JH_E8; \
    h4h ^= input[0]; h4l ^= input[1]; \
    h5h ^= input[2]; h5l ^= input[3]; \
    h6h ^= input[4]; h6l ^= input[5]; \
...
void jh256_hash(__private uchar *output, __private const uchar *input, int len)
{
    ...
    ulong h2h = 0x45B92677269E23A4UL, h2l = 0x77941AD4481AFBE0UL;
    ulong h3h = 0x7A176B0226ABB5CDUL, h3l = 0xA82FFF0F4224F056UL;
    ulong h4h = 0x754D2E7F8996A371UL, h4l = 0x62E27DF70849141DUL;
    ulong h5h = 0x948F2476F7957627UL, h5l = 0x6C29804757B6D587UL;
    ulong h6h = 0x6C0D8EAC2D275E5CUL, h6l = 0x0F7A0557C6508451UL;
    ulong h7h = 0xEA12247067D3E47BUL, h7l = 0x69D71CD313ABE389UL;
    ulong tmp;

    for (uint i = 0; i < 3; ++i) {
        ulong in[8];
        for (uint x = 0; x < 8; ++x)
            in[x] = ((__private const ulong*)input)[i * 8 + x];
        JH_XOR(in);
    }

    {
        ulong in[8] = { ((__private const ulong*)input)[24], 0x80UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL };
        JH_XOR(in);
    }

    {
        ulong in[8] = { 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x00UL, 0x4006000000000000UL };
        JH_XOR(in);
    }
...
// =============================================================================
// Section 3d: Skein-512-256 extra hash
// =============================================================================
...
inline ulong8 SkeinMix8(ulong8 pv, const ulong4 pv1, const uint rc0, const uint rc1, const uint rc2, const uint rc3)
{
    pv.even += pv.odd;
    pv.odd.s0 = rotate(pv.odd.s0, (ulong)rc0);
    pv.odd.s1 = rotate(pv.odd.s1, (ulong)rc1);
    pv.odd.s2 = rotate(pv.odd.s2, (ulong)rc2);
    pv.odd.s3 = rotate(pv.odd.s3, (ulong)rc3);
    pv.odd ^= pv.even;
    return pv;
}

inline ulong8 SkeinEvenRound(ulong8 p, const ulong8 h, const ulong *t, const uint s)
{
    SKEIN_INJECT_KEY(p, s);
    ulong4 pv0 = p.even, pv1 = p.odd;

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 46, 36, 19, 37).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 46, 36, 19, 37).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 33, 27, 14, 42).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 33, 27, 14, 42).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 17, 49, 36, 39).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 17, 49, 36, 39).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 44, 9, 54, 56).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 44, 9, 54, 56).odd;
    return shuffle2(pv0, pv1, (ulong8)(1, 4, 2, 7, 3, 6, 0, 5));
}

inline ulong8 SkeinOddRound(ulong8 p, const ulong8 h, const ulong *t, const uint s)
{
    SKEIN_INJECT_KEY(p, s);
    ulong4 pv0 = p.even, pv1 = p.odd;

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 39, 30, 34, 24).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 39, 30, 34, 24).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 13, 50, 10, 17).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 13, 50, 10, 17).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 25, 29, 39, 43).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 25, 29, 39, 43).odd;
    pv0 = shuffle(pv0, (ulong4)(1, 2, 3, 0));
    pv1 = shuffle(pv1, (ulong4)(0, 3, 2, 1));

    pv0 = SkeinMix8((ulong8)(pv0, pv1), pv1, 8, 35, 56, 22).even;
    pv1 = SkeinMix8((ulong8)(pv0, pv1), pv1, 8, 35, 56, 22).odd;
    return shuffle2(pv0, pv1, (ulong8)(1, 4, 2, 7, 3, 6, 0, 5));
}

inline ulong8 Skein512Block(ulong8 p, ulong8 h, ulong h8, const ulong *t)
{
    for (int i = 0; i < 18; ++i) {
...
#undef SKEIN_INJECT_KEY

// =============================================================================
// Section 4: CN helper functions
// =============================================================================

#define CN_AES_BLOCK_SIZE  16
#define CN_INIT_SIZE_BYTE  128

inline ulong cn_mul128(ulong a, ulong b, __private ulong *hi)
{
    *hi = mul_hi(a, b);
    return a * b;
}

inline uint cn_e2i(__private const uchar* a, uint cn_aes_init)
{
    return (uint)((*((__private const ulong*)a) / CN_AES_BLOCK_SIZE) & (cn_aes_init - 1));
}

inline void cn_copy_block(__private uchar* dst, __private const uchar* src)
{
    ((__private ulong*)dst)[0] = ((__private const ulong*)src)[0];
    ((__private ulong*)dst)[1] = ((__private const ulong*)src)[1];
}

inline void cn_xor_blocks(__private uchar* a, __private const uchar* b)
{
    ((__private ulong*)a)[0] ^= ((__private const ulong*)b)[0];
    ((__private ulong*)a)[1] ^= ((__private const ulong*)b)[1];
}

// XOR two private blocks, store result to global memory
inline void cn_xor_blocks_dst_g(__private const uchar* a, __private const uchar* b, __global uchar* dst)
{
    ((__global ulong*)dst)[0] = ((__private const ulong*)a)[0] ^ ((__private const ulong*)b)[0];
    ((__global ulong*)dst)[1] = ((__private const ulong*)a)[1] ^ ((__private const ulong*)b)[1];
}

inline void cn_sum_half_blocks(__private uchar* a, __private const uchar* b)
{
    ((__private ulong*)a)[0] += ((__private const ulong*)b)[0];
    ((__private ulong*)a)[1] += ((__private const ulong*)b)[1];
}
    ...
    __global const uchar *global_input)
{
    ...
    // 3. AES key expansion (256-bit key → 40 uint expanded key)
    __private uint expanded_key[40];
    ((__private uint8*)expanded_key)[0] = ((__private uint8*)aes_key)[0];
    AESExpandKey256(expanded_key);
    ...
    for (uint i = 0; i < cn_init; ++i) {
        ...
        for (uint j = 0; j < 8; ++j) {
            uint4 block_in = vload4(j, (__private const uint*)text);
            uint4 block_out = AES_Round(AES0, AES1, AES2, AES3, block_in, vload4(0, (__private const uint*)expanded_key));
            for (uint k = 1; k < 10; ++k) {
                block_out = AES_Round(AES0, AES1, AES2, AES3, block_out, vload4(k, (__private const uint*)expanded_key));
            }
            vstore4(block_out, j, (__private uint*)text);
        }
        // Copy text to scratchpad
        for (uint j = 0; j < CN_INIT_SIZE_BYTE; ++j)
            scratchpad[i * CN_INIT_SIZE_BYTE + j] = text[j];
    }
    ...
    for (uint i = 0; i < iter_div; ++i) {
        ...
        __global uchar *sptr = scratchpad + (ulong)j * CN_AES_BLOCK_SIZE;

        // c = AES_single_round(scratchpad[j], a)
        uint4 sp_block = vload4(0, (__global const uint*)sptr);
        ...
        // VARIANT1_1: modify byte 11
        uchar tmp = sptr[11];
        uchar index = (((tmp >> 3) & 6) | (tmp & 1)) << 1;
        sptr[11] = tmp ^ ((0x75310 >> index) & 0x30);

        // Iteration 2
        j = cn_e2i(c, cn_aes_init);
        ...
        ((__private ulong*)a)[1] += lo;

        // scratchpad[j] = a
        ((__global ulong*)dptr)[0] = ((__private const ulong*)a)[0];
        ((__global ulong*)dptr)[1] = ((__private const ulong*)a)[1];

        // a ^= t
        ((__private ulong*)a)[0] ^= t0;
        ((__private ulong*)a)[1] ^= t1;

        // VARIANT1_2: scratchpad[j][1] ^= tweak1_2
        ((__global ulong*)dptr)[1] ^= tweak1_2;
    ...
    for (int i = 0; i < 32; ++i)
        aes_key[i] = state[32 + i];

    ((__private uint8*)expanded_key)[0] = ((__private uint8*)aes_key)[0];
    AESExpandKey256(expanded_key);

    for (uint i = 0; i < cn_init; ++i) {
        // text XOR scratchpad, then AES pseudo-round
        for (uint j = 0; j < 8; ++j) {
            uint4 text_block = vload4(j, (__private const uint*)text);
            uint4 sp_block = vload4(0, (__global const uint*)(scratchpad + (ulong)i * CN_INIT_SIZE_BYTE + j * CN_AES_BLOCK_SIZE));
            text_block ^= sp_block;

            for (uint k = 0; k < 10; ++k) {
                text_block = AES_Round(AES0, AES1, AES2, AES3, text_block, vload4(k, (__private const uint*)expanded_key));
            }
            vstore4(text_block, j, (__private uint*)text);
        }
    }

    // 9. Copy text back to state.init (state[64..191])
    for (int i = 0; i < CN_INIT_SIZE_BYTE; ++i)
        state[64 + i] = text[i];
    ...
    if (hash_sel == 0) {
        ...
        groestl256_hash(output, state, 200);
    } else if (hash_sel == 2) {
        jh256_hash(output, state, 200);
    } else {
        skein256_hash(output, state, 200);
    }
}
