// KawPow (RVN/CLORE/EVR/MEWC) OpenCL kernel — REAL implementation from xmrig.
//
// Implements the full ProgPow/KawPow algorithm with:
//   - keccak_f800 (32-bit words, 800-bit state)
//   - KISS99 RNG for random math sequence (changes every 10 blocks)
...
//   - Rust codegen: AuXpow/src/progpow_codegen.rs

// ── KawPow defs (inlined from kawpow_defs.h) ────────────────────────
...
#endif
__kernel void progpow_search(__global dag_t const* g_dag, __global uint* job_blob, ulong target, uint hack_false,
                             volatile __global uint* results, volatile __global uint* stop,
                             __global ulong *output_nonce, __global uchar *output_mix, __global volatile uint *found)
{
...
#pragma unroll
    ...
    if (result <= target)
    {
        ...
        // Also write to xmrig-style results buffer for compatibility
        *stop = 1;
