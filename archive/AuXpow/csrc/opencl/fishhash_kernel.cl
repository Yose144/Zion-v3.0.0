// FishHash OpenCL kernel for IronFish (IRON) mining.
//
// Implements the FishHash memory-hard PoW algorithm (FIP-3):
//   1. Blake3(block_header) → 64-byte seed (3 passes for 180-byte header)
//   2. 32 rounds of DAG access: fetch 3 values per round, FNV mix + multiply-add
//   3. FNV collapse 128→32 bytes
//   4. Blake3(mix_hash) → 32-byte final hash
//   5. Compare to target
//
// DAG: 37,748,717 items × 128 bytes = ~4.6GB (same as Ethash epoch 448
// but with Blake3 seed and 512 parent elements per DAG item)
//
// References:
//   - FishHash spec: https://fips.ironfish.network/fips/fip-3-memory-hard-mining-algorithm
//   - FishHashMiner: https://github.com/Lolliedieb/FishHashMiner
//   - iron-fish/fish-hash: https://github.com/iron-fish/fish-hash
