// Verushash V2.2 CUDA kernel — NVRTC-compatible, self-contained.
//
// Adapted from monkins1010/ccminer verus2.2gpu branch (verus/verus.cu).
// Original: Copyright (c) monkins1010, released under GPL/MIT.
//
// Key changes for NVRTC:
//   - No external includes (miner.h, cuda_helper.h removed)
//   - __constant__ vkey/blockhash_half/ptarget replaced with kernel args
//   - All host-side code removed
//   - Single verus_mine kernel entry point
//
// Kernel signature:
//   __global__ void verus_mine(
//       const uint4 *vkey,           // 552 uint4 precomputed key
//       const uint4 *blockhash_half, // 4 uint4 block header hash (64 bytes)
//       const uint32_t *ptarget,     // 8 uint32 target
...
// ── NVRTC compatibility: operator overloads for uint4/uint2 ───────────────
// CUDA built-in vector types don't have ^, &, | operators by default.
// The original ccminer code relies on these via cuda_helper.h.
...
__device__ __forceinline__ uint4 operator^(uint4 a, uint32_t b) {
...
__constant__ uint32_t saes_table[4][256] = { saes_data(saes_u0), saes_data(saes_u1), saes_data(saes_u2), saes_data(saes_u3) };
...
#define THREADS 128
#define TOTAL_MAX 0x1000  // 4096 threads — scratch = 4096 * 552 * 16 = ~36MB
...
__device__ uint128m _mm_clmulepi64_si128_emu(uint128m ai, uint128m bi, int imm)
{
	...
	uint64_t r[2];
	if (__popcll(a) > __popcll(b)) { a = b; b = ((uint64_t*)&ai)[0]; }
	r[0] = 0; r[1] = 0;
	...
	uint64_t selector, uint128m &acc, uint128m *randomsource, uint32_t prand_idx)
{
	...
	do {
		...
		if (loop_c) {
			...
			uint4 modulo = { 0, 0, 0, 0 }; ((int32_t*)&modulo)[0] = (dividend % divisor);
			acc = modulo ^ acc;
			rounds--;
			if (rounds != (uint64_t)0xffffffffffffffff) loop_c = selector & ((uint64_t)0x10000000 << rounds);
		}
...
// ── Main VerusHash CLMUL loop ─────────────────────────────────────────────

__device__ __forceinline__ uint2 __verusclmulwithoutreduction64alignedrepeatgpu(
	uint128m * __restrict__ randomsource, const uint128m * __restrict__ buf,
	uint32_t * __restrict__ sharedMemory1, const uint128m * __restrict__ vkey_local,
	uint8_t version)
{
	uint128m const *pbuf;
	uint128m acc = vkey_local[513];
	...
	prand_idx = ((acc.x >> 5) & 511);
	prandex_idx = ((acc.y) & 511);
	prand = vkey_local[prand_idx];
	prandex = vkey_local[prandex_idx];
	int i = 0;
	uint8_t case_v;
...
// ── Haraka512 port keyed V2.2 ─────────────────────────────────────────────

__device__ __forceinline__ uint32_t haraka512_port_keyed2222(
	uint128m * __restrict__ in, uint128m * __restrict__ rc,
...
// output_nonce:   found nonce (u64)
// output_hash:    32-byte hash output
// found:          found flag (0 = not found)
...
)
{
	...
	const uint64_t nonce = base_nonce + (uint64_t)thread;
	...
	// Check found flag AFTER __syncthreads to avoid deadlock
	if (*found) return;

	// Each thread gets its own key workspace in global memory
	uint128m *randomsource = &scratch[(VERUS_KEY_SIZE128 * (thread & (TOTAL_MAX - 1)))];

	// Copy key from shared memory to per-thread global memory
