...
use zion_core::{
    CoreRuntime, DifficultyTarget, MiningHeader, MiningJob, MiningSolution, RevenueSource,
};
use zion_pool::{
...
mod crash_handler {
    ...
    extern "C" fn handler(sig: c_int) {
        ...
        eprintln!(
            "[CRASH] signal={} — miner will exit. Watchdog should restart.",
            sig
        );
        unsafe {
            signal(sig, SIG_DFL);
...
fn is_deeksha_algorithm(algorithm: &str) -> bool {
    matches!(
        ...
        "deeksha_lite_v1" | "deeksha_lite_fire" | "cosmic_harmony_ekam_deeksha_v2" | "ekam_deeksha"
    )
}
...
fn cuda_verify_label(algo: &str) -> Option<&'static str> {
    match algo {
        ...
        "kawpow" | "kawpow_rvn" | "kawpow_clore" | "kawpow_evr" | "kawpow_mewc" => Some("KAWPOW"),
...
fn cuda_verify_share(
    ...
    let (header, cpu_hash_fn): (
        ...
        "blake3" | "blake3_alph" => {
            ...
            (
                header.clone(),
                Box::new(move |nonce, _gpu_hash, _mix| eh::hash_blake3_alph(&header, &[], nonce)),
            )
        }
        "blake3_dcr" => {
    ...
    match algo {
        ...
        "zelhash" | "zelhash_flux" => {
            ...
            (
                ...
                Box::new(move |share| {
                    ...
                    let sol = share.solution.as_ref().expect("zelhash requires solution");
        ...
        "ethash" | "etchash" | "ethash_etc" | "ethash_ethw" => {
            ...
            (
                header,
                Box::new(move |share| {
                    let mix = share.mix_hash.expect("ethash requires mix_hash");
        ...
        "kawpow" | "kawpow_rvn" | "kawpow_clore" | "kawpow_evr" | "kawpow_mewc" => {
            ...
            (
                header,
                Box::new(move |share| {
                    let mix = share.mix_hash.expect("kawpow requires mix_hash");
                    let cpu = eh::ethash_final_hash(&header_arr, share.nonce, &mix);
        ...
        | "evrprogpow_evr" | "meowpow" | "meowpow_mewc" => {
            ...
            (
                header,
                Box::new(move |share| {
                    let mix = share.mix_hash.expect("progpow requires mix_hash");
...
fn main() -> Result<()> {
    ...
    if std::env::var("ZION_MINER_VERBOSE")
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false)
    ...
    #[cfg(not(feature = "public_build"))]
    {
        let auto_mode = std::env::var("ZION_AUTO_MODE")
            ...
            .unwrap_or(false);
        if auto_mode {
            let s1 = std::env::var("ZION_STREAM1_ENABLED")
                .map(|v| v != "0")
                .unwrap_or(true);
            let s2 = std::env::var("ZION_STREAM2_ENABLED")
                .map(|v| v != "0")
                .unwrap_or(true);
            let s3 = std::env::var("ZION_STREAM3_ENABLED")
                .map(|v| v != "0")
                .unwrap_or(true);
            println!("=== ZION Auto Mode ===");
            println!(
                "  Stream 1 (ZION primary):  {}",
                if s1 { "ON" } else { "OFF" }
            );
            private_print!(
    ...
    if std::env::args().any(|a| a == "--verus-bench") {
        ...
        println!(
            "expected_time={:.0}s = {:.1}min = {:.1}h = {:.1}days",
            secs_per_share,
            secs_per_share / 60.0,
            secs_per_share / 3600.0,
            secs_per_share / 86400.0
        );
        ...
        println!(
            "expected_time={:.1}s = {:.1}min",
            secs_per_share_easy,
            secs_per_share_easy / 60.0
        );
    ...
    if std::env::args().any(|a| a == "--randomx-bench") {
        ...
        #[cfg(feature = "native-randomx")]
        {
            ...
            println!(
                ...
                secs_per_block,
                secs_per_block / 60.0,
                secs_per_block / 3600.0,
                secs_per_block / 86400.0
            );
    ...
    #[cfg(feature = "gpu-cuda")]
    {
        ...
        if let Some(pos) = args.iter().position(|a| a == "--test-cuda-kernel") {
            ...
            println!(
                "algorithm={} work_size={} bench_secs={}",
                algo, work_size, secs
            );
                    ...
                    if let Some(label) = cuda_verify_label(&algo) {
                        ...
                        match cuda_verify_share(&algo, &mut miner, &target, batch) {
                            Ok(Some((nonce, gpu_hash, mix_hash, cpu_hash, matched))) => {
                                ...
                                if matched {
                                    println!(
                                        "{}_CPU_GPU_MATCH nonce={} hash_prefix={} mix_prefix={}",
                                        label,
                                        nonce,
                                        prefix(&gpu_hash),
                                        mix
        ...
        if let Some(pos) = args.iter().position(|a| a == "--test-opencl-kernel") {
            ...
            match miner.mine_simple(&algo, &header, &target, 0, batch) {
                Ok(Some(share)) => {
                    ...
                    if matched {
                        println!(
                            "{}_OPENCL_CPU_GPU_MATCH nonce={} hash_prefix={}",
                            label,
                            share.nonce,
                            prefix(&share.hash)
                        );
                        ...
                        println!(
                            "{}_OPENCL_CPU_GPU_MISMATCH nonce={} gpu_prefix={} cpu_prefix={}",
                            label,
                            share.nonce,
                            prefix(&share.hash),
                            prefix(&cpu_hash)
    ...
    if interactive {
        let log_path =
            std::env::var("ZION_MINER_LOG").unwrap_or_else(|_| "/tmp/zion-miner.log".to_string());
...
fn run_local_session(
    ...
    let mut tri_gpu = match gpu_backend::TriGpuManager::with_work_sizes(
        ...
        Err(e) => {
            ...
            gpu_available = false;
            gpu_backend::TriGpuManager::with_work_sizes(gpu_backend::GpuBackendKind::Cpu, 1, 1, 1)?
        }
    };
    ...
    let stream2_local = config.stream2_enabled
        && gpu_available
        && config.gpu_backend != gpu_backend::GpuBackendKind::Cpu;
    ...
    if stream2_local {
        ...
        thread::spawn(move || {
            private_print!(
                "[{}] external_gpu_thread_spawned (local benchmark)",
                log_timestamp()
            );
    ...
    for iteration in 0..config.loop_count {
        ...
        let scan_result = if can_gpu {
            ...
            if let Err(e) = g.update_epoch(job.height) {
                ...
                let result =
                    gpu_backend::gpu_scan_job(g, job, &current_algorithm, &raw_header_bytes);
        ...
        let submit_line = if let Some(blob) = gpu_solution_blob {
            ...
            encode_message(&pool.solution_message(&config.miner_id, &config.worker_name, solution))?
        };
...
fn fetch_pool_stream_config(pool_addr: &str) -> Option<PoolStreamConfig> {
    ...
    let socket_addrs: Vec<std::net::SocketAddr> =
        std::net::ToSocketAddrs::to_socket_addrs(&api_addr)
            .ok()?
            .collect();
...
fn run_remote_session(
    ...
    if let Some(pool_cfg) = fetch_pool_stream_config(pool_addr) {
        ...
        if config.stream2_enabled && !pool_cfg.gpu.enabled {
            private_print!(
                "[{}] pool_config_override stream2=disabled (pool GPU stream disabled)",
                log_timestamp()
            );
        ...
        if config.stream3_enabled && !pool_cfg.cpu.enabled {
            private_print!(
                "[{}] pool_config_override stream3=disabled (pool CPU stream disabled)",
                log_timestamp()
            );
        ...
        private_print!(
            "[{}] pool_config_unreachable — using env-var stream config (stream2={} stream3={})",
            log_timestamp(),
            stream2_enabled,
            stream3_enabled
        );
    ...
    if profit_router.is_enabled() {
        private_print!(
            "[{}] autonomous_mode_enabled — running initial coin selection",
            log_timestamp()
        );
    ...
    let mut gpu_available = effective_gpu_backend != gpu_backend::GpuBackendKind::Cpu
        && (config.stream1_enabled || config.stream2_enabled);
    // When stream1 (primary/deeksha) is disabled, create a CPU dummy
    // TriGpuManager so no GPU VRAM is allocated for the primary backend.
    // This frees all VRAM for the external GPU thread (stream2) — critical
    // for memory-heavy external algorithms like ERG Autolykos whose R table
    // (~5.7 GB at production height) would OOM alongside the deeksha
    // scratchpad (~2 GB) on consumer 8 GB GPUs.
    let tri_gpu_kind = if config.stream1_enabled {
        effective_gpu_backend
    } else {
        gpu_backend::GpuBackendKind::Cpu
    };
    ...
    if dual_gpu_enabled {
        ...
        private_print!(
            "[{}] stream2_gpu_external_disabled (gpu_available={})",
            log_timestamp(),
            gpu_available
        );
    }
    ...
    println!(
        "[{}] pool_io_thread_started — job pre-fetch + async submit enabled",
        log_timestamp()
    );
    ...
    private_print!(
        "[{}] ext_share_submitter_thread_started — immediate ext share submission",
        log_timestamp()
    );
    sync_miner_metrics(
        metrics,
    ...
    if let Some(pref_msg) = profit_router.build_coin_preference(&config.miner_id) {
        ...
        private_print!(
            "[{}] autonomous_coin_preference_sent {}",
            log_timestamp(),
            pref_line.trim()
        );
    ...
    let drain_late_external = |accepted: bool, status: &str, coin: &str, waiting_for: &str| {
        ...
        if accepted {
            private_print!(
                "[{}] external_share_accepted coin={} status={}",
                log_timestamp(),
                coin,
                status
            );
            ...
            private_print!(
                "[{}] external_share_rejected coin={} status={}",
                log_timestamp(),
                coin,
                status
            );
    ...
    for iteration in 0..config.loop_count {
        ...
        if !QUIET.load(Ordering::Relaxed) {
            println!(
                ">> new job #{} height={} algo={}",
                job.job_id, job.height, algorithm
            );
        }
        ...
        if let Some(ref ext) = external_stream {
                ...
                || ext.algorithm.eq_ignore_ascii_case("pearlhash")
            {
                if VERBOSE.load(Ordering::Relaxed) {
                    private_print!(
                        "external_stream_ignore coin={} algo={} reason=pearl_disabled",
                        ext.coin,
                        ext.algorithm
                    );
                }
                ...
                if VERBOSE.load(Ordering::Relaxed) {
                    private_print!(
                        "external_stream_ignore coin={} algo={} reason=ext_gpu_disabled",
                        ext.coin,
                        ext.algorithm
                    );
                }
        ...
        if let Some(g) = gpu_ref.as_mut() {
            if !stream_weights_str.is_empty()
                && config.gpu_backend == gpu_backend::GpuBackendKind::Metal
            {
                match zion_cosmic_harmony::stream_profit::StreamWeights::parse(&stream_weights_str)
                {
                    Ok(weights) => {
                        if let Err(e) = g.set_stream_weights(&weights) {
                            private_print!(
                                "stream_weights_apply_error job={} err=\"{e}\"",
                                job.job_id
                            );
                        }
                    }
                    Err(e) => {
                        private_print!(
                            "stream_weights_parse_error job={} err=\"{e}\" raw=\"{}\"",
                            job.job_id,
                            stream_weights_str
                        );
                    }
                        ...
                        Ok(result) => {
                            ...
                            if let Some((nonce, gpu_hash, mix_hash)) = result.solutions.first() {
                                ...
                                if is_mismatch && VERBOSE.load(Ordering::Relaxed) {
                                    eprintln!(
                                        "[{}] gpu_hash_mismatch nonce={} gpu={} cpu={}",
                                        log_timestamp(),
                                        nonce,
                                        hex(&gpu_hash[..8]),
                                        hex(&cpu_hash[..8])
        ...
        let Some(solution) = scan_result else {
            ...
            let no_solution_line =
                write_wire_message(&mut *writer.lock().unwrap(), &no_solution_message)?;
            ...
            let (result_line_raw, result_message) = loop {
                match result_rx.recv() {
                    ...
                    Err(_) => {
                        return Err(anyhow!("pool I/O channel closed during no_solution result"))
                    }
                }
            };
            ...
            maybe_send_adaptive_update(&adaptive_tx, &last_adaptive_update, &hashrate);
            continue;
        };
        let search_depth = solution.candidate.nonce.saturating_sub(job.start_nonce) + 1;
        let tested = if can_gpu {
        ...
        let mix_hash_hex = if let Some(blob) = &gpu_solution_blob {
            ...
            gpu_mix_hash.map(|mh| mh.iter().map(|b| format!("{:02x}", b)).collect::<String>())
        };
    ...
    fn maybe_print_status(
            ...
            && !no_dashboard
        {
            ...
            raw_stdout(&format!(
                "║  Uptime: {:<6}  Pool: {:<20}   ║\n",
                uptime_str,
                &pool_addr[..pool_addr.len().min(20)]
            ));
            let dev_name = if self.gpu_device_name.is_empty() {
                "cpu"
            } else {
                &self.gpu_device_name
            };
...
fn submit_external_share(
    ...
    match result_rx.recv_timeout(Duration::from_millis(ext_timeout_ms)) {
        Ok(PoolIncoming::Result(line, msg)) => {
            ...
            if let PoolMessage::ExternalResult {
                accepted,
                status,
                coin,
            } = msg
...
fn external_gpu_thread(
    ...
    #[cfg(feature = "gpu-cuda")] shared_cuda_dev: Option<
        std::sync::Arc<cudarc::driver::CudaDevice>,
    >,
    #[cfg(not(feature = "gpu-cuda"))] shared_cuda_dev: Option<()>,
) {
    private_print!(
        "[{}] external_gpu_thread_entered backend={} shared_cuda={} adaptive={}",
        log_timestamp(),
        backend_kind.as_str(),
        shared_cuda_dev.is_some(),
        adaptive_rx.is_some()
    );
    ...
    loop {
        ...
        match latest_job {
            Some(job) => {
                ...
                let is_new_job = current_job
                    .as_ref()
                    .map_or(true, |j| j.job_id != job.job_id || j.height != job.height);
                if is_new_job {
                    reset_ext_gpu_nonce(
                        &job,
                        &mut nonce_base,
            ...
            None => {
                ...
                {
                    private_print!(
                        "[{}] ext_gpu_rx_empty (no job yet, thread alive)",
                        log_timestamp()
                    );
                }
            }
        ...
        let target_bytes =
            match zion_pool::parse_fixed_hex::<32>(&job.target_hex, "external target") {
                Ok(t) => t,
                Err(e) => {
        ...
        if batch_count == 0
            && (algo.contains("progpow") || algo.contains("ethash") || algo.contains("kawpow"))
        {
        ...
        match result {
            Ok(br) => {
                ...
                if (batch_count < 5 || batch_count % 100 == 0) && !QUIET.load(Ordering::Relaxed) {
                    private_print!(
                ...
                if let Some((found_nonce, hash, mix_hash)) = br.solutions.first() {
                    ...
                    if let Some(mix) = mix_hash {
                        if let Ok(header_arr) =
                            <[u8; 32]>::try_from(&header_bytes[..32.min(header_bytes.len())])
                        {
                            let rust_seed = zion_auxpow::external_hashers::progpow_seed_pub(
                                &header_arr,
                                *found_nonce,
                            );
                            let real_hash = zion_auxpow::external_hashers::progpow_final_hash(
                                &header_arr,
                                *found_nonce,
                                mix,
                            );
            ...
            if let Some(ref arx) = adaptive_rx {
                while let Ok((new_burst, new_gap_ms)) = arx.try_recv() {
                    ...
                    private_print!(
                        ...
                        new_gap_ms,
                        if new_burst > 0 {
                            100.0 * new_burst as f64 / (new_burst as f64 + new_gap_ms as f64 / 50.0)
                        } else {
                            0.0
                        }
                    );
...
impl RandomXWorkerPool {
    ...
    fn scan(
        ...
        let found = std::sync::Arc::new(std::sync::Mutex::new(
            None::<zion_auxpow::miner_harness::FoundShare>,
        ));
        let stop = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
...
fn ext_cpu_thread(
    ...
    let randomx_nonce_count =
        parse_env_u64("ZION_EXT_CPU_RANDOMX_NONCE_COUNT", 10_000).unwrap_or(10_000);
    ...
    let ghostrider_nonce_count =
        parse_env_u64("ZION_EXT_CPU_GHOSTRIDER_NONCE_COUNT", 600).unwrap_or(600);
    ...
    loop {
        ...
        loop {
            match rx.try_recv() {
                ...
                Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                    private_print!(
                        "[{}] ext_cpu_thread: channel closed, exiting",
                        log_timestamp()
                    );
                    return;
                }
        ...
        if let Some(ref ext) = current_job {
            ...
            let share_opt = if is_randomx {
                ...
                match coin {
                    ...
                    Some(coin) => {
                        let header_bytes =
                            match hex::decode(ext.header_hex.trim_start_matches("0x")) {
                                Ok(b) => b,
                                Err(e) => {
                        ...
                        let extranonce1 = if ext.extranonce1_hex.is_empty() {
                            ...
                            hex::decode(ext.extranonce1_hex.trim_start_matches("0x"))
                                .unwrap_or_default()
                        };
                        ...
                        rx_pool
                            .scan(&job_pkg, start_nonce, effective_nonce_count)
            ...
            if let Some(share) = share_opt {
                ...
                let next_offset =
                    nonce_offset.wrapping_add(share.nonce.wrapping_sub(start_nonce) + 1);
                let _ = tx.send(share);
...
fn ext_share_submitter_thread(
    ...
    loop {
        ...
        let share = match ext_cpu_share_rx.recv_timeout(Duration::from_millis(500)) {
            ...
            Err(std::sync::mpsc::RecvTimeoutError::Disconnected) => {
                private_print!(
                    "[{}] ext_share_submitter_thread: CPU channel closed, exiting",
                    log_timestamp()
                );
                return;
            }
            ...
            if !check_and_record(
                ...
                let send_result = {
                    ...
                    match zion_pool::encode_message(&ext_submit) {
                        ...
                        Err(e) => {
                            private_eprint!(
                                "[{}] ext_share_submitter: encode error: {}",
                                log_timestamp(),
                                e
                            );
                            Err("encode failed")
                ...
                if let Err(e) = send_result {
                    private_eprint!(
                        "[{}] ext_share_submitter: send error: {}",
                        log_timestamp(),
                        e
                    );
                } else {
...
fn pool_io_thread(
    ...
    loop {
        ...
        match message {
                ...
                if let Some(ref ext) = external_stream {
                        ...
                        && !ext.algorithm.eq_ignore_ascii_case("pearlhash")
                    {
                        ...
                        if gpu_backend::is_cpu_only_algorithm(&ext.algorithm) {
                            let _ = ext_cpu_tx.send(ext.clone());
                        } else if disable_ext_gpu
                            && gpu_backend::is_external_algorithm(&ext.algorithm)
                        {
            ...
            other => {
                eprintln!(
                    "[{}] pool_io_unexpected_message: {other:?}",
                    log_timestamp()
                );
            }
        }
...
fn parse_header_hex(raw: &str) -> Result<MiningHeader> {
    ...
    let decoded = hex::decode(normalized).with_context(|| "job header contains invalid hex")?;
...
impl MinerConfig {
    fn from_env_and_args() -> Result<Self> {
            ...
            match args[i].as_str() {
                ...
                "--auto-tune" => {
                    ...
                    println!(
                        ...
                        result.cpu_model, result.cpu_physical_cores, result.cpu_cores,
                    );
                    println!("  RAM:  {} MB", result.sys_ram_bytes / (1024 * 1024),);
                    println!();
                    println!("Recommended Settings:");
                    println!("  ZION_GPU_WORK_SIZE={}", result.gpu_work_size,);
                    private_print!(
                        "  ZION_SECONDARY_GPU_WORK_SIZE={}",
                        result.secondary_gpu_work_size,
                    );
                    println!("  ZION_THREADS={}", result.threads,);
                    private_print!(
                    ...
                    println!(
                        ...
                        result.gpu_compute_units, result.gpu_work_size,
                    );
                ...
                "--help" | "-h" => {
                    ...
                    private_print!(
                        "  External CPU:      verushash (VRSC), randomx (XMR), ghostrider (RTM)"
                    );
        ...
        if autotune_enabled {
            ...
            println!(
                "  CPU: {} ({} physical / {} logical cores)",
                autotune.cpu_model, autotune.cpu_physical_cores, autotune.cpu_cores,
            );
            println!("  RAM: {} MB", autotune.sys_ram_bytes / (1024 * 1024),);
            private_print!(
