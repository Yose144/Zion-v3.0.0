...
impl StratumClient {
    pub fn new(
        ...
        Ok(Self {
            ...
            difficulty,
            group_hint: group_hint.map(|s| s.to_string()),
    ...
    fn password(&self) -> String {
        let mut parts = vec![format!("algo={}", self.algorithm)];
        if let Some(d) = self.difficulty {
            parts.push(format!("d={}", d));
        }
        if let Some(ref g) = self.group_hint {
            if !g.trim().is_empty() {
                parts.push(format!("g={}", g.trim()));
            }
        }
        parts.join(",")
    }
    ...
    pub async fn connect(&self) -> Result<()> {
        ...
        *self.writer.lock().await = Some(write_half);
        ...
        // NOTE: Some external pools (e.g. MoneroOcean) return a temporary suspension error
        // for new connections from an IP. In that case we must NOT fallback or retry
        ...
        loop {
            ...
            match self.connect().await {
                ...
                Err(e) => {
                    ...
                    // When we detect this condition, back off for ~10 minutes.
                    let emsg = e.to_string();
                    let delay = if emsg.contains("temporarily suspended")
                        || emsg.contains("New connections from this IP")
                    {
                        600
                    } else {
    ...
    pub async fn submit_share(&self, job_id: &str, nonce: u32, result: &str) -> Result<bool> {
        ...
        let submit_request = if protocol == ClientProtocol::Xmrig {
            ...
            // XMRig/CryptoNote pools expect the nonce as 4 bytes little-endian hex.
            // If we send it as a big-endian u32 ("{:08x}"), the pool will reconstruct
            // a different blob, compute a different hash, and reject as low difficulty.
