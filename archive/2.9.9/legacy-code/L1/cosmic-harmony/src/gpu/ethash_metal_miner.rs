...
impl EthashDagGenerator {
    ...
    pub fn generate_dag(epoch: &EthashEpoch) -> Vec<u8> {
        log::debug!(
            "🔧 Generating Ethash DAG for epoch {} ({:.2} GB)...",
            epoch.number,
        ...
        log::debug!(
            "   DAG generated: {} MB in {:.1}s",
            dataset.len() / (1024 * 1024),
            start.elapsed().as_secs_f64()
        );
    ...
    fn generate_cache(epoch: &EthashEpoch) -> Vec<[u8; 64]> {
        ...
        for _ in 0..3 {
            for i in 0..cache_len {
                let v = u32::from_le_bytes([cache[i][0], cache[i][1], cache[i][2], cache[i][3]])
                    as usize
                    % cache_len;
    ...
    fn generate_dataset(epoch: &EthashEpoch, cache: &[[u8; 64]]) -> Vec<u8> {
        ...
        for i in 0..dataset_items {
            ...
            if i % 1_000_000 == 0 && i > 0 {
                log::debug!(
                    "   DAG progress: {:.1}%",
                    (i as f64 / dataset_items as f64) * 100.0
                );
            }
        }
...
impl EthashMetalMiner {
    ...
    pub fn new(batch_size: usize) -> Result<Self, EthashMetalError> {
        let device = Device::system_default().ok_or(EthashMetalError::NoDevice)?;
    ...
    pub fn new_with_device(device: Device, batch_size: usize) -> Result<Self, EthashMetalError> {
        ...
        let mine_fn = library
            .get_function("ethash_mine", None)
            .map_err(|e| EthashMetalError::FunctionNotFound(format!("ethash_mine: {:?}", e)))?;
        let benchmark_fn = library
            .get_function("ethash_benchmark", None)
            .map_err(|e| {
                EthashMetalError::FunctionNotFound(format!("ethash_benchmark: {:?}", e))
            })?;
    ...
    pub fn load_dag_for_epoch(&mut self, epoch: &EthashEpoch) -> Result<(), EthashMetalError> {
        ...
        log::debug!(
            "🔧 Loading DAG for epoch {} ({:.2} GB, available: {:.2} GB)",
            epoch.number,
        ...
        log::debug!(
            "✅ DAG uploaded to Metal GPU: {} items ({:.2} GB)",
            epoch.dataset_items,
    ...
    pub fn load_dag_data(
        &mut self,
        epoch_number: u64,
        dag_items: u32,
        data: &[u8],
    ) -> Result<(), EthashMetalError> {
        let options = MTLResourceOptions::StorageModeShared;
        let dag_buf =
            self.device
                .new_buffer_with_data(data.as_ptr() as *const _, data.len() as u64, options);
        ...
        log::debug!(
            "✅ Pre-computed DAG loaded: epoch={}, items={}",
            epoch_number,
            dag_items
        );
        Ok(())
...
impl std::fmt::Display for EthashMetalError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DagNotLoaded => {
                write!(f, "Ethash DAG not loaded — call load_dag_for_epoch() first")
            }
            Self::DagTooLarge(need, have) => write!(
                f,
                "DAG too large: need {} GB, have {} GB",
