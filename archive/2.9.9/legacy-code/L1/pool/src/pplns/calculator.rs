...
impl PPLNSCalculator {
    ...
    pub async fn calculate_distribution(
        ...
        let mut miner_weighted_shares: HashMap<String, (u128, u64)> = HashMap::new();
        let mut total_weighted_shares: u128 = 0;

        for share in &window_shares {
            let weight = share.difficulty as u128;
        ...
        // AUDIT-FIX P-06 (16 Feb 2026): Use u128 integer arithmetic instead of
        // f64 to prevent systematic rounding bias that inflated pool_fee at the
        // expense of miners.  remainder goes to first eligible miner (not pool).
        ...
        for (miner_address, (weighted_shares, share_count)) in &miner_weighted_shares {
            ...
            if amount > 0 {
                ...
                payouts.push(Payout {
                    ...
                    weighted_shares: *weighted_shares as f64,
                });
            }
        }

        // Distribute remainder (dust) to first miner to avoid loss
        let remainder = miner_share_reward.saturating_sub(distributed);
...
pub fn compute_pplns_payouts(
    ...
    let mut miner_weighted: HashMap<String, (u128, u64)> = HashMap::new();
    let mut total_weight: u128 = 0;

    for (addr, diff) in shares {
        let w = *diff as u128;
        total_weight += w;
        let e = miner_weighted.entry(addr.clone()).or_insert((0, 0));
        e.0 += w;
        e.1 += 1;
    }

    // AUDIT-FIX P-06: integer arithmetic (same as calculate_distribution)
    let mut payouts = Vec::new();
    let mut distributed: u64 = 0;
    for (miner_address, (weighted, count)) in &miner_weighted {
        let amount = ((miner_share_reward as u128) * weighted / total_weight) as u64;
        if amount > 0 {
            distributed += amount;
            payouts.push(Payout {
                miner_address: miner_address.clone(),
                ...
                timestamp,
                shares_count: *count,
                weighted_shares: *weighted as f64,
            });
        }
    }
    let remainder = miner_share_reward.saturating_sub(distributed);
