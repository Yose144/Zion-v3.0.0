...
pub fn build(state: State) -> Router {
    ...
    let protected = Router::new()
        .route(
            "/rpc/submit_block",
            post(methods::submit_block).with_state(state.clone()),
        )
        .route(
            "/rpc/submit_tx",
    ...
    let public = Router::new()
        ...
        .route(
            ...
            get(methods::get_block_template).with_state(state.clone()),
        )
        .route("/rpc/get_premine_total", get(methods::get_premine_total))
        .route(
            "/rpc/get_premine_summary",
            get(methods::get_premine_summary),
        )
        .route("/rpc/get_premine_list", get(methods::get_premine_list))
        .route(
            "/api/block/hash/:hash",
            get(methods::get_block_by_hash_rest).with_state(state.clone()),
        )
        .route(
            "/api/block/height/:height",
            get(methods::get_block_by_height_rest).with_state(state.clone()),
        )
        .route(
            "/api/blocks/range/:start/:end",
            get(methods::get_blocks_range_rest).with_state(state.clone()),
        )
        .route(
            "/api/tx/:txid",
            get(methods::get_tx_rest).with_state(state.clone()),
        )
        .route(
            "/api/mempool/info",
            get(methods::get_mempool_info_rest).with_state(state.clone()),
        )
        .route(
            "/api/address/:address/balance",
            get(methods::get_address_balance_rest).with_state(state.clone()),
        )
        .route(
