...
impl Inner {
    pub fn new(db_path: &str) -> State {
        ...
        let (height, tip) = storage.get_tip().unwrap_or((
            0,
        ...
        let (final_height, final_tip) = if height == 0 {
            ...
            println!(
                "Genesis timestamp: {} ({})",
                genesis_ts,
                crate::network::get_network().name()
            );
        ...
        metrics
            .current_height
            .store(final_height, std::sync::atomic::Ordering::Relaxed);
        metrics
            .current_difficulty
    ...
    pub fn process_block(&self, block: Block) -> Result<(u64, String), String> {
        ...
        for (tx_index, tx) in block.transactions.iter().enumerate() {
            ...
            for input in &tx.inputs {
                ...
                let addr_from_pubkey =
                    crate::crypto::keys::address_from_public_key_hex(&input.public_key);
                ...
                if let Ok(Some(src_block_hash)) =
                    self.storage.get_block_hash_for_tx(&input.prev_tx_hash)
                {
                    if let Ok(Some(src_height)) =
                        self.storage.get_height_for_block_hash(&src_block_hash)
                    {
        ...
        if let Err(e) = self.storage.save_block_and_apply_utxos(&block) {
            return Err(format!(
                "Database error saving block + UTXOs atomically: {}",
                e
            ));
        }
        ...
        let next_difficulty = if block.height() == 0 {
            ...
            use crate::blockchain::consensus::{lwma_next_difficulty, BlockInfo, LWMA_WINDOW};
        ...
        self.metrics
            .validation_time_us
            .store(validation_time_us, std::sync::atomic::Ordering::Relaxed);
        self.metrics
            .blocks_processed
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        self.metrics
        ...
        self.metrics
            .current_difficulty
            .store(block.difficulty(), std::sync::atomic::Ordering::Relaxed);
        self.metrics
            .last_block_time
        ...
        self.metrics
            .txs_in_mempool
            .store(self.mempool.size(), std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn reorg_to_fork(
        ...
        if is_ibd {
            println!(
                "🔀 IBD reorg: rolling back {} blocks from tip={} to fork_point={}",
