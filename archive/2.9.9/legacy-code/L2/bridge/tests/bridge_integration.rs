...
fn make_lock(tx_hash: &str, amount_zion: u64, chain: &str, recipient: &str) -> L1LockEvent {
    let atomic = amount_zion * 1_000_000;
    L1LockEvent {
...
fn make_burn(burn_id: &str, amount_wzion_zion: u64, recipient: &str) -> EvmBurnEvent {
    let atomic = amount_wzion_zion * 1_000_000;
    EvmBurnEvent {
...
fn test_full_lock_flow_l1_to_evm() {
    ...
    assert_eq!(lock.amount_atomic, 5_000_000_000);
    assert_eq!(lock.amount_wzion, "5000000000000000000000"); // 5000 × 1e18
...
fn test_full_burn_flow_evm_to_l1() {
    ...
    assert_eq!(burn.amount_l1_atomic, 1_000_000_000);
...
fn test_decimal_conversion_full_roundtrip() {
    ...
    let atomic = whole * 1_000_000 + frac;
    assert_eq!(atomic, 1_234_567_890);

    // Convert to wZION wei
    let wzion_wei = l1_atomic_to_wzion_wei(atomic);
    ...
    // Display
    assert_eq!(atomic_to_zion_display(recovered), "1234.56789");
}
...
fn test_amount_validation_ranges() {
    ...
    let small = l1_atomic_to_wzion_wei(50_000_000); // 50 ZION
...
fn test_supply_invariant() {
    ...
    for amount in &lock_amounts {
        let atomic = amount * 1_000_000;
        let wzion = l1_atomic_to_wzion_wei(atomic);
        let back = wzion_wei_to_l1_atomic(&wzion).unwrap();
