...
fn burn(burn_id: &str, wzion_zion: u64, l1_recipient: &str) -> EvmBurnEvent {
    let atomic = wzion_zion * 1_000_000;
    EvmBurnEvent {
...
fn test_api_block_format_basic_conversion() {
    // Verify that 6-decimal L1 atomic ↔ 18-decimal EVM wei conversion is correct
    // At L1 block height 4574, a lock TX output pays 5_400_000 atomic = 5.4 ZION
    let amount_atomic: u64 = 5_400_000;
    let wzion_wei = l1_atomic_to_wzion_wei(amount_atomic);
    ...
    assert_eq!(recovered, amount_atomic);
}
...
fn test_l1_block_empty_inputs_field() {
            ...
            "transactions": [
                {
                    "id": "coinbase_tx_001",
                    "outputs": [
                        {"address": "zion1qminer", "amount": 5400000000}
                    ]
                }
            ]
        },
    ...
    assert_eq!(tx["outputs"][0]["amount"].as_u64().unwrap(), 5_400_000_000);
}
...
fn test_amount_below_minimum() {
    ...
    let small = l1_atomic_to_wzion_wei(99_000_000); // 99 ZION
...
fn test_amount_exactly_at_minimum() {
    ...
    let exact = l1_atomic_to_wzion_wei(100_000_000); // 100 ZION
...
fn test_amount_just_below_timelock() {
    ...
    let just_under = l1_atomic_to_wzion_wei(999_999_000_000); // 999,999 ZION
...
fn test_amount_exactly_at_timelock() {
    ...
    let exactly = l1_atomic_to_wzion_wei(1_000_000_000_000); // 1M ZION
...
fn test_amount_exceeds_single_limit() {
    ...
    let too_big = l1_atomic_to_wzion_wei(5_000_001_000_000); // 5M+1 ZION
...
fn test_supply_invariant_conversion_precision() {
    ...
    for &zion in amounts_zion {
        let atomic = zion * 1_000_000;
        let wzion = l1_atomic_to_wzion_wei(atomic);
        let recovered = wzion_wei_to_l1_atomic(&wzion).unwrap();
        assert_eq!(
            atomic, recovered,
