...
pub struct Treasury {
    ...
    /// Multi-sig threshold
    threshold: u32,
    /// Total balance (tracked off-chain, verified against L1)
    balance: u64,
...
impl Treasury {
    pub fn new(guardians: Vec<Guardian>, balance: u64) -> Self {
        Self {
            guardians,
    ...
    pub fn submit_operation(
        ...
        // Check balance
        let amount = self.operation_amount(&operation);
        ...
        if self.daily_spent + amount > DAILY_SPEND_LIMIT {
            return Err(DaoError::DailyLimitExceeded {
                limit: DAILY_SPEND_LIMIT / 1_000_000, // in ZION
            });
        }
    ...
    pub fn execute(&mut self, operation_id: &str) -> DaoResult<TreasuryOperation> {
        ...
        let amount = self.operation_amount(&pending.operation);
        self.balance -= amount;
