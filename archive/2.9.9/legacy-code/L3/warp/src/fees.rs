...
pub struct RouteFee {
    ...
    pub fee_rate: f64,
    /// Minimum fee in ZION atomic units (6 decimals)
    pub min_fee: u64,
    /// Maximum fee in ZION atomic units
    pub max_fee: u64,
}
...
impl FeeEngine {
    ...
    pub fn split_fee(&self, fee_atomic: u64) -> (u64, u64, u64) {
        let burn = fee_atomic * self.distribution.burn_pct as u64 / 100;
        let dao = fee_atomic * self.distribution.dao_pct as u64 / 100;
        let validator = fee_atomic - burn - dao; // remainder to validators
        (burn, dao, validator)
    }
...
mod tests {
    ...
    fn test_fee_evm_basic() {
        let engine = FeeEngine::with_defaults();
        // 1000 ZION = 1_000_000_000 atomic → 0.1% = 1_000_000 (1 ZION)
    ...
    fn test_fee_minimum_enforced() {
        let engine = FeeEngine::with_defaults();
        // 0.01 ZION = 10_000 atomic → 0.1% = 10 → clamped to min 100_000
    ...
    fn test_fee_maximum_enforced() {
        let engine = FeeEngine::with_defaults();
        // 100B ZION → 0.1% = 100M → clamped to max 10_000 ZION
        let fee = engine.calculate_fee("base", 100_000_000_000_000).unwrap();
    ...
    fn test_fee_bitcoin_higher_rate() {
        let engine = FeeEngine::with_defaults();
        // 10,000 ZION = 10_000_000_000 atomic → 0.25% = 25_000_000
        let fee = engine.calculate_fee("bitcoin", 10_000_000_000).unwrap();
    ...
    fn test_fee_cardano_rate() {
        let engine = FeeEngine::with_defaults();
        // 10,000 ZION → 0.2% = 20_000_000
        let fee = engine.calculate_fee("cardano", 10_000_000_000).unwrap();
