...
impl WarpConfig {
    ...
    /// Convert ZION whole units to atomic (6 decimals).
    pub fn daily_limit_atomic(&self) -> u64 {
        self.daily_limit_zion * 1_000_000
    }

    pub fn timelock_threshold_atomic(&self) -> u64 {
...
mod tests {
    ...
    #[test]
    fn test_daily_limit_atomic() {
        let config = WarpConfig::default();
        assert_eq!(config.daily_limit_atomic(), 10_000_000_000_000);
    }

    #[test]
    fn test_timelock_atomic() {
        let config = WarpConfig::default();
        assert_eq!(config.timelock_threshold_atomic(), 1_000_000_000_000);
    }
