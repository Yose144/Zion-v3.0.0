...
//! ```text
//! base_xp     = 50
//! volume_xp   = min(amount_atomic / 1_000_000, 200)   ← up to 200 XP for large transfer
...
use crate::types::{ChainFamily, WarpTransfer};
...
pub struct WarpXpEvent {
    ...
    /// Amount bridged (atomic units, 6 decimals).
...
impl WarpXpReward {
    ...
    pub fn for_transfer(ev: &WarpXpEvent) -> u64 {
        let base: u64 = 50;
        let volume: u64 = (ev.amount_atomic / 1_000_000).min(200);
...
mod tests {
    ...
    fn test_xp_cross_family_large() {
        let t = make_transfer(500_000_000, ChainId::solana());
        let ev = WarpXpEvent::from_transfer(&t);
        let xp = WarpXpReward::for_transfer(&ev);
    ...
    fn test_event_from_transfer_fields() {
        ...
        assert_eq!(ev.amount_atomic, 1_000_000);
    }
