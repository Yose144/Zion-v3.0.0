...
try:
    from src.miner.cosmic_harmony_native import CosmicHarmonyNative
    ...
    try:
        # Fallback path
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src', 'miner'))
        from cosmic_harmony_native import CosmicHarmonyNative
        _cosmic_v3_native = CosmicHarmonyNative()
        COSMIC_V3_AVAILABLE = True
        print(f"[OK] Cosmic Harmony v3 NATIVE loaded (fallback)")
    except Exception as e:
        print(f"[WARN] Cosmic Harmony v3 Native not available: {e} (hint: set ZION_CHV3_LIB_PATH to the built library)")

try:
    from src.miner.cosmic_harmony_v3_gpu import CosmicHarmonyV3GPU, GPU_AVAILABLE as CV3_GPU
    if CV3_GPU:
    ...
    try:
        ...
        if CV3_GPU:
            _cosmic_v3_gpu = CosmicHarmonyV3GPU(batch_size=500_000)
            COSMIC_V3_GPU_AVAILABLE = True
            print(f"[OK] Cosmic Harmony v3 GPU loaded (fallback)")
    except Exception as e:
...
class StratumClient:
    ...
    def __init__(self, pool_host: str, pool_port: int, worker_id: str = "worker"):
        ...
        # Some pools validate Cosmic Harmony state0 with different endianness.
        # For the native Rust pool v2.9.5 (ShareValidator::new("little")), the
        # correct default is little-endian.
            ...
            if isinstance(result, dict):
                ...
                if isinstance(job, dict):
                    ...
                    self.current_job = {
                        ...
                        "algo": job.get("algo"),
                        "cosmic_state0_endian": job.get("cosmic_state0_endian"),
                    }
                    try:
                        while True:
        ...
        if method == 'mining.notify':
            ...
            if isinstance(params, dict):
                job = {
                    ...
                    'cosmic_state0_endian': params.get('cosmic_state0_endian'),
                }
                try:
                    while True:
                        self.job_queue.get_nowait()
                ...
                if message.get('error'):
                    ...
                    try:
                        ...
                        if (
                            ...
                            # If it should pass under LE but is rejected, and BE would fail,
                            # assume pool uses BE for state0.
                ...
                if accepted:
                    ...
                    try:
                        if (
                            ...
                            # Since we only submit shares that passed our local check,
                            # a reject here is strong evidence of an endian mismatch.
                    ...
                    # Rate-limited reject logging (mirrors the structured error path).
                    try:
                        now = time.time()
                        if (now - self._last_reject_log_time) >= 10.0:
                            suffix = ""
...
class ZionNativeMiner:
    ...
    def hash_single_cpu(self, data: bytes, nonce: int, height: int = 0) -> bytes:
        ...
        For Cosmic Harmony v1: nonce is XORed with height to match core/pool.
        """
        algo = self.config.algorithm
        ...
        if algo == Algorithm.COSMIC_HARMONY:
            ...
            # Match core/pool semantic: effective_nonce = nonce32 ^ height32.
            effective_nonce = (nonce & 0xFFFFFFFF) ^ (int(height) & 0xFFFFFFFF)
            ...
            if 'cosmic_harmony' in self.loader.libs:
                if "cosmic_harmony_cpp" not in self._hash_impl_logged:
                    logger.info("⚙️  Cosmic Harmony: using native C++ hasher (with height XOR)")
                    self._hash_impl_logged.add("cosmic_harmony_cpp")
                lib = self.loader.libs['cosmic_harmony']
                input_array = (ctypes.c_uint8 * len(data)).from_buffer_copy(data)
                output_array = (ctypes.c_uint8 * 32)()
                lib.cosmic_hash(input_array, len(data), effective_nonce, output_array)
            ...
            # Check if we have Python wrapper
                ...
                if "cosmic_harmony_wrapper" not in self._hash_impl_logged:
                    ...
                    if self.cosmic_hasher.cpp_lib:
                        logger.info("⚙️  Cosmic Harmony: using wrapper (C++ library, with height XOR)")
                    else:
                        logger.info("⚙️  Cosmic Harmony: using wrapper (Python fallback, with height XOR)")
                    self._hash_impl_logged.add("cosmic_harmony_wrapper")
                return self.cosmic_hasher.hash(data, effective_nonce)
            else:
    ...
    def mine_to_pool(self, duration: Optional[float] = None):
        ...
        while True:
            ...
            def _update_job_from_stratum(job: Dict[str, Any]):
                ...
                pool_cosmic_endian = job.get("cosmic_state0_endian")
                try:
                    pool_difficulty = int(pool_difficulty_raw or 0)
                except Exception:
                    pool_difficulty = 0

                # If pool provides Cosmic endianness hint, apply it.
                # This must match pool share_validator (state0 interpretation).
                if isinstance(pool_cosmic_endian, str) and pool_cosmic_endian in ("little", "big"):
                    prev = getattr(stratum, "cosmic_state0_endian", None)
                    stratum.cosmic_state0_endian = pool_cosmic_endian
                    if prev != pool_cosmic_endian:
                        logger.info(f"🧭 Pool set Cosmic state0 endianness={pool_cosmic_endian}")

                if pool_difficulty <= 0 and pool_target_raw:
                    try:
                        target_hex = str(pool_target_raw)
                        target_hex = target_hex.strip().lower().removeprefix("0x")
                        # ZION pool encodes job.target as a normal hex integer.
                        # Prefer integer parsing (not XMRig LE-bytes semantics).
                        if target_hex and all(c in "0123456789abcdef" for c in target_hex):
                ...
                if self.config.algorithm == Algorithm.RANDOMX:
                    ...
                    if self.config.algorithm in [Algorithm.COSMIC_HARMONY, Algorithm.COSMIC_HARMONY_V4_2, Algorithm.COSMIC_HARMONY_V2, Algorithm.COSMIC_HARMONY_V3]:
                        ...
                        # Pool share_validator for Cosmic uses: state0(u32) <= target(u32).
                        if pool_target_raw:
                            try:
                                target_hex = str(pool_target_raw).strip().lower().removeprefix("0x")
                                if target_hex and len(target_hex) <= 8 and all(
                                    c in "0123456789abcdef" for c in target_hex
                                ):
                                    target_cosmic32 = int(target_hex, 16) & 0xFFFFFFFF
                            except Exception:
                                target_cosmic32 = None

                        # Fallback: derive exactly like the pool: floor(u32::MAX / diff).
                        if target_cosmic32 is None:
                            target_cosmic32 = 0xFFFFFFFF // max(1, int(difficulty))
                    else:
                        target_cosmic32 = None
                ...
                with job_lock:
                    ...
                    prev_target32 = job_state.get("target_cosmic32")

                    # Update not only on job_id changes. XMRig pools can push
                    # difficulty/target updates (VarDiff) with the same job_id.
                    should_update = (
                        prev_job_id != job_id
                        or prev_blob_hex != blob_hex
                        or prev_diff != difficulty
                        or prev_height != height
                        or prev_target32 != target_cosmic32
                    )
                    ...
                    if should_update:
                        ...
                        # Only reset the nonce cursor when the actual work template changes
                        # or when the pool indicates a clean job. Avoid resetting on pure
                        # difficulty/target retargets.
                        reset_nonce = bool(job.get("clean_jobs")) or (prev_blob_hex != blob_hex) or (prev_job_id != job_id)
                        if reset_nonce:
                            with nonce_lock:
                                nonce_cursor = 0
...
def main():
    ...
    parser.add_argument("--processes", type=int, default=1,
                       help="Number of miner processes (Cosmic Harmony: scales CPU safely when C++ is thread-unsafe). Each process uses 1 CPU thread and worker suffix -pN")
    ...
    # Multi-process launcher (primarily for Cosmic Harmony v1 CPU scaling).
    if int(getattr(args, "processes", 1) or 1) > 1 and not args.benchmark:
        procs = int(args.processes)
        if args.algorithm != "cosmic_harmony":
            safe_print("⚠️  --processes is currently supported only for --algorithm cosmic_harmony")
        elif not args.pool or not (args.wallet or ""):
            safe_print("❌ --processes requires --pool and --wallet")
            return 1
        else:
            safe_print(f"🚀 Spawning {procs} mining processes (CPU-only, 1 thread each)")
            safe_print("   Reason: Cosmic Harmony C++ is not thread-safe after initialize; processes scale safely.")
            ...
            base = [
                ...
                "--ui",
                str(args.ui or "lines"),
                "--processes",
                "1",
            ]
            if args.duration is not None:
                base += ["--duration", str(args.duration)]

            stats_file_base = args.stats_file or ""
            children: List[subprocess.Popen] = []
            try:
                for i in range(procs):
                    worker = f"{args.worker}-p{i+1}"
                    cmd = base + ["--worker", worker]
                    if stats_file_base:
                        root, ext = os.path.splitext(stats_file_base)
                        cmd += ["--stats-file", f"{root}-p{i+1}{ext or '.json'}"]
                    children.append(subprocess.Popen(cmd, env=os.environ.copy()))

                exit_codes = [p.wait() for p in children]
                return 0 if all(code == 0 for code in exit_codes) else 1
            except KeyboardInterrupt:
                safe_print("\n⏹️  Stopping child miner processes...")
                for p in children:
                    try:
                        p.terminate()
                    except Exception:
