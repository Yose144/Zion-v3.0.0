...
>
> **Update (2026-08-27):** **ZionDex + ZIS multichain wallet — dokumentace srovnána s kódem, per-chain decimals opraveny a Rust E2E test deposit → swap → withdraw přidán.** `ZionDexZis.md` a `StatusV3.md` nyní reflektují, že Fáze 0–7 jsou ve `main`, HTLC lock/claim/refund implementovány. Přidána `ChainId::decimals()` do `zion-l1-types`; hardcoded `8`/`0`/`6` decimaly nahrazeny per-chain nativními hodnotami a zachováním `AssetId` contractu. Nový test `V31/L2/multichain/tests/service.rs::e2e_deposit_swap_withdraw` pokrývá core custodial flow. Zbývají: UI E2E (Cypress/Playwright), reálný on-chain testnet/mainnet E2E a operátorská dokumentace. `cargo test -p zion-multichain` 584 passed, `cargo clippy -p zion-multichain` čisté.
>
