...
> **Update (2026-08-05):** **Fáze D E2E COMPLETE.** `cargo test --workspace` 2079 testů pass. V31 miner na Edge našel a odevzdal pool share s `WALLET.worker` autentizací (`zion1g5u0m3j5x5w2t730c8s4h4m5a5v4a7p6p0c07y7.v31-miner`), pool zapsal `submitBlock` accepted, block height 50+. Web `/api/health` `ok`, rpc_node i mining_pool healthy, e2e API scénáře pass. Dashboard V31 služby/logy nasazeny. Git tag `v3.1.0-alpha.2-phase-D`. Následně proběhlo více rychlých rebuildů/restartů služeb; pozor na `fail2ban`/`ufw` ban při masivních lokálních restartech.
