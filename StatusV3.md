...
> **Datum poslední aktualizace:** 2026-09-25
>
> **Update (2026-09-25):** **DAO treasury truth-sync + Zlatý dům / DAO Parlament UI nasazeny na Edge.** `GET /api/dao/treasury` nyní fail-closed rozlišuje pozorovaný UTXO zůstatek od skutečně utratitelné částky: při lead ověření L1 height **57 826**, treasury **1 500 000 000 ZION UTXO**, `unlock_height=144000`, `time_locked=true`, `admin_unlock_known=false`, `admin_unlocked=false`, `spendable=false`, `spendable_zion=0`. Produkční L1 ještě nemá `getAdminUnlocks`; `unknown` je očekávaný bezpečný stav do samostatného rollout premine-unlock node binárky. Web `/dao` má nový VISION tab **Parlament**: čtyři komory (Péče, Opravy, Poznání, Horizontu), Hiran jako „zrcadlo + drafty“ (**0 hlasů, 0 podpisových klíčů, lidský sponsor povinný**) a Síť Země s **144 000 jako symbolickým cílem**, ne live countem, capem ani treasury signer setem. Sedm treasury Guardianů je oddělený nakonfigurovaný 5-of-7 approval model; dnešní `treasury_sigs` jsou koordinační/auditní approval records, ne kryptografické podpisy ani L1 broadcast. DAO/L2 UI opravuje humanitární podíl na 5 %, hlasovací váhu na L1 snapshot balance, prototypový stav consent/CoAdmin a chain selector podle live registry — enabled pouze `zion-l1` + `base`, ostatní `Planned`; test tokeny jsou v produkčním swapu skryté. `/api/guardians/stats` zůstává záměrně 501 a Hiran zůstává nenasazený. Legacy `/dex/liquidity` source má vypnuté přidávání likvidity, ale veřejná cesta se nadále permanentně přesměrovává (`308`) na `/multichain#dex` dle `next.config.ts`. Edge build/deploy ověřen: `cargo fmt` PASS, `zion-dao` **81 lib + 2 smoke**, `tsc --noEmit` PASS, Next build **121/121**, `/dao` 200; node/pool/multichain/DAO/web active, BTC swap stále `enabled:false`. Backups: `zion-dao.bak-20260925T152109Z-dao-parliament`, `.next.bak-20260925T152109Z-dao-parliament`. Tento update nahrazuje starší formulace, které approval records označovaly jako hotový kryptografický multisig.
>
> **Update (2026-09-21):** **DAO lifecycle fix + rozšířené API + přepracované DAO UI nasazeny na Edge.** Backend `zion-dao`: nový periodický task v `main.rs` volá `runtime.process_expired()` každých `scan_interval_secs` (min 15 s) — expirované návrhy se už samy sčítají; návrh #1 přešel `Active`→`Failed` (0 hlasů, kvórum nesplněno). `/api/dao/stats` rozšířeno o `awaiting_tally`, `quorum_percent`, `voting_period_days`, `timelock_hours`, `min_vote_weight`, `proposal_threshold`, `guardian_count`, `multisig`, `total_votes_cast`, `unique_voters` — hodnoty se čtou z `dao-mainnet.toml` (15 % / 14 d / 72 h / 10M ZION). `list_proposals` podporuje `?status=&proposal_type=&limit=&offset=` (newest-first, cap 200). Nový `GET /api/dao/treasury/ops` listuje multisig operace vč. podpisů. Frontend `/dao`: `dao-api.ts` přepsán na reálný Rust kontrakt (tagged `kind/data` proposal types, `votes_for`/`is_voting_open`/`voter_count` mapping); create modal má výběr ze všech 5 typů návrhů + live threshold note; `ProposalCard` gatuje vote buttons na `is_voting_open`, zobrazuje „awaiting tally" a expandable voter list; treasury tab ukazuje multisig operace; stat karty čtou live config hodnoty. `core-endpoints.ts` DAO default sjednocen na 8456. Mobile `CONFIG.DAO.API_BASE` → `https://app.zionterranova.com/api/dao` (fungující veřejný proxy; `zionterranova.com/api/dao` je operator allowlist). `cargo test -p zion-dao` 78 pass, `tsc --noEmit` + `next build` čisté (121 stránek), nasazeno a ověřeno na Edge (`/dao` 200, `/api/dao/stats` nová pole, `/treasury/ops` 200). Dokumentace: [`DAO.md`](./DAO.md) — včetně V3.3 extension plánu (proposal detail, kvórum progress, quadratic voting, guardian registry, L5/L6 proposal bridge, on-chain vote UX).
>
> **Update (2026-09-20, latest safety hold):** **WARP BTC swap flow je na Edge vypnutý (`WARP_BTC_SWAP_ENABLED=0`).** Audit zjistil, že ZIS-gated offer endpoint dovoloval volajícímu zvolit oba objemy směny bez server-side quote či operátorského schválení. Při zásahu byl počet aktivních swapů 0 a existoval jen terminální failed rehearsal; po restartu zůstává `zion-v31-multichain` active, `/health=ok` a veřejný i lokální list vrací `enabled:false`. Hardening je nasazený (2026-09-20, backup `warpd.bak-20260920-hardening`, full suite green): samostatný `WARP_BTC_SWAP_OFFER_KEY`/`X-Warp-Key` (live 503 fail-closed), fail-closed polling, redakce backend credentials, strict network/WIF kontrola, bitcoind watch preflight, data-aware failover, skutečný per-IP limiter a úplný insolvency report (live ověřen). Mainnet bitcoind: 608706 bloků / 967828 headers, 34.35 % IBD; L1: height 50916, protokol `3.1.0-alpha`. Re-enable blokuje externí audit, server-side signed quote/pricing/approval, dedikovaný offer key, dokončený IBD/lokální backend, production WIF review a explicitně schválený capped pilot.
>
> **Earlier 2026-09-20 update (historical baseline before safety hold):** **WARP BTC-swap: vlastní bitcoind backend + regtest E2E kompletní + monitoring napojen.** Pruned `bitcoind` 31.1 na Edge (`zion-bitcoind`, IBD ~5 %) + nativní `bitcoind+rpc://` backend v BTC adapteru (`warp/bitcoind_rpc.rs` — esplora→JSON-RPC překlad, watch-only `warpwatch`, auto `importdescriptors`, esplora failover zachován). Testnet3 rehearsal (dead-end na faucetech) nahrazen **regtest rehearsem na vlastní nodu** (`zion-bitcoind-regtest`, RPC 18443): **6/6 `btc_swap_flow` e2e PASS přes bitcoind backend** — oba směry settled, oba refund paths, restart recovery; live-ZION leg znovu SETTLED na mainnet L1 (lock `fa193379…`, claim `72abda00…`). Monitoring: `/v1/multichain/swaps/btc/metrics` emituje Prometheus text (`warp_btc_swap_*`), Edge Prometheus scrape + alert rules + Grafana provisioned rules zapnuty. **Robinhood Chain Phase B code-prep hotový** — `DeployRobinhood.s.sol` forge script (deterministické adresy ověřeny `cast compute-address`: wZION `0x0c49…bb6`, bridge `0xa5a0…1721`, threshold 4/5), `wzion_contract("robinhood")` + `BRIDGE_CONTRACTS_ROBINHOOD` doplněny; deploy čeká jen na ETH funding deployera na chain 4663. `zion-multichain` 666 testů green. Zbývá: externí audit → mainnet dust pilot (~100k sats) → Grafana notifikační kanál (operátor).
>
> **Update (2026-09-14):** **DAO UI + treasury API live, ZIS-gated governance, miner/reconciliation fixes nasazeny.** `/dao` je opět skutečná stránka (redirect na `/multichain#governance` odstraněn); `zion-dao` API doplněno o `GET /api/dao/treasury` (live L1 UTXO součet přes genesis treasury adresy 7+8 = **1.5B ZION**, ne statické konstanty) a `POST /treasury/submit|sign|execute` s guardian multisig 5-of-7 + perzistentní `treasury_sigs` tabulkou. Governance mutace (vote/create) přijímají `zion_session` cookie — identita a váha hlasu se resolvují server-side přes ZIS `/api/auth/me` + L1 balance ve snapshot bloku (client-supplied voter/weight se u session cesty ignoruje); operator `X-DAO-Key` zůstává. `VoteChoice` deserializace sjednocena na lowercase pro web. `/dao` stránka používá `useAuth()` místo fake `zion1demo…` identit. Vytvořen první produkční návrh #1 (ratifikace treasury 5-of-7). Produkční `/etc/zion/dao-mainnet.toml` opraven na správné treasury adresy + `DAO_CONFIG` env v systemd. Miner: reportuje skutečný počet PoW evaluací (~140 H/s místo nadhodnocených ~400 kH/s), stale-share `unknown_job` eliminován kontrolou `latest_zion_job_id` před submitem, tuning `Nice=19`/`CPUWeight=10`/IO idle (předchozí drop-in dával mineru *vyšší* prioritu než službám). Reconciliation sčítá funded deposit adresy, ne jen hot wallet; EVM provider má interní retry+backoff na public Base RPC rate-limity. Nginx/fail2ban/ufw doplněny o aktuální operátorské IP. `cargo test -p zion-dao` 78 passed, `zion-miner` 105, `zion-multichain` čisté; `next build` OK; vše nasazeno na Edge a ověřeno (treasury/stats/proposals endpointy + `/dao` 200). **Čeká na klíče:** dedikovaný Base RPC (Alchemy/QuickNode) do `warp.toml` — veřejný endpoint sdílí rate-limit budget; npm publish `zion-wallet-sdk` — expired npm token. **Solana watcher:** deposit polling kompletní (ATA discovery přes `getTokenAccountsByOwner`, outer+inner SPL transfer parsing, slot-based confirmations, dedup `user:tx_hash:asset` v DB); chain `enabled=false` — čeká na reálný ZION SPL mint; outbound payments `Unsupported`.
>
...
>
> **Update (2026-08-27):** **ZionDex + ZIS multichain wallet — dokumentace srovnána s kódem, per-chain decimals opraveny, Rust E2E test deposit → swap → withdraw přidán a Playwright smoke test `/wallet/multichain`.** `ZionDexZis.md` a `StatusV3.md` nyní reflektují, že Fáze 0–7 jsou ve `main`, HTLC lock/claim/refund implementovány. Přidána `ChainId::decimals()` do `zion-l1-types`; hardcoded `8`/`0`/`6` decimaly nahrazeny per-chain nativními hodnotami a zachováním `AssetId` contractu. Rust test `V31/L2/multichain/tests/service.rs::e2e_deposit_swap_withdraw` pokrývá core custodial flow. Playwright test `APP&WEB/website-v2.9/e2e/wallet-multichain.spec.ts` ověřuje rendering stránky, sign-in prompt a navigaci. Zbývají: plnohodnotný UI E2E flow (deposit → swap → withdraw) vyžadující autentizovaný test účet, reálný on-chain testnet/mainnet E2E a operátorská dokumentace. `cargo test -p zion-multichain` 584 passed, `npx playwright test e2e/wallet-multichain.spec.ts` 3 passed.
>
...
>
> **Update (2026-08-23):** **Fáze 7 backend hardening + frontend UI pushnuto na `origin/feat/ziondex-zis-multichain-wallet` (`8b1246091`).** Zůstávají otevřené: E2E testy (Cypress/Playwright) deposit → swap → withdraw a operátorská dokumentace.
>
> **Update (2026-08-23):** **ZionDex + ZIS Multichain Wallet Fáze 0–7 hardening a UI implementovány, testy a build zelené.** Feature branch `feat/ziondex-zis-multichain-wallet` nyní obsahuje oddělený wallet seed (`ZION_WALLET_MNEMONIC`), `multichain_wallet` modul, per-user deposit adresy (`/v1/wallet/derive`), ledger (`/v1/wallet/balance`), `DepositWatcher` pro BTC deposit detection, `SwapExecutor` pro `/v1/swap/execute-v2` s on-chain ERC-20 settlement, `WithdrawalProcessor` pro `/v1/wallet/withdraw` a **cross-chain DEX** s `BridgeRegistry`, decimal-aware `DexRouter` a ZION/BTC AMM pool. Phase 7 přidala audit log (`V31/L2/multichain/src/audit.rs`), per-user rate limiting (`V31/L2/multichain/src/rate_limit.rs`), nové wallet endpointy `GET /v1/wallet/me/orders/deposits/withdrawals`, Next.js proxy `/api/multichain/[...path]`, `MultichainWalletContext`, `/wallet/multichain` stránku a prolink z `/wallet`. `cargo test -p zion-multichain` prochází 582 testy. `APP&WEB/website-v2.9` `npm run build` úspěšně (114 stránek včetně `/wallet/multichain`). Detailní plán a stav v [`ZionDexZis.md`](docs/3.2/ZionDexZis.md).
>
> **Update (2026-08-23):** **ZionDex + ZIS Multichain Wallet plán vytvořen.** Detailní E2E plán pro plnohodnotný DEX, multichain peněženku a settlement pod ZIS je v [`ZionDexZis.md`](docs/3.2/ZionDexZis.md). Řeší per-user deposit adresy, ledger/UTXO tracking, keyring integraci, quote → settle tok a web UI.
>
...
>
> **Update (2026-08-23):** **Nativní L1 HTLC nasazeno na Edge Mainnet Alpha — E2E lock→claim úspěšný na live serveru.** Nové binárky `zion-node`, `warpd`, `zion-pool` nasazeny na Edge (`62.171.141.136`). `WARP_MNEMONIC` nastaven v edge-environment.sh, keyring adresa fundingována 1000 ZION. HTLC lock (100 ZION, timelock 2099) → `state:"pending"` → block confirmation → claim s 32B preimage → `state:"claimed"` ✅. HTLC API na DEX portu 8454 (`/v1/multichain/swaps/htlc/{lock,claim,refund,pending,escrow,:hash}`). `quick_mine` binary nasazen pro solo mining (pool má stale template bug — merkle root mismatch). Report: [`NATIVE_L1_HTLC_REPORT.md`](docs/3.2/NATIVE_L1_HTLC_REPORT.md). API docs: [`docs/3.0.8/HTLC_API_REFERENCE.md`](./docs/3.0.8/HTLC_API_REFERENCE.md).
>
> **Update (2026-08-22):** **Všechny necommitnuté změny z multichain E2E smoke testu a UTXO builder alignmentu byly zkommitovány a připraveny k pushi:** V31 core soft-fork pro premine locks a coinbase maturity (`5f6b7f2c7`), `zion-wallet-sdk` + mobilní `TransactionBuilder` na V31 native transaction format (`a2ad953e6`), website swap/atomic proxy na port 8454 (`d1b2aac0c`), CLI `zion node start --soft-fork-activation-height` (`53c1fc6e4`) a archivovaný desktop-agent feature detection (`4dc269144`). `cargo test -p zion-core` 311 passed, `APP&WEB/website-v2.9` build OK, working tree je čistý.
>
> **Update (2026-08-22):** **Lokální E2E smoke test `zion-multichain` (`warpd`) proběhl.** WARP `/health`, `/v1/multichain/chains`, `/v1/multichain/height` a `/v1/multichain/contracts` jsou zelené. DEX `/v1/swap/quote`, `/v1/swap/quote/multi`, `/v1/swap/execute` a `/v1/bridge/submit` (burn base → zion-l1) vracejí 200. Intent lifecycle `create` → `solver/register` → `bid` → `settle` je zelený; `execute` vyžaduje reálný bridge signing a vrací 400 v lokálním prostředí bez nasazeného bridge. Detailní report: [`docs/3.1/REPORTS/REPORT_2026-08-22_L2_MULTICHAIN_E2E_SMOKE_TEST.md`](./docs/3.1/REPORTS/REPORT_2026-08-22_L2_MULTICHAIN_E2E_SMOKE_TEST.md). Současně byl commitnut fix desktop agent UTXO builderu na V31 native transaction formát (`7dfd279f0`).
>
> **Doplnění (2026-08-27):** Výše uvedené `claim`/`refund` selhání bylo zastaralé — `chain/adapters/evm.rs` i `chain/adapters/zion_l1.rs` nyní implementují `TransferDirection::Htlc` (`htlc_lock`/`htlc_claim`/`htlc_refund`). `cargo test -p zion-multichain` nyní 583 passed; `tests/service.rs::cross_chain_intent_executes_bridge_hop` a HTLC lock/claim/refund testy procházejí. Reálný mainnet cross-chain intent execution stále čeká na WARP bridge deploy.
>
> **Update (2026-08-22):** **ZIS (ZION Identity Service) nasazen na Edge a UTXO v2 transaction hash fixy začleněny.** `zion-zis.service` je `active` na Edge, `/health` vrací OK na `https://auth.zionterranova.com`. Opraveny `@fastify/rate-limit` verze (`^9.1.0` pro Fastify 4) a systemd `ExecStart` cesta. Wallet SDK (`APP&WEB/zion-wallet-sdk`) nyní používá v2 hash odpovídající `V31/L1/core/src/v3_tx.rs` a odesílá UTXO transakce přes `submitUtxoTransaction`; CLI `wallet send` používá `zion_core::v3_wallet::build_and_sign` s chain tip height. `cargo test -p zion-multichain` prošel 573 testy (1 ignored) po serializaci NEAR env-var testů.
>
> **Update (2026-08-22):** **Gate G2 — non-EVM WARP `disabled_reason` + config-driven registry — CLOSED.** `ChainConfig`, `ChainRegistry`, `WarpError` a `/chains` API nyní nesou `disabled_reason`. `WarpRuntime` staví registry z `warp.toml` místo hardcoded defaults. `warp.example.toml` pro 3.2.0 povoluje pouze `base` (EVM pilot) a `zion-l1`; všechny nedeployované non-EVM chainy (aptos, sui, cardano, cosmos, near, lightning) mají `enabled = false` s explicitním důvodem. `cargo test -p zion-multichain` (574 testů) a `cargo clippy -p zion-multichain` čisté.
>
> **Update (2026-08-22, E2E audit):** **Web E2E audit — Explorer, PPLNS/payout, CZ/EN, výkon.** Explorer (`/explorer` + `/api/blockchain/*`) ověřen end-to-end proti živému Edge node/pool — všechny stránky a API vrací 200 s reálnými daty (height, richlist, mempool, emission, miners, consensus). Nalezené a opravené drobné bugy: (1) offline fallback v `/api/blockchain/stats` a `/api/blockchain/richlist` používal zastaralou hodnotu premine supply (`16.78B` místo `16.78B` — `GENESIS_PREMINE_ZION`); (2) `/explorer/api-docs` zobrazovala starou verzi `v3.0.6` místo `v3.2.0`; (3) `core-endpoints.ts` dokumentoval `nodeRpc` jako `127.0.0.1:8443`, ale node ve skutečnosti naslouchá jen na `127.0.0.1:9445` (8443 je jen veřejný nginx TCP proxy) — opraveno, žádný funkční kód to naštěstí nepoužíval. PPLNS a payout systém (`V31/L1/pool`) prošel code review: difficulty-weighted sliding window, 89/5/5/1 fee split, atomic persist/restore, potvrzené payouty na chainu — žádné aktivní bugy nenalezeny, jen menší operational gapy (chybí retry-limit alerting na payout sweep, chybí HTTP endpoint na fee drain) zaznamenané jako budoucí vylepšení. Homepage (`src/app/page.tsx`) přepnuta z `force-dynamic` na ISR `revalidate=60` — bezpečné, protože deploy vždy restartuje `zion-website` (cache se stejně vyčistí), výrazně to ale zlepšuje TTFB mezi deploye. CZ/EN audit nenašel žádné obrácené jazyky ani chybějící klíče; opraven jeden nezabalený hardcoded label (`Multichain` v `HomeQuickLinks.tsx`).
>
...
>
> **Update (2026-08-17):** **LWMA difficulty clamp fix + CPU-only `zion-miner` enumeration fix nasazeny na Edge.** Chain dosáhl height **7000+** (accepted 7001), difficulty kleslo z ~21k na ~16k a block time se ustálil kolem 60s po nasazení nových LWMA konstant v `V31/L1/core/src/difficulty.rs` (`MIN_SOLVE_TIME=6`, `MAX_SOLVE_TIME=360`, ±50% per-block clamp). `zion-v31-node` byl přestavěn a redeploynut s novými konstantami. Současně byl opraven `zion-miner` v `V31/L1/miner/src/gpu/mod.rs` — `detect_gpus()` a `query_gpu_details()` nyní short-circuitují OpenCL/CUDA/Metal když `ZION_GPU_BACKEND=cpu`, takže miner nepanicuje při `ocl::Platform::list()` na CPU-only serveru. `zion-v31-miner` běží v CPU-only režimu a produkuje ZION share s 100% accept rate. Všechny V31 služby (node, pool, multichain, DAO, OASIS, dashboard, web, marketplace) jsou active. Root dokumentace vyčištěna: historické 3.1 reporty archivovány do [`docs/3.1/REPORTS/`](./docs/3.1/REPORTS/), kanonická 3.2 roadmap vytvořena v [`docs/3.2/ROADMAP.md`](./docs/3.2/ROADMAP.md).
>
> **Update (2026-08-11):** **Edge V31 audit + hardening COMPLETE.** `ANKR_API_KEY` je legacy klíč pro starý V3 bridge — `zion-v31-multichain` ho nepotřebuje a běží na veřejných RPC endpointech (`base.org`, `arbitrum.io/rpc`, Solana public atd.). Doporučeno odstranit `ANKR_API_KEY` z `/etc/zion/edge-environment.sh` a 1Password. Nasazeny nové binárky `zion-node`, `zion-pool`, `zion-dao`, `zion-oasis` s opravami: `getStatus` nyní vrací V31 `chain_height` a `protocol_version=3.1.0-alpha`; OASIS `blockchain_listener` správně parsuje `subsidy_zion` jako float a používá `reward` (flowers); `zion-pool` HTTP API nyní binduje `127.0.0.1:8080`; `zion-dao` HTTP API binduje `127.0.0.1:8456`; nginx `/api/dao` proxy opraven na `127.0.0.1:8456`; `zion-v31-multichain` restartován a active. `zion-v31-miner` je zastaven a disabled na Edge (CPU-only server nemá OpenCL/CUDA, binárka padá při enumeraci). Edge env/config zabezpečeny: `chmod 600` na `/etc/zion/edge-environment.sh`, `/etc/zion/config/dao-v31.toml`, `/etc/zion/xmr-pool-environment.sh` a `MarketPlace/.env`; `api_key` odstraněn z `dao-v31.toml` a načítán z `ZION_DAO_API_KEY` env; záloha `edge-environment.sh.bak-20260806` s vyzrazeným ANKR klíčem smazána. Operátorská IPv6 `2a00:11b1:10e2:af49:b90b:20ed:4eee:b48b` a IPv4 `46.135.81.225` přidány do `fail2ban` `sshd` ignoreip a `ufw`. Chain height ~1617+, pool a OASIS healthy. Podrobný audit: [`REPORT_2026-08-10_EDGE_V31_AUDIT.md`](./docs/3.1/REPORTS/REPORT_2026-08-10_EDGE_V31_AUDIT.md); deploy report bude doplněn.
>
> **Update (2026-08-10):** **VRSC share rejection eliminated + performance tuning +14% hashrate.** Dva bugy způsobovaly VRSC share reject (~86-97% accept rate): (1) race condition na sdíleném `ext_result_rx` kanálu — VRSC a ZANO výsledky se zaměňovaly; fix: per-coin kanály (`vrsc_result_rx`/`zano_result_rx`) + stale result draining. (2) `skip_stale` check `job_rx.has_changed()` se aktivoval při každém novém V3 job bundlu (i ZION-only) → validní VRSC share přeskočeny; fix: kontrola pouze když se změnil `cpu_external.job_id`. Performance tuning: `ZION_EXT_GPU_GAP_MS=0` (yield_now místo 50ms sleep — CUDA driver scheduling), `ZION_NONCE_COUNT=10M`, `ZION_EXT_CPU_NONCE_COUNT=10M`. Výsledek: **22-24 MH/s total (ZION 3.14 + ZANO 9.13 + VRSC 11.77), 100% accept rate napříč všemi 3 streamy**. Commity: `9cbaa6302` (VRSC fix), `4f6f8dd89` (perf tuning). `work_size=8192` testován ale horší (18-20 MH/s, VRAM bandwidth pressure) — reverted na 4096. GPU power limit 140W (throttle na 1493 MHz místo 1683 MHz boost) — pro increase na 170W potřebný sudo. Kompletní report: [`REPORT_2026-08-10_VRSC_REJECTION_FIX_AND_TUNING.md`](./docs/3.1/REPORTS/REPORT_2026-08-10_VRSC_REJECTION_FIX_AND_TUNING.md).
>
...
|---------|---------|------|-------|--------|
| zion-v31-node | 8335 (P2P), 9445 (RPC) | P2P 0.0.0.0, RPC 127.0.0.1 | L1 (V31 PRODUCTION) | active (live height: query `getChainInfo`; public RPC 8443 / 443 -> 9445) |
