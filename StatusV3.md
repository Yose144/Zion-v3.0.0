...
> **Update (2026-08-11):** **Edge V31 audit přes IPv6 — height 1595, multichain inactive, miner crash loop, fail2ban lockout.** Připojení přes IPv6 `2a02:c207:2342:5821::1` potvrzeno (dokud nebyla IP zabanována při auditu). Služby `zion-v31-node`, `zion-v31-node2`, `zion-v31-node3`, `zion-v31-pool`, `zion-v31-dao`, `zion-v31-oasis`, `zion-website`, `zion-oasis-web`, `zion-marketplace` a `zion-edge-python-dashboard` běží. `zion-v31-multichain` je `inactive (dead)` od 2026-08-10 10:19 CEST. `zion-v31-miner` je v restart smyčce — binárka `zion-miner` sestavená s `gpu-opencl` padá na `ocl::Platform::list` na CPU-only serveru; `zion-universal-miner` CPU-only test OK a přijal share z poolu. `getChainInfo` vrací `native_chain_height: 1595`. Disk 43G/145G (30 %), RAM 1.6G/7.8G. Aktuální operátorská IP `46.135.81.225` / `2a00:11b1:10e2:af49:b90b:20ed:4eee:b48b` byla přidána do `V31/AGENTS.md` `OPERATOR_IPS` a musí být whitelisted na Edge, jinak `fail2ban` `sshd` jail dává 24h ban. Podrobný audit: [`REPORT_2026-08-10_EDGE_V31_AUDIT.md`](./REPORT_2026-08-10_EDGE_V31_AUDIT.md).
>
