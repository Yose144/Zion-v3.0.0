...
>
> **Update (2026-08-29):** **Web UI `/dex` swap widget opraven a E2E pokrývá quote → execute flow.** Příčina runtime chyby `Cannot access 'es' before initialization` v `CrossChainSwapWidget` byla TDZ — `minOutput` byl deklarován až po `useCallback` pro `executeSwap` a přitom byl v jeho dependency array. `minOutput` přesunut před callback, `slippageBps` přidán do dependency array. Přidán `e2e/multichain-dex-flow.spec.ts` s mockovanými `/api/swap/quote/multi` a `/api/swap/execute-v2`, který ověřuje quote a same-chain swap wZION → USDT. Dohromady web E2E (`e2e/`) nyní 8/8 passed. Build a clippy workspace jsou čisté.
>
