...
>
> **Update (2026-08-27):** **ZionDex + ZIS multichain wallet — dokumentace srovnána s kódem a per-chain decimals opraveny.** `ZionDexZis.md` a `StatusV3.md` nyní reflektují, že Fáze 0–7 jsou ve `main`, HTLC lock/claim/refund implementovány a zbývají E2E testy s operátorskou dokumentací. Přidána `ChainId::decimals()` do `zion-l1-types`; hardcoded `8`/`0`/`6` decimaly v reconciliation, deposits, withdrawals, server, zion-l1 adaptéru a DEX executor/intent_engine/aggregator nahrazeny per-chain nativními hodnotami a zachováním `AssetId` contractu. `cargo test -p zion-multichain` 583 passed, `cargo clippy -p zion-multichain` čisté.
>
