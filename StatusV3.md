...
| XMR | randomx | Stratum | ✅ Native RandomX (tevador/RandomX, JIT + HW AES + AVX2 + huge pages, per-thread VM, 161 H/s 4-core EPYC vs XMRig 245 H/s). Pool E2E BLOCKED: MoneroOcean auto-switches to KawPow (more profitable), all pure-RandomX pools (2miners, SupportXMR, Nanopool, Kryptex) unreachable from Edge server (datacenter IP blocking). See `RandomXReport.md` and [`docs/3.0.5/RANDOMX_PERF_OPTIMIZATION_2026-07-16.md`](./docs/3.0.5/RANDOMX_PERF_OPTIMIZATION_2026-07-16.md) |
...
| **RTM** | GhostRider (15 algos + 6 CN) | Stratum | ✅ OpenCL kernel `ghostrider_kernel.cl` (580 lines, 15 x16r hash algos + 6 CryptoNight variants, 18-step hash chain, 1MB scratchpad per work-item) + GPU dispatch wired (`0e5ef6c40`) + E2E PASS (zpool) |
...
|------|-----------|-------------|
| 07-19 | **EPIC ProgPow share acceptance pipeline complete** — Three-phase fix: (1) dedicated one-shot TLS connection for EPIC submit avoids TLS EOF race with poll loop (`epic_submit_dedicated()`), (2) `pool_io_thread` forwards external_stream jobs directly to GPU/CPU threads (~10s latency instead of ~14s), (3) full ProgPow final hash verification on pool before upstream submit — `ethash_final_hash()` computes `keccak256(keccak512(header || nonce_le) || mix_hash)` without needing DAG, drops GPU kernel u64 pre-check false positives locally instead of wasting ~14-min EPIC share windows on "low difficulty" rejections. Live verified: false positive dropped at 14:37. Report: [`EPIC_PROGPOW_SHARE_FIX_REPORT_2026-07-19.md`](./EPIC_PROGPOW_SHARE_FIX_REPORT_2026-07-19.md) | `736ae9fb7`, `1b7a0d454`, `1683f21ec` |
...
| **Miner autotune + sticky header** | `AUTOTUNE_STICKY_REPORT_2026-07-16.md` | Hardware autotune + Claymore-style sticky header |
| **Miner DAG GPU + VRSC share fix** | `MINER_FIXES_REPORT_2026-07-16.md` | DAG generation exclusively on GPU + ExternalResult share accept fix |
| **EPIC ProgPow share fix** | `EPIC_PROGPOW_SHARE_FIX_REPORT_2026-07-19.md` | Dedicated TLS submit + stale job forwarding + full ProgPow hash verification on pool |
