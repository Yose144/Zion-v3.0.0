...
>
> **Update (2026-08-23):** **ZionDex + ZIS Multichain Wallet Fáze 0–5 implementovány a testy zelené.** Feature branch `feat/ziondex-zis-multichain-wallet` nyní obsahuje oddělený wallet seed (`ZION_WALLET_MNEMONIC`), `multichain_wallet` modul, per-user deposit adresy (`/v1/wallet/derive`), ledger (`/v1/wallet/balance`), `DepositWatcher` pro BTC deposit detection, `SwapExecutor` pro `/v1/swap/execute-v2` s on-chain ERC-20 settlement (`ChainAdapter::transfer_token`), `WithdrawalProcessor` pro `/v1/wallet/withdraw` a **cross-chain DEX** s `BridgeRegistry`, decimal-aware `DexRouter` a ZION/BTC AMM pool. `POST /v1/swap/execute-v2` transparentně provádí multi-hop cesty (např. ZION → wZION → USDC) a cross-chain settlement na cílový řetězec. `cargo test -p zion-multichain` prochází 581 testy. Detailní plán a stav v [`ZionDexZis.md`](./ZionDexZis.md).
>
