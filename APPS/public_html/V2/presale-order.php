...
$order = [
    ...
    'payment' => [
        ...
        'status' => 'pending',
        'variableSymbol' => $data['variableSymbol'] ?? generateVariableSymbol($orderId)
    ],
    'zion' => [
        'wallet' => $walletData,
        'qr' => $qrData ? [
...
function generateVariableSymbol(string $orderId = ''): string
{
    // Extract timestamp from orderId if available (e.g., PRESALE-1767340840-81b95d)
    if ($orderId && preg_match('/-(\d{10})-/', $orderId, $matches)) {
        return $matches[1]; // 10-digit timestamp as VS
    }
    // Fallback: current timestamp (10 digits)
    return (string)time();
}
...
function sendPresaleEmails(array $order): array
{
    ...
    $adminSubject = "🚀 Nová ZION Presale objednávka #{$order['orderId']}";
    $adminBody = <<<EMAIL
========================================
