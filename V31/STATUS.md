...
> **Update 2026-08-23 (L5/L6 fund trackers live on Edge):** `zion-free-world` a `zion-issobella` jsou nasazeny jako systemd jednotky na Edge a aktivně scanují L1 coinbase. L5 běží na `127.0.0.1:8095`, L6 běží na `127.0.0.1:8097` (posunuto z 8096 kvůli portovému konfliktu se ZIS). Nginx proxy, edge-env, health probe a Python dashboard jsou aktualizovány. Veřejný `MissionControlDashboard` a `StatusV3.md` označeny jako active. Detail: [`docs/3.2/REPORTS/REPORT_2026-08-23_L5_L6_G10_DETAIL.md`](../docs/3.2/REPORTS/REPORT_2026-08-23_L5_L6_G10_DETAIL.md).
>
> **Update 2026-08-23 (G8 30-day continuous run 🔄 + web UI fix):** Gate G8 **spuštěna** 2026-08-23 07:00 CET, cílový konec 2026-09-22 07:00 CET. Veřejný status UI je na `https://app.zionterranova.com/g8` (také `/dashboard` 301 přesměruje na `/g8`). Předcházející 502 na `app.zionterranova.com/dashboard` bylo způsobeno zaseknutým `zion-website.service` (stuck `deactivating`) — service restartována a nyní 200. Přidána `/g8` a `/api/g8` endpoint do Python dashboardu (`zion-edge-python-dashboard`) s public G8 status stránkou; nginx na `app.zionterranova.com` proxy `/g8` a `/api/g8` na `127.0.0.1:8766`. Stav běží z `/opt/zion/data/g8_run.json` a počítá elapsed/remaining/progress. Gate **G8** a úkol **F6** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označeny jako 🔄 In progress.
>
> **Update 2026-08-22 (G11 V3→V31 migration ✅):** Gate G11 dokončena. Phase H remediation: H1 Foundry testy (43 tests pass — wZION 22, ZIONBridge 12, ZDXToken 9) + `zion deploy` napojeno na `forge create/script/test/verify` s chain presets; H2+H3 miner TUI a Cargo features verifikovány (`tui`, `public_build`, `gpu-cuda,native-all,tui` všechny build OK na Linuxu); H4 `zion update now` opraveno — stahuje z GitHub releases API, najde platform-specific asset, backupne starou binárku a nainstaluje novou. H6/H7/H8 (Stratum v2, PPS+SOLO, pool proxy) dokumentovány jako post-3.2. CLI build `cargo build --release -p zion-cli` prošel (4m43s). Report: [`docs/3.2/REPORTS/REPORT_2026-08-22_G11_V3_V31_MIGRATION.md`](../docs/3.2/REPORTS/REPORT_2026-08-22_G11_V3_V31_MIGRATION.md). Gate **G11** a úkoly **H1–H4** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označeny jako ✅ Complete.
>
> **Update 2026-08-22 (E4 Bridge Base mainnet round-trip ✅):** Kompletní E4 bridge round-trip test úspěšně dokončen: 100 ZION locked na L1 (TX `b7f227a6...` @ block 13184) → 100 wZION minted na Base → 100 wZION burned (TX `0xa5148c44...`, burn_id `0xebfce5b8...`) → 100 ZION unlocked na L1 (TX `9f3e654e...` @ block 13217), potvrzeno v `v3_utxos` tabulce. Opraveny: V31 RPC relay format handler v `v3_rpc.rs`, bridge vault UTXO sync v `rpc.rs`, V3 compat mempool flush po native block, systemd env vars drop-in pro bridge validator pubkeys, a vytvořen standalone `zion-bridge-unlock` binary pro přímé podepsání a odeslání unlock TX (bypassing relay's `is_burn_released` check). Report: [`docs/3.2/REPORTS/REPORT_2026-08-22_E4_BRIDGE_ROUND_TRIP.md`](../docs/3.2/REPORTS/REPORT_2026-08-22_E4_BRIDGE_ROUND_TRIP.md). Úkol **E4** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označen jako ✅ Complete.
>
> **Update 2026-08-22 (Daily summary + F2 fuzz spuštěn):** Denní report vytvořen: [`docs/3.2/REPORTS/REPORT_2026-08-22_DAILY_SUMMARY.md`](../docs/3.2/REPORTS/REPORT_2026-08-22_DAILY_SUMMARY.md). Shrnuje dnešní práci: G5/E8 XMR/RandomX ✅, H5 AuxPoW E2E ✅, G7 chaos/load ✅, F2 24h transaction fuzz spuštěn na pozadí (PID 1235651, cíl `127.0.0.1:8446`, concurrency 10, report do `docs/3.2/REPORTS/fuzz_logs/`). Celkem 48 commitů dnes, všechny pushnuty na `origin/main`. Zbývají: F2 dokončení (24h), G8 (30d continuous run), G9 (security audit), G10 (L5/L6 decision), G11 (V3→V31 migration), Phase G release, Phase I ZIS, Phase J cross-app.
>
> **Update 2026-08-22 (H5 AuxPoW E2E test script):** Vytvořen a lokálně ověřen Python harness `scripts/ops/auxpow_e2e_test.py`. Spouští mock CryptonoteStratum upstream, lokální `zion-pool` (s per-coin override `ZION_POOL_AUXPOW_POOL_XMR` přes nový `CoinProfile::with_pool_address`) a CPU-only `zion-miner` s `ZION_MINER_CPU_COIN=XMR`. Test potvrdil, že alespoň jeden AuxPoW share projde celou cestou miner → pool → upstream mock. Logy zůstávají v `/tmp/auxpow_e2e_<id>/`. Podrobný report: [`docs/3.2/REPORTS/REPORT_2026-08-22_H5_AUXPOW_E2E_TEST.md`](../docs/3.2/REPORTS/REPORT_2026-08-22_H5_AUXPOW_E2E_TEST.md). Úkol **H5** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označen jako ✅ Complete.
>
> **Update 2026-08-22 (G5/E8 XMR / RandomX path via MoneroOcean):** Vybrán upstream **MoneroOcean** (`gulf.moneroocean.stream:10001`) pro XMR / RandomX AuxPoW. Stratum handshake (`login` + `job` s `"algo":"rx/0"`) probíhá přes plain TCP. V `zion-miner` opraveno parsování CryptonoteStratum `job` notifikací, kompaktního targetu (8 hex znaků little-endian), přihlašování `wallet.worker` a formátu `submit` (nonce + result). Testy `cargo test -p zion-miner --lib --features default` procházejí (103 pass). Návod na zapnutí XMR na poolu i na lokálním mineru je v [`docs/3.2/REPORTS/REPORT_2026-08-22_G5_E8_XMR_RANDOMX_MONEROOCEAN.md`](../docs/3.2/REPORTS/REPORT_2026-08-22_G5_E8_XMR_RANDOMX_MONEROOCEAN.md). Gate **G5** a úkol **E8** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označeny jako ✅ Complete.
>
> **Update 2026-08-22 (V31 consensus + pool security hardening — Edge redeployed):** Hardened `submit_block` (exact coinbase structure, 2h future-timestamp cap, block/transaction size and count caps), `submit_utxo_transaction` and `block_template` (mempool size validation). Hardened `PayoutSweeper` (skip immature coinbase/premine UTXOs, fee cap 10% of total payout, 64-char hex tx_id validation). Added HTLC `preimage` field and SHA-256 secret hashing. Multichain EVM relay wallet now prefers `WARP_EVM_RELAY_KEY` env with keyring fallback. Binárky `zion-node`, `zion-pool`, `warpd` buildnuty a redeploynuty na Edge (native height 13065 → aktivace soft-forku 13100; služby active, pool has rigs connected). nginx: `/api/rpc` a `/api/dao` omezeny na operátorské IP; dashboard má IP allowlist. Watchdog hardening: skript `/opt/zion/scripts/watchdog.sh` ownnutý `root:root`, systemd jednotka s `ProtectSystem=full`, `PrivateDevices`, `ProtectHome`, `NoNewPrivileges`, `PrivateTmp`. Secrets v `/etc/zion` nastaveny na `600` pro `.sh`/`.env` a `640` pro `.toml`. Tests: `cargo test --workspace` 0 fail, `cargo clippy --workspace` clean.
>
> **Update 2026-08-22 (G7 chaos / load tests):** Spuštěny a úspěšně dokončeny G7 zátěžové a chaos testy: 10 000-miner lokální pool handshake 100 % pass, 10 000-miner Edge connect storm přežit bez vlivu na reálné rigy, DEX `/v1/swap/quote/multi` overload 1 972 req/s při 100 % 200, bridge `/v1/bridge/submit` overload 1 793 req/s bez pádu, P2P reconnect storm OK, 10-minutový transaction fuzz preview (2 280 požadavků, 0 selhání health checku). Plný 24h transaction fuzz zůstává pending podle F2. Detailní report: [`docs/3.1/REPORTS/REPORT_2026-08-22_G7_CHAOS_LOAD_TESTS.md`](../docs/3.1/REPORTS/REPORT_2026-08-22_G7_CHAOS_LOAD_TESTS.md). Gate **G7**, **F3** a **F4** v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označeny jako ✅ Complete (F2 preview).

> **Update 2026-08-22 (G1 GPU/rig E2E closed):** Dva produkční rigy aktivní na Edge poolu `62.171.141.136:8444`: (1) lokální GTX 1070 Ti (`zion-miner` CUDA, ~18 MH/s, 1105 accepted / 10 rejected = 99.1 %), (2) SimpleMining OS rig 518837 (`ZionRig`, AMD RX 5700 XT + Vega 64, ZION ~0.8 MH/s + ZANO ~17 MH/s, 172 accepted / 1 rejected = 99.4 %). Přidán monitorovací skript `scripts/monitor_g1_rigs.py`. Gate G1 a E1 v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označeny jako ✅ Complete.
>
> **Update 2026-08-22 (G3 solver network E2E closed):** Implementována konfigurovatelná podpora pro nezávislé off-chain solvery přes internet: `SolverConfig` + `SolverEntry` v `MultichainConfig`/`WarpConfig`, automatická registrace pre-configured solverů do intent engine, autentizace `/v1/swap/solve` přes `X-Solver-Key`, per-solver API klíče v `HttpSolverClient` a integrační testy ověřující auth + plný intent→bid→execute flow. `cargo test -p zion-multichain` 574 testů pass, `cargo clippy -p zion-multichain` bez nových warningů. Gate G3 v [`docs/3.2/ROADMAP.md`](../docs/3.2/ROADMAP.md) označen jako ✅ Complete; `warp.example.toml` dokumentuje operativní nasazení.
>
> **Update 2026-08-22 (git sync — core, wallet-sdk, mobile-app, website, CLI, archive):** Zbývající necommitnuté změny z multichain E2E smoke testu a UTXO builder alignmentu byly zkommitovány a připraveny k pushi: V31 core soft-fork pro premine locks a coinbase maturity (`5f6b7f2c7`), `zion-wallet-sdk` + mobilní `TransactionBuilder` na V31 native transaction format (`a2ad953e6`), website swap/atomic proxy na port 8454 (`d1b2aac0c`), CLI `zion node start --soft-fork-activation-height` (`53c1fc6e4`) a archivovaný desktop-agent feature detection (`4dc269144`). `cargo test -p zion-core` 311 passed, `APP&WEB/website-v2.9` build OK, working tree čistý.
>
...
>
> **Update 2026-08-22 (V31 premine/coinbase fix — Edge deploy):** Nová binárka `zion-node` (`+ zion-pool/multichain/dao/oasis`) buildnuta na Edge z `/opt/zion/V31`. `ZION_SOFT_FORK_ACTIVATION=13100` nastaveno v `/etc/zion/edge-environment.sh`; restartovány `zion-v31-node`, `zion-v31-node2`, `zion-v31-node3`, `zion-v31-pool`, `zion-v31-multichain`, `zion-v31-dao` a `zion-v31-oasis`. Aktuální V31 native height=12935, aktivace bude za ~165 bloků (při ~60s/block cca 2.75h). Watchdog `zion-v31-watchdog` byl upraven: přidán 300s startup grace, aby nerestartoval node během prvního startu, když se provádí `tx_index`/`UTXO` backfill (node nyní nabíhá ~142s dle logu `RPC listening`). Watchdog nyní reportuje `OK: v31=12935 version=3.1.0-alpha`.
>
