...
pub async fn run(cmd: ExplorerCmd, node_rpc: &str) -> Result<()> {
    match cmd {
            ...
            match node_rpc::call(node_rpc, "getStatus", Value::Null).await {
                Ok(v) => {
                    ...
                    for h in (start..=tip).rev() {
                        if let Ok(bv) =
                            node_rpc::call(node_rpc, "getBlock", serde_json::json!({"height": h}))
                                .await
                        {
                            let hash = bv["result"]["hash"].as_str().unwrap_or("?");
                            let txs = bv["result"]["transactions"]
                                .as_array()
                                .map(|a| a.len())
                                .unwrap_or(0);
                            println!(
        ...
        ExplorerCmd::Block { height } => {
            ...
            match node_rpc::call(node_rpc, "getBlock", serde_json::json!({"height": height})).await
            {
            ...
            .await
            {
                Ok(v) => {
                    if let Some(utxos) = v["result"]["utxos"].as_array() {
                        ...
                        ui::print_row(
                            "Balance",
                            &format!("{} flowers ({} ZION)", total, total as f64 / 1_000_000.0),
                        );
