//! Ekam Deeksha — canonical V31 PoW (Ekam v3.2, 512 KiB scratchpad).
//!
//! Pipeline (must match GPU kernels in `deeksha_lite.cl`, `deeksha_lite.cu`, `deeksha_lite.metal`):
//!   1. Keccak256(header[0..80] || nonce_le[0..8])             → s1[32]
//!   2. Memory-hard scratchpad (512 KiB, 16384 blocks × 32B)
...
//!        Phase D: 128 random reads, idx from 8-byte u64 accumulator
...
// ── Scratchpad reuse ───────────────────────────────────────────────────────
// The original code allocated a full scratchpad on EVERY nonce, causing
// massive allocation pressure (512 KiB × H/s). We now allocate the scratchpad
...
pub const SCRATCHPAD_SIZE: usize = 512 * 1024; // 512 KiB (ASIC-hardened v3.2)
pub const BLOCK_SIZE: usize = 32;
pub const BLOCK_COUNT: usize = SCRATCHPAD_SIZE / BLOCK_SIZE; // 16384
pub const PASSES: usize = 2; // forward + backward (was 1)
pub const RANDOM_READS: usize = 128; // 4× more serial bottleneck (was 32)
...
// 512 KiB on every nonce. The buffer is allocated once per batch/thread.
fn step2_memory_hard_with_scratchpad(seed: &[u8; 32], scratchpad: &mut [u8]) -> [u8; 32] {
    ...
    // Phase C: 128 random reads, idx from 8 bytes (u64) — matches GPU
    let mut acc = [0u8; 32];
...
/// Avoids the 512 KiB allocation per nonce — pass a reusable buffer.
#[inline]
...
// Known-answer test vectors for the canonical Ekam Deeksha v3.2 pipeline.
// Generated by the Ekam Deeksha v3.2 implementation and locked; if these change,
// the CPU↔GPU pipeline is broken.
// v3.2: scratchpad=512 KiB, reads=128, passes=2
pub const LITE_KAT_HEADER: &[u8] = b"ZION_LITE_KAT_V1";
pub const LITE_KAT: &[(&str, u64)] = &[
    (
        "cd2de414f1e9c74e4023cedced65883c5295be2c3c94729c8de8c98518872c73",
        0,
    ),
    (
        "908f214814ccd24c03f2ef16223535c2846862cf1d1ab057e80bd597ba8a21aa",
        1,
    ),
    (
        "6edcfb4c5210b417ccadfd1620fbe51205a425cdae4c58151f14799c6e7290bf",
        42,
    ),
    (
        "c1bca4b577a925c28e6522e4bb9aaa0930ab7de638465074f3e1b591f10c747b",
        0xDEADBEEF,
    ),
    (
        "92865072325f71769b4fd5896f87a9ed7227aa7bf016f87e78587d8a37891b87",
        u64::MAX,
    ),
];
...
mod tests {
    ...
    // ── Ekam v2 canonical test vector (v3.2 ASIC-hardened) ─────────────────
    ...
    fn ekam_v2_canonical_vector() {
        ...
        let expected = "9cd303283063f36f7ea2b696b70b95c294c9143a53b884a1112cd2a7e357e224";
        let got: String = hash.iter().map(|b| format!("{:02x}", b)).collect();
        assert_eq!(
            got, expected,
