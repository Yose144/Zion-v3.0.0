...
pub mod etchash {
    ...
    // ── DAG generation (for GPU ProgPoW/Ethash mining) ──────────────────

    /// RAII wrapper for an Ethash DAG buffer allocated by the C code.
    pub struct EthashDag {
        ptr: *mut u8,
        /// Size of the DAG in bytes (= dag_size_entries * 128).
        pub len: usize,
        /// Number of 128-byte DAG entries.
    ...
    /// RAII wrapper for an Ethash light cache buffer.
    ...
    /// Generate the full Ethash DAG for a given epoch.
    pub fn generate_dag(epoch: u32) -> Option<EthashDag> {
    ...
    pub fn generate_light_cache(epoch: u32) -> Option<EthashLightCache> {
