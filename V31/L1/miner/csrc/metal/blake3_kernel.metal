// Blake3 Metal kernel for Alephium (ALPH) and Decred (DCR) mining.
//
...
//
// Translated from opencl/blake3_kernel.cl.
...
void blake3_inner(
    thread const uchar nonce[24],
    device const uchar *header_blob,
    const uint header_len,
    thread uint inner_out[8]
) {
    uint chain[8];
