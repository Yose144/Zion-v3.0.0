// KarlsenHashV2 OpenCL kernel for Karlsen (KLS) mining.
//
// KarlsenHashV2 = FishHashPlus + Blake3 wrapper:
//   1. PoWHash = Blake3(prePowHash || timestamp || 32_zeros || nonce) → 32 bytes
//   2. seed = PoWHash padded to 64 bytes
//   3. mix = seed || seed (128 bytes)
//   4. 32 rounds of FishHashPlus DAG access:
//      - mixGroup[8] = XOR of 4 uint32 groups from mix
//      - p0 = (mixGroup[0] ^ mixGroup[3] ^ mixGroup[6]) % dagSize
//      - p1 = (mixGroup[1] ^ mixGroup[4] ^ mixGroup[7]) % dagSize
//      - p2 = (mixGroup[2] ^ mixGroup[5] ^ round) % dagSize
//      - fetch3, FNV mix, multiply-add (same as FishHash)
//   5. FNV collapse 128→32 bytes
//   6. finalHash = Blake3(mix_hash) → 32 bytes
//   7. Compare to target
//
// Key difference from FishHash: modified address calculation (FishHashPlus)
// and Blake3 wrapper instead of direct header hashing.
//
// DAG: 37,748,717 items × 128 bytes = ~4.6GB (same as FishHash)
//
// References:
//   - karlsend: https://github.com/karlsen-network/karlsend (fishhashplus_kernel.go)
//   - FishHashMiner: https://github.com/Lolliedieb/FishHashMiner
//   - karlsencoin/karlsen-miner: https://github.com/karlsencoin/karlsen-miner
...
/****************************************************
	   ...
	   Adapted for ZION AuXpow GPU miner:
	   - Output: output_nonce (u64), output_hash (32 bytes),
	     found_flag (u32) — standard ZION pattern
	   - Target: 32-byte big-endian buffer (standard ZION pattern)
...
__attribute__((reqd_work_group_size(WORKSIZE , 1, 1)))
__kernel void karlsenhash_mine (
			__global uint8 * dag,
    ...
    //
    // KLS PoW hash input: prePoWHash(32) || timestamp(8) || zeros(32) || nonce(8) = 80 bytes
    // This takes 2 Blake3 passes: 64 bytes + 16 bytes.
    // The blockHeader buffer contains prePoWHash || timestamp || zeros (72 bytes)
    // in the first chunk, and the nonce is placed in the second chunk.
    ...
    // Blake3: 2 passes for 80-byte input
    for (uint c=0; c<2; c++) {
    	cv = v.lo;
    	m = blockHeader[c];
	...
	if (c == 0) {
	    // First 64 bytes: prePoWHash(32) + timestamp(8) + zeros(24)
	    v.se = (uint) 64;
	    v.sf = (uint) CHUNK_START;
	} else {
	    // Second chunk: zeros(8) + nonce(8) = 16 bytes
	    // Nonce goes at byte offset 8 in this chunk (m.s2..s3)
	    m.s01 = (uint2)(0, 0);
	    m.s23 = as_uint2(nonce);
	    v.se = (uint) 16;
    ...
    // Entering the pointer chasing phase, here 4 threads work together on one hash
    uint8 mix;
    uint8 mixResult;
    ...
    for (uint tid = 0; tid < 4; tid++) {
            ...
            for (uint a = 0; a < 32; a++) {
            	...
            	// Share full 32-byte mix per thread for FishHashPlus address calc.
            	// Only threads 0 and 1 have unique data (threads 2,3 duplicate).
            	if (thread_id < 2) {
                    share->uints[thread_id * 8 + 0] = mix.s0;
                    share->uints[thread_id * 8 + 1] = mix.s1;
                    share->uints[thread_id * 8 + 2] = mix.s2;
                    share->uints[thread_id * 8 + 3] = mix.s3;
                    share->uints[thread_id * 8 + 4] = mix.s4;
                    share->uints[thread_id * 8 + 5] = mix.s5;
                    share->uints[thread_id * 8 + 6] = mix.s6;
                    share->uints[thread_id * 8 + 7] = mix.s7;
            	}

            	barrier(CLK_LOCAL_MEM_FENCE);

            	// FishHashPlus address calculation.
            	// mixGroup[c] = XOR of 4 consecutive uint32s from the 64-byte mix.
            	// Since 128-byte mix = seed||seed, mixGroup[4..7] = mixGroup[0..3].
            	// p0 = mixGroup[0] ^ mixGroup[3] ^ mixGroup[6] = mg0 ^ mg3 ^ mg2
            	// p1 = mixGroup[1] ^ mixGroup[4] ^ mixGroup[7] = mg1 ^ mg0 ^ mg3
            	// p2 = mixGroup[2] ^ mixGroup[5] ^ round       = mg2 ^ mg1 ^ a
            	uint mg0 = share->uints[0] ^ share->uints[1] ^ share->uints[2] ^ share->uints[3];
            	uint mg1 = share->uints[4] ^ share->uints[5] ^ share->uints[6] ^ share->uints[7];
            	uint mg2 = share->uints[8] ^ share->uints[9] ^ share->uints[10] ^ share->uints[11];
            	uint mg3 = share->uints[12] ^ share->uints[13] ^ share->uints[14] ^ share->uints[15];

            	uint p0 = (mg0 ^ mg3 ^ mg2) % dagSize;
		uint p1 = (mg1 ^ mg0 ^ mg3) % dagSize;
		uint p2 = (mg2 ^ mg1 ^ a) % dagSize;
    ...
    // KLS final hash: Blake3(mixHash) — single 32-byte block.
    // Unlike FishHash which hashes (seed || mixResult), KLS only hashes mixResult.
    ...
    v.sd = 0;
    v.se = (uint) 32;
    v.sf = (uint) CHUNK_START | CHUNK_END | ROOT;
    ...
    // The final 32-byte hash is in v.s0..v.s7 (uint, little-endian).
    // Convert to byte array for target comparison and output.
    uchar hash_out[32];
    hash_out[ 0] = (uchar)(v.s0 & 0xFF); hash_out[ 1] = (uchar)(v.s0 >> 8);
    hash_out[ 2] = (uchar)(v.s0 >> 16); hash_out[ 3] = (uchar)(v.s0 >> 24);
    hash_out[ 4] = (uchar)(v.s1 & 0xFF); hash_out[ 5] = (uchar)(v.s1 >> 8);
    hash_out[ 6] = (uchar)(v.s1 >> 16); hash_out[ 7] = (uchar)(v.s1 >> 24);
    hash_out[ 8] = (uchar)(v.s2 & 0xFF); hash_out[ 9] = (uchar)(v.s2 >> 8);
    hash_out[10] = (uchar)(v.s2 >> 16); hash_out[11] = (uchar)(v.s2 >> 24);
    hash_out[12] = (uchar)(v.s3 & 0xFF); hash_out[13] = (uchar)(v.s3 >> 8);
    hash_out[14] = (uchar)(v.s3 >> 16); hash_out[15] = (uchar)(v.s3 >> 24);
    hash_out[16] = (uchar)(v.s4 & 0xFF); hash_out[17] = (uchar)(v.s4 >> 8);
    hash_out[18] = (uchar)(v.s4 >> 16); hash_out[19] = (uchar)(v.s4 >> 24);
    hash_out[20] = (uchar)(v.s5 & 0xFF); hash_out[21] = (uchar)(v.s5 >> 8);
    hash_out[22] = (uchar)(v.s5 >> 16); hash_out[23] = (uchar)(v.s5 >> 24);
    hash_out[24] = (uchar)(v.s6 & 0xFF); hash_out[25] = (uchar)(v.s6 >> 8);
    hash_out[26] = (uchar)(v.s6 >> 16); hash_out[27] = (uchar)(v.s6 >> 24);
    hash_out[28] = (uchar)(v.s7 & 0xFF); hash_out[29] = (uchar)(v.s7 >> 8);
    hash_out[30] = (uchar)(v.s7 >> 16); hash_out[31] = (uchar)(v.s7 >> 24);

    // Big-endian target comparison: hash (as big-endian integer) must be
    // less than target. The hash bytes are already in the right order for
    // big-endian comparison (byte 0 is most significant).
    bool meets = true;
    for (int i = 0; i < 32; i++) {
        if (hash_out[i] < target_buf[i]) break;
        if (hash_out[i] > target_buf[i]) { meets = false; break; }
    }
