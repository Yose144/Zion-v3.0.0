 ...
 *
 *  FFI signature matches AuXpow/src/native_ffi.rs and V3/L1/native-ffi/src/lib.rs:
...
/* ============================================================================
 * FFI compatibility stubs
 *
 *   The correct Autolykos v2 algorithm does not use a pre-computed table;
 *   the 8192-byte "M" array is generated internally by autolykos_hash().
 *   These entry points are retained so the symbols expected by the Rust
 *   native_ffi.rs / lib.rs bindings continue to exist.
 * ============================================================================ */

EXPORT const char* autolykos_version(void) {
    return "ZION Autolykos v2.0.0 - Ergo reference (BLAKE2b, hitForVersion2)";
}
...
EXPORT void autolykos_generate_table(
    ...
    (void)table;
    (void)table_size;
    /* The real Autolykos v2 hash path is autolykos_hash(); no precomputed
       table is required because M is built on demand. */
}
...
EXPORT int autolykos_mine(
    ...
    /* ABI-compatibility stub. The real Autolykos v2 hash path is
       autolykos_hash(); this entry point recomputes at height 0 and
       performs a big-endian 32-byte target comparison. */
    if (!output) return 0;
    (void)autolykos_hash(header, header_len, nonce, 0u, output);
