/*
 * DeekshaLite v1 / DeekshaChv3 — Native CUDA GPU Mining Kernel
 *
 * Ported from the canonical OpenCL kernel (deeksha_lite.cl / deeksha_chv3.cl).
 ...
 *
 * v3 optimizations (same as deeksha_lite_fire.cu):
...
/* ========================================================================== */
/* Main kernel — deeksha_lite_v1 / deeksha_chv3 (NO thermal loop)              */
    ...
    uint8_t *result_hash)
{
    ...
    /* Step 4: Keccak256 final (NO thermal loop — deeksha_lite_v1/chv3) */
    uint64_t st[25];
...
extern "C" __global__ void deeksha_lite_debug(
    const uint64_t *header_keccak_state,
    uint64_t nonce,
    uint8_t *output_hash,
    uint8_t *scratchpad)
{
    ...
    /* NO thermal loop — deeksha_lite_v1/chv3 */
