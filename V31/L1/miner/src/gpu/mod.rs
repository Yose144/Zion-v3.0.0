...
use zion_core::{MiningHeader, MiningJob, MiningSolution, V3DifficultyTarget as DifficultyTarget};
...
pub trait GpuMiner: Send {
    ...
    /// Algorithm this backend mines (e.g. "ekam_deeksha", "ekam_deeksha").
            ...
            match self.ensure_algorithm(algo) {
                Ok(gpu) => match gpu.benchmark(secs) {
                    Ok((hashes, elapsed, khps)) => {
                        tlog!("benchmark_algo={algo} hashes={hashes} elapsed={elapsed:.2}s khps={khps:.2}");
                        results.push((algo.to_string(), khps));
                    }
                    Err(e) => {
                        tlog!("benchmark_algo={algo} error={e}");
                    }
                },
                Err(e) => {
                    tlog!("benchmark_algo={algo} init_error={e}");
                }
            }
        }
        results
...
impl TriGpuManager {
    ...
    pub fn new(kind: GpuBackendKind, primary_work_size: usize) -> Result<Self> {
        ...
        let primary_algo =
            std::env::var("ZION_MINER_ALGORITHM").unwrap_or_else(|_| "ekam_deeksha".to_string());
    ...
    /// Ekam Deeksha algorithm. All V31 pool jobs use `ekam_deeksha`; if the
    /// current primary backend advertises a different name, recreate it so
    /// hashes are submitted under the canonical name.
    ///
    ...
    pub fn ensure_primary_algorithm(&mut self, algorithm: &str) -> Result<()> {
        ...
        // Only switch for canonical Ekam Deeksha jobs.
        let is_deeksha = matches!(algorithm, "ekam_deeksha");
        if !is_deeksha {
            return Ok(());
        }
...
impl MultiGpuMiner {
    ...
    fn update_hashrates(&mut self, results: &[(Result<GpuBatchResult>, u64)]) {
        ...
        if self.batch_count.is_multiple_of(50) {
        ...
        for (i, (result, _)) in results.into_iter().enumerate() {
            match result {
                Ok(r) => {
                    ...
                    if merged.solution_blob.is_none() && r.solution_blob.is_some() {
                        merged.solution_blob = r.solution_blob.clone();
                    }
                }
                Err(e) => {
    ...
    #[cfg(target_os = "macos")]
    {
            ...
            .output()
        {
            if out.status.success() {
                ...
                let model = if let Some(rest) = brand.strip_prefix("Apple M") {
...
fn detect_apple_gpu_cores() -> u32 {
        ...
        .output()
    {
        ...
        for line in s.lines() {
            ...
            if let Some(rest) = trimmed.strip_prefix("Total Number of Cores:") {
                let digits: String = rest.chars().filter(|c| c.is_ascii_digit()).collect();
    ...
    match kind {
        ...
        GpuBackendKind::OpenCL | GpuBackendKind::Auto => {
            #[cfg(feature = "gpu-opencl")]
            {
                ...
                // Ekam Deeksha v3.2 is the canonical V31 PoW.
                match opencl_deeksha_lite::OpenClDeekshaLiteMiner::new(work_size) {
                    Ok(miner) => return Ok(Box::new(miner)),
                    Err(e) => {
                        if kind == GpuBackendKind::OpenCL {
                            anyhow::bail!("Ekam Deeksha OpenCL init failed: {e}");
                        }
                        println!("ekam_deeksha_opencl_unavailable reason=\"{e}\"");
                    }
                }
            }
            #[cfg(not(feature = "gpu-opencl"))]
            ...
            #[cfg(feature = "gpu-cuda")]
            {
                match cuda_deeksha_lite::CudaDeekshaLiteMiner::new(work_size) {
                    Ok(miner) => return Ok(Box::new(miner)),
            ...
            #[cfg(all(feature = "gpu-metal", target_os = "macos"))]
            {
                match metal_deeksha_lite::MetalDeekshaLiteMiner::new(work_size) {
                    Ok(miner) => return Ok(Box::new(miner)),
        ...
        GpuBackendKind::Cuda => {
            #[cfg(feature = "gpu-cuda")]
            {
                ...
                if is_external_algorithm(algorithm) {
                    ...
                    if crate::gpu::cuda_external::CudaExtAlgo::from_name(algorithm).is_some() {
                        let miner_result = if let Some(ref dev) = shared_dev {
                            crate::gpu::cuda_external::CudaExternalMiner::new_with_device(
                                algorithm,
                                work_size,
                                std::sync::Arc::clone(dev),
                            )
                        } else {
                            crate::gpu::cuda_external::CudaExternalMiner::new(algorithm, work_size)
                        };
                        match miner_result {
                            Ok(miner) => return Ok(Box::new(miner)),
                            Err(e) => {
                    ...
                    #[cfg(any(feature = "native-kheavyhash", feature = "native-blake3-algo"))]
                    {
                        ...
                        let miner = crate::gpu::cpu_external_fallback::CpuExternalMiner::new(
        ...
        GpuBackendKind::Metal => {
            #[cfg(all(feature = "gpu-metal", target_os = "macos"))]
            {
                ...
                if algorithm == "progpow_zano" {
                    ...
                    {
                        let miner = crate::gpu::metal_external::MetalExternalMiner::new(
                            algorithm, coin, work_size,
                        )?;
                        return Ok(Box::new(miner));
                    }
                ...
                Ok(Box::new(metal_deeksha_lite::MetalDeekshaLiteMiner::new(
                    work_size,
                )?))
            }
            #[cfg(not(all(feature = "gpu-metal", target_os = "macos")))]
            anyhow::bail!(
                "Metal support not compiled — rebuild with --features gpu-metal on macOS"
            );
        }
    }
}
...
pub fn gpu_scan_job(
    ...
    // for ekam_deeksha, which takes ~3s on RTX 3090 — well within the
        ...
        Ok(result) => {
            ...
            if let Some((nonce, gpu_hash, mix_hash)) = result.solutions.first() {
                ...
                let cpu_hash = if use_raw && algorithm == "blake3_dcr" {
                    crate::auxpow::hash_blake3(raw_header_bytes, 0, *nonce)
                } else {
                    crate::parallel::hash_candidate(&candidate, algorithm)
                };
...
impl Default for GpuPipelineState {
    fn default() -> Self {
        Self::new()
    }
}
...
impl GpuPipelineState {
    ...
    pub fn step(
        ...
        let prev_outcome = if self.has_pending {
            ...
            let _prev_raw = self.prev_raw_header.take().unwrap_or_default();
            ...
            let outcome = match collect_result {
                Ok(result) => {
                    ...
                    if let Some((nonce, gpu_hash, mix_hash)) = result.solutions.first() {
                        ...
                        #[cfg(feature = "auxpow")]
                        let cpu_hash = crate::parallel::hash_candidate(&candidate, algorithm);
                        #[cfg(feature = "auxpow")]
                        let is_mismatch = cpu_hash != *gpu_hash;
    ...
    pub fn collect(&mut self, gpu: &mut dyn GpuMiner, _algorithm: &str) -> Option<GpuScanOutcome> {
        if !self.has_pending {
            return None;
        }
        self.has_pending = false;
...
// ─── OpenCL Backend: Ekam Deeksha v3.2 ─────────────────────────────────────
...
pub mod opencl_deeksha_lite {
    ...
    pub struct OpenClDeekshaLiteMiner {
        ...
        /// Algorithm name advertised by this backend (`ekam_deeksha`).
        algorithm: String,
    ...
    impl OpenClDeekshaLiteMiner {
        ...
        pub fn new(requested_work_size: usize) -> Result<Self> {
            Self::new_with_kernel(requested_work_size)
        }

        fn new_with_kernel(requested_work_size: usize) -> Result<Self> {
            let kernel_src = opencl_kernel::get_ekam_deeksha_kernel_source().to_string();
            let kernel_name = opencl_kernel::EKAM_DEEKSHA_KERNEL_NAME;
            ...
            let output_hashes_buf_b = Buffer::<u8>::builder()
                ...
                .len(actual_work_size * 32)
                .build()?;
            // On-device result buffers for the 10-arg ekam_deeksha kernel.
...
// ─── CUDA Backend: Ekam Deeksha v3.2 ───────────────────────────────────────
...
pub mod cuda_deeksha_lite {
    ...
    const CUDA_KERNEL_SRC: &str = include_str!("kernels/cuda/ekam_deeksha.cu");
    ...
    impl CudaDeekshaLiteMiner {
        ...
        pub fn new(work_size: usize) -> Result<Self> {
            ...
            let arch = crate::gpu::cuda_external::detect_cuda_arch(&dev);
            let mut opts = vec![
            ...
            dev.load_ptx(
                ptx,
                "ekam_deeksha",
                &["ekam_deeksha_mine", "ekam_deeksha_debug"],
            )
    ...
    impl GpuMiner for CudaDeekshaLiteMiner {
        ...
        fn mine_batch(
            ...
            let func = self
                .dev
                .get_func("ekam_deeksha", "ekam_deeksha_mine")
...
// ─── Metal Backend: Ekam Deeksha v3.2 ──────────────────────────────────────

#[cfg(all(feature = "gpu-metal", target_os = "macos"))]
pub mod metal_deeksha_lite;

/// CPU fallback for external AuxPoW algorithms when no GPU kernel is available.
#[cfg(any(feature = "native-kheavyhash", feature = "native-blake3-algo"))]
            ...
            match self.algorithm.as_str() {
                ...
                "kheavyhash" | "kheavyhash_kas" => {
                    for i in 0..actual_batch {
                        ...
                        let hash = zion_native_ffi::kheavyhash::mine(pre_pow_hash, 0, nonce);
            ...
            while start.elapsed().as_secs_f64() < secs {
                for _ in 0..1000 {
                    match self.algorithm.as_str() {
                        #[cfg(feature = "native-kheavyhash")]
                        "kheavyhash" | "kheavyhash_kas" => {
                            let _ = zion_native_ffi::kheavyhash::mine(&header, 0, nonce);
                        }
                        #[cfg(feature = "native-blake3-algo")]
                        "blake3" | "blake3_alph" | "blake3_dcr" => {
...
/// STUB: V31 does not yet expose a generic Metal AuxPoW miner.
...
pub mod metal_external {
    ...
    impl MetalExternalMiner {
        pub fn new(algorithm: &str, _coin: &str, _work_size: usize) -> Result<Self> {
            anyhow::bail!(
                "Metal external AuxPoW mining is not yet ported for V31 (algorithm={algorithm})"
            )
        }
    }
    ...
    impl GpuMiner for MetalExternalMiner {
        ...
        fn mine_batch(
            ...
            anyhow::bail!("Metal external AuxPoW mining is not yet ported")
        }
...
mod tests {
    ...
    fn gpu_pipeline_state_steps_and_collects() {
        ...
        let result3 = pipeline.step(&mut gpu, job3, "ekam_deeksha", &[]);
        let sol2 = result3
        ...
        let final_result = pipeline.collect(&mut gpu, "ekam_deeksha");
