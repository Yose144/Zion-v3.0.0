...
/// Compute the rank of a 64×64 matrix over the reals using Gaussian elimination.
...
impl ExtGpuMiner {
    ...
    pub fn mine(
        ...
        let kernel = match algorithm {
            ...
            "ethash" | "etchash" | "ethash_etc" => {
                ...
                let dag = ethash_dag.ok_or_else(|| {
                    anyhow!(
                        "Ethash DAG not set; call ExtGpuMiner::set_ethash_dag() \
            ...
            | "evrprogpow" | "evrprogpow_evr" | "meowpow" | "meowpow_mewc" => {
                ...
                let dag = kawpow_dag.ok_or_else(|| {
                    anyhow!(
                        "KawPow DAG not set; call ExtGpuMiner::set_kawpow_dag() \
            ...
            "progpow" | "progpow_epic" | "progpow_zano" | "progpowz" => {
                ...
                let dag = progpow_dag.ok_or_else(|| {
                    anyhow!(
                        "ProgPow DAG not set; call ExtGpuMiner::set_progpow_dag() \
            ...
            "fishhash" | "fishhash_iron" | "karlsenhash" | "karlsenhash_kls" => {
                let dag = fishhash_dag.ok_or_else(|| {
                    anyhow!(
                        "FishHash DAG not set; call ExtGpuMiner::set_fishhash_dag() \
        ...
        // takes ~2-4s. In DEDICATED mode (no ZION contention), 1048576 (1M)
        // is safe — 1M/128 = 8192 WGs, 8192/64 = 128 WGs per CU. Tested:
        // 262K → 12.82 MH/s, 524K → 16.54 MH/s (+29%), 1M target → ~18+ MH/s.
    ...
    fn mine_beamhash_solver(
        ...
        let result: Result<Option<GpuFoundShare>> = (|| {
            ...
            for pos in 0..solution_count.min(10) {
                ...
                let hash = [0u8; 32]; // beamhash not available in V31
    ...
    pub fn generate_kawpow_dag_on_gpu(&mut self, epoch: u32) -> Result<()> {
        anyhow::bail!(
            "KawPow DAG generation requires native-hashers feature (epoch={}).",
            epoch
        );
    }
...
"#;
    ...
    pub fn generate_ethash_dag_on_gpu(&mut self, epoch: u32) -> Result<()> {
        // Cache check: skip regeneration if DAG for this epoch already exists
        if let Some(ref dag) = self.ethash_dag {
            if dag.epoch == epoch {
                return Ok(());
            }
        }
    ...
    pub fn generate_progpow_dag_on_gpu(&mut self, epoch: u32) -> Result<()> {
        // Cache check: skip regeneration if DAG for this epoch already exists
        if let Some(ref dag) = self.progpow_dag {
            if dag.epoch == epoch {
                return Ok(());
            }
        }
    ...
    fn ensure_proque_progpow(
        ...
        use crate::auxpow::progpow_codegen;
    ...
    fn mine_verthash(
        ...
        let verthash = self.verthash_data.clone().ok_or_else(|| {
            anyhow!(
                "Verthash data file not loaded; call ExtGpuMiner::set_verthash_data() \
                 before mining VTC"
            )
        })?;
...
impl DagManager {
    ...
    pub fn ensure_ethash_dag(&mut self, miner: &mut ExtGpuMiner, epoch: u32) -> Result<()> {
    ...
    /// previous GPU generation that was saved), it is loaded and uploaded
    /// instead — this is just a host→GPU transfer, not CPU generation.
    pub fn ensure_kawpow_dag(&mut self, miner: &mut ExtGpuMiner, epoch: u32) -> Result<()> {
    ...
    /// instead.
    pub fn ensure_progpow_dag(&mut self, miner: &mut ExtGpuMiner, epoch: u32) -> Result<()> {
...
pub mod opencl_backend {
    ...
    pub fn new(work_size: usize) -> Result<ExtGpuMiner> {
        let mut miner = ExtGpuMiner::new()?;
    ...
    pub fn list_devices() -> Vec<String> {
        ...
        match ExtGpuMiner::list_kernels() {
            Ok(kernels) => {
                if kernels.is_empty() {
                    Vec::new()
                } else {
...
mod tests {
    ...
    fn kernel_sources_exist() {
        let kernels = ExtGpuMiner::list_kernels();
    ...
    fn zelhash_blake2b_state_zelproof_personalization() {
        let header = [0u8; 140];
        let state = ExtGpuMiner::zelhash_blake2b_state(&header);
        assert_eq!(state.len(), 8);
        // Determinism
        let state2 = ExtGpuMiner::zelhash_blake2b_state(&header);
        assert_eq!(state, state2, "ZelHash Blake2b state must be deterministic");
        // Different input → different state
        let mut header2 = [0u8; 140];
        header2[0] = 1;
        let state3 = ExtGpuMiner::zelhash_blake2b_state(&header2);
        assert_ne!(
            state, state3,
    ...
    fn equihash_blake2b_state_zcash_personalization() {
        ...
        let state = ExtGpuMiner::zcash_blake2b_state(&header);
        ...
        let state2 = ExtGpuMiner::zcash_blake2b_state(&header);
        assert_eq!(state, state2, "Blake2b state must be deterministic");

        // Different input → different state
        let mut header2 = [0u8; 140];
        header2[0] = 1;
        let state3 = ExtGpuMiner::zcash_blake2b_state(&header2);
        assert_ne!(
            state, state3,
            "Different headers must produce different states"
        );
    ...
    fn equihash_solution_encoding_size() {
        ...
        let encoded = ExtGpuMiner::encode_equihash_solution(&inputs, 24, 7);
        ...
        let encoded2 = ExtGpuMiner::encode_equihash_solution(&inputs2, 24, 7);
