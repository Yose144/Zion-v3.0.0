...
//! let mut miner = ExtGpuMiner::new().unwrap();
//!
...
/// [`ExtGpuMiner::set_ethash_dag`].  `size_entries` is the number of 128-byte
...
//
// Stubs for algorithms not needed for ZANO ProgPoW. These are used by
// kheavyhash/autolykos code paths which we don't need for ZANO mining.
...
"#;
        ...
        for batch in 0..num_batches {
            ...
            // Progress report only when pct crosses a 10% boundary or on the last batch
            let pct = ((batch + 1) * 100) / num_batches;
            let prev_pct = (batch * 100) / num_batches;
            if (pct / 10) > (prev_pct / 10) || batch + 1 == num_batches {
    ...
    pub fn generate_ethash_dag_on_gpu(&mut self, epoch: u32) -> Result<()> {
        // Cache check: if DAG for this epoch already exists, skip regeneration
...
pub mod opencl_backend {
    ...
    /// Create a new OpenCL backend (wraps ExtGpuMiner).
