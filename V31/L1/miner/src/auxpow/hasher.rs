//! External algorithm hashing stubs.
//!
//! These are *scaffold* implementations that return deterministic 32-byte
//! hashes for the AuxPoW runtime. Native algorithms (RandomX, VerusHash,
//! GhostRider, kheavyhash, progpow, ethash, kawpow, etc.) will eventually be
//! provided by `zion-native-ffi` and the GPU backends. The runtime only needs
//! a uniform `hash_for_coin` signature.
