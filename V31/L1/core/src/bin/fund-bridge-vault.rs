...
//!
//! Usage:
//!   ZION_WALLET_SK_HEX=<hex> ZION_RPC_ADDR=127.0.0.1:9445 cargo run --release --manifest-path V3/Cargo.toml -p zion-core --bin fund-bridge-vault
...
fn main() {
    ...
    // Submit via RPC
    let rpc_addr = std::env::var("ZION_RPC_ADDR").unwrap_or_else(|_| "127.0.0.1:9445".into());
    let payload = json!({
        "jsonrpc": "2.0",
        "id": 1,
