...
pub struct WarpRouter {
    ...
    /// Maximum daily throughput per chain (atomic units).
    pub daily_limit: u64,
...
impl WarpRouter {
    ...
    pub fn initiate_outbound(
        ...
        if current_volume + proof.amount_atomic > self.daily_limit {
            return Err(WarpError::DailyLimitExceeded {
                amount: proof.amount_atomic,
        ...
        *self.daily_volume.entry(chain_name).or_insert(0) += proof.amount_atomic;
...
mod tests {
    ...
    fn test_outbound_transfer_basic() {
        ...
        assert!(t.fee_atomic > 0);
    }

    #[test]
    fn test_outbound_transfer_timelock() {
        let mut router = test_router();
        router.timelock_threshold = 500_000; // set low for test
