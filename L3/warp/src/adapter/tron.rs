...
impl TronAdapter {
    pub fn new() -> Self {
        ...
        let api_url = std::env::var("WARP_TRON_API")
        ...
        if let Some(ref key) = api_key {
            ...
            headers.insert("TRON-PRO-API-KEY",
                reqwest::header::HeaderValue::from_str(key).unwrap());
            builder = builder.default_headers(headers);
        }
        Self { network, api_url, api_key, client: builder.build().unwrap() }
    }

    async fn get_now_block(&self) -> WarpResult<u64> {
        let url = format!("{}/wallet/getnowblock", self.api_url);
        let resp: TronBlockResp = self.client.post(&url).send().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?
            .json().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?;
    ...
    async fn get_contract_events(&self, contract: &str) -> WarpResult<Vec<TronEvent>> {
        ...
        let resp: TronEventsResp = self.client.get(&url).send().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?
            .json().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?;
    ...
    fn event_to_proof(&self, ev: &TronEvent, current_block: u64) -> Option<DepositProof> {
        ...
        let amount: u64 = res.amount.as_deref()
...
impl ChainAdapter for TronAdapter {
    fn family(&self) -> ChainFamily { ChainFamily::Tron }
    fn name(&self) -> &str { "tron" }

    async fn health_check(&self) -> WarpResult<bool> {
        match self.get_now_block().await {
            Ok(n) => { info!("[WARP][tron] Health OK — block {}", n); Ok(true) }
            Err(e) => { warn!("[WARP][tron] Health FAIL: {}", e); Ok(false) }
        }
    }
    ...
    async fn watch_events(&self) -> WarpResult<Vec<DepositProof>> {
        let contract = match wzion_contract(&self.network) {
            Some(c) => c,
            None => { debug!("[WARP][tron] No wZION contract configured"); return Ok(vec![]); }
        };
        let tip = self.get_now_block().await?;
        let events = self.get_contract_events(contract).await?;
        let proofs: Vec<_> = events.iter()
    ...
    async fn confirmations(&self, tx_hash: &str) -> WarpResult<u64> {
        ...
        let resp: TronTxInfo = self.client.get(&url).send().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?
            .json().await
            .map_err(|e| WarpError::AdapterError { chain: "tron".into(), reason: e.to_string() })?;
        if let Some(c) = resp.confirmations { return Ok(c); }
...
mod tests {
    ...
    fn test_event_to_proof_valid() {
        ...
        assert_eq!(proof.amount_atomic, 2_000_000);
    ...
    fn test_event_to_proof_zero_amount_skipped() {
        ...
        let ev = TronEvent {
            ...
            block_number: Some(1),
            result: Some(TronEventResult { from: None, amount: Some("0".into()), destAddr: None }),
        };
    ...
    async fn test_tron_execute_mint_is_err() {
        let inst = MintInstruction {
            dest_chain: "tron".into(), recipient: "TAddr".into(),
